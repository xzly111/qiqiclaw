...
const qiqiclawAPI = {
  ...
  validateQiqiclawHome: (dir: string): Promise<boolean> =>
    ipcRenderer.invoke("validate-qiqiclaw-home", dir),

  adoptQiqiclawHome: (dir: string): Promise<boolean> =>
    ipcRenderer.invoke("adopt-qiqiclaw-home", dir),
  ...
  providerAuthStatus: (
    ...
    error?: string;
    error_code?: string;
  }> => ipcRenderer.invoke("provider-auth-status", provider),
  ...
  getQiqiclawHome: (profile?: string): Promise<string> =>
    ipcRenderer.invoke("get-qiqiclaw-home", profile),
  ...
  prepareLocalApi: (
    ...
    ipcRenderer.invoke("prepare-local-api", profile),
  ...
  addModel: (
    ...
    ipcRenderer.invoke("add-model", name, provider, model, baseUrl, apiKey, profile),
...
if (process.contextIsolated) {
  try {
    ...
    contextBridge.exposeInMainWorld("qiqiclawAPI", qiqiclawAPI);
  } catch (error) {
    console.error(error);
  }
} else {
  ...
  // @ts-ignore (define in dts)
  window.qiqiclawAPI = qiqiclawAPI;
}
