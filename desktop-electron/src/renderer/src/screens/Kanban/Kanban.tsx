...
const ACTIVE_BOARD_LS_KEY = "qiqiclaw:kanban:active-board";
...
function Kanban({ profile, visible }: KanbanProps): React.JSX.Element {
    ...
    async (silent = false): Promise<void> => {
      ...
      try {
        ...
        const [boardsRes, tasksRes, hqRes] = await Promise.all([
          window.qiqiclawAPI.kanbanListBoards(false, profile),
          wantHq
            ? Promise.resolve<TasksRes>({ success: true, data: [] })
            : window.qiqiclawAPI.kanbanListTasks({
                includeArchived: false,
                profile,
              }),
          window.qiqiclawAPI.kanbanListClaw3dHqTasks(),
        ]);
  ...
  useEffect(() => {
    ...
    window.qiqiclawAPI.listProfiles().then((profiles) => {
  ...
  useEffect(() => {
    ...
    window.qiqiclawAPI.kanbanGetTask(detailTaskId, profile).then((res) => {
      if (cancelled) return;
  ...
  async function handleCreate(): Promise<void> {
    ...
    const res = await window.qiqiclawAPI.kanbanCreateTask(
      {
        title: newTitle.trim(),
  ...
  async function handleBoardSwitch(slug: string): Promise<void> {
    ...
    const res = await window.qiqiclawAPI.kanbanSwitchBoard(slug, profile);
    setActionBusy(null);
    if (!res.success) {
  ...
  async function handleCreateBoard(): Promise<void> {
    ...
    const res = await window.qiqiclawAPI.kanbanCreateBoard(
      newBoardSlug.trim(),
  ...
  async function handleMove(task: KanbanTask, target: string): Promise<void> {
    ...
    if (target === "done") {
      res = await window.qiqiclawAPI.kanbanCompleteTask(
        task.id,
        undefined,
        profile,
      );
      ...
      res = await window.qiqiclawAPI.kanbanBlockTask(
        task.id,
        reason || undefined,
        profile,
      );
    } else if (target === "ready" && task.status === "blocked") {
      res = await window.qiqiclawAPI.kanbanUnblockTask(task.id, profile);
    } else {
      setActionBusy(null);
      setError(
  ...
  async function handleSpecify(task: KanbanTask): Promise<void> {
    setActionBusy(task.id);
    const res = await window.qiqiclawAPI.kanbanSpecifyTask(task.id, profile);
    setActionBusy(null);
    if (!res.success) {
  ...
  async function handleArchive(task: KanbanTask): Promise<void> {
    if (!window.confirm(t("kanban.confirmArchive", { title: task.title })))
      return;
    setActionBusy(task.id);
    const res = await window.qiqiclawAPI.kanbanArchiveTask(task.id, profile);
    setActionBusy(null);
    if (!res.success) {
  ...
  async function handleReclaim(task: KanbanTask): Promise<void> {
    setActionBusy(task.id);
    const res = await window.qiqiclawAPI.kanbanReclaimTask(
      task.id,
  ...
  async function handleDispatch(): Promise<void> {
    ...
    const res = await window.qiqiclawAPI.kanbanDispatchOnce(false, profile);
    setActionBusy(null);
    if (!res.success) {
