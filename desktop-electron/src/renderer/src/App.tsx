...
function App(): React.JSX.Element {
  ...
  const runInstallCheck = useCallback(async () => {
    ...
    try {
      const conn = await window.qiqiclawAPI.getConnectionConfig();
      ...
      if (conn.mode === "ssh" && conn.ssh) {
        ...
        try {
          await window.qiqiclawAPI.startSshTunnel();
          next = "main";
        } catch (tunnelErr) {
        ...
        const ok = await window.qiqiclawAPI.testRemoteConnection(conn.remoteUrl);
        if (ok) {
          next = "main";
        } else {
          error = `Cannot reach remote QiQiClaw at ${conn.remoteUrl}. Check the URL or switch to local mode.`;
          next = "welcome";
        }
      } else {
        const status = await window.qiqiclawAPI.checkInstall();
        if (!status.installed) {
          next = "welcome";
    ...
    // Python + script paths (QIQICLAW_PYTHON / QIQICLAW_SCRIPT in installer.ts),
    ...
    if ((next === "main" || next === "setup") && !isRemote) {
      window.qiqiclawAPI.verifyInstall().then((ok) => {
