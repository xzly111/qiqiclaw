...
function useMediaContextMenu(
  ...
  return (event) => {
    ...
    window.qiqiclawAPI.showMediaMenu(token.src, token.name, {
        ...
        >
          ...
          >
            <button
              ...
              onClick={() =>
                window.qiqiclawAPI.saveMediaFile(token.src, token.name)
              }
            >
              <Download size={14} />
              Save image
...
export function DownloadChip({
  ...
  return (
    <button
      ...
      onClick={() => window.qiqiclawAPI.saveMediaFile(token.src, token.name)}
      onContextMenu={onContextMenu}
    >
      <Download size={14} />
      {token.name}
