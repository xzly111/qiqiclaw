...
import {
  ...
  validateQiqiclawHome,
  setQiqiclawHomeOverride,
  getHermesVersion,
  clearVersionCache,
  runHermesDoctor,
  runHermesUpdate,
...
import {
  runQiqiclawAuthLogin,
  cancelQiqiclawAuthLogin,
  detectDeviceCode,
...
import {
  ...
  stopHealthPolling,
  restartGateway,
  restartInstalledGateway,
  stopInstalledGatewayService,
...
const AUTH_REQUIRED_MODEL_PROVIDERS = new Set([
  "nous",
  "openai-codex",
  "qwen-oauth",
  "google-gemini-cli",
...
async function ensureApiServerKey(profile?: string): Promise<string> {
...
function setupIPC(): void {
  ...
  ipcMain.handle("start-install", async (event) => {
    try {
      await stopInstalledGatewayService();
      await runInstall((progress: InstallProgress) => {
        event.sender.send("install-progress", progress);
      }, mainWindow);
      await ensureApiServerKey();
      const restarted = await restartInstalledGateway();
      if (!restarted) restartGateway();
      await ensureLocalApiUsable();
      return { success: true };
    } catch (err) {
  ...
  ipcMain.handle("validate-qiqiclaw-home", (_event, dir: string) =>
    validateQiqiclawHome(dir),
  );
  ipcMain.handle("adopt-qiqiclaw-home", (_event, dir: string) => {
    if (!validateQiqiclawHome(dir)) return false;
    // Persist the choice only. QIQICLAW_HOME is resolved once at module
    ...
    setQiqiclawHomeOverride(dir);
    return true;
  });
  ipcMain.handle("quit-app", () => app.quit());

  // QiQiClaw engine info
  ipcMain.handle("get-hermes-version", async () => {
    const conn = getConnectionConfig();
    if (conn.mode === "ssh" && conn.ssh) return sshGetQiqiclawVersion(conn.ssh);
    ...
    try {
      ...
      if (conn.mode === "ssh" && conn.ssh) {
        event.sender.send("install-progress", {
          step: 1,
          totalSteps: 1,
          title: "Updating remote QiQiClaw",
          detail: "Running QiQiClaw update over SSH...",
          log: "Running QiQiClaw update over SSH...\n",
        });
  ...
  // OAuth provider sign-in — spawns `qiqiclaw auth add <provider> --type
  ...
  ipcMain.handle("oauth-login", (event, provider: string, profile?: string) => {
    ...
    return runQiqiclawAuthLogin(
      provider,
      (chunk) => {
        // The user can close the modal mid-flow before cancelQiqiclawAuthLogin
  ...
  ipcMain.handle("oauth-login-cancel", () => cancelQiqiclawAuthLogin());
  ipcMain.handle("provider-auth-status", (_event, provider: string) =>
  ...
  ipcMain.handle("get-qiqiclaw-home", (_event, profile?: string) => {
    const conn = getConnectionConfig();
    if (conn.mode === "ssh" && conn.ssh)
      return sshGetQiqiclawHome(conn.ssh, profile);
    return getQiqiclawHome(profile);
  });
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (AUTH_REQUIRED_MODEL_PROVIDERS.has(cleanProvider)) {
        const status = await getProviderAuthStatus(cleanProvider);
        if (!status.logged_in) {
          throw new Error(
            status.error ||
              `${cleanProvider} is not authenticated. Run qiqiclaw model to configure it.`,
          );
        }
      }
      ...
      // Await readiness before returning to the renderer; otherwise a user can switch
      // models and immediately send a message that races the old gateway/session.
      const changed =
        prev.provider !== provider ||
        prev.model !== model ||
        prev.baseUrl !== baseUrl;
      if (changed && isGatewayRunning()) {
        const restarted = await restartInstalledGateway(profile);
        if (!restarted) restartGateway(profile);
        await ensureLocalApiUsable(profile);
      }
  ...
  ipcMain.handle(
    ...
    async (_event, profile?: string) => {
      const key = await ensureApiServerKey(profile);
      // Restart gateway so it picks up the new key immediately. Prefer the
      // installed CLI/systemd path; fall back to the app-managed gateway.
      const restarted = await restartInstalledGateway(profile);
      if (!restarted && isGatewayRunning()) {
        restartGateway(profile);
      }
      return { key };
    },
  ...
  ipcMain.handle("prepare-local-api", async (_event, profile?: string) => {
    if (isRemoteMode()) return { ok: true };
    await ensureApiServerKey(profile);
    const restarted = await restartInstalledGateway(profile);
    if (!restarted && !isGatewayRunning()) {
      startGateway(profile);
    } else if (!restarted && isGatewayRunning()) {
      restartGateway(profile);
    }
    return ensureLocalApiUsable(profile);
  });
  ...
  ipcMain.handle(
    ...
    (
      ...
      const normalizedRemoteUrl =
        mode === "remote" ? coerceQiqiclawGatewayUrl(remoteUrl) : remoteUrl;
      setConnectionConfig({
        ...existing,
        mode,
  ...
  ipcMain.handle(
    ...
    (
      ...
      return addCredentialPoolEntry(provider, apiKey, label, baseUrl, profile);
    },
  );
  ...
  ipcMain.handle(
    ...
    async (
      ...
      const discovery = await discoverProviderModels(
        cleanProvider,
        cleanProvider === "custom" ? cleanBaseUrl : undefined,
        apiKey,
        profile,
      );
      if (
        discovery.status !== "ok" ||
        !discovery.models.includes(cleanModel)
      ) {
        throw new Error(
          `Model "${cleanModel}" was not returned by the provider model list.`,
        );
      }
      const conn = getConnectionConfig();
      if (conn.mode === "ssh" && conn.ssh) {
        return sshAddModel(conn.ssh, name, cleanProvider, cleanModel, cleanBaseUrl);
      }
      return addModel(name.trim(), cleanProvider, cleanModel, cleanBaseUrl);
    },
  );
...
function buildMenu(): void {
  ...
  const template: Electron.MenuItemConstructorOptions[] = [
    ...
    },
    {
      ...
      submenu: [
        {
          label: "QiQiClaw on GitHub",
          click: (): void => {
            openExternalUrl("https://github.com/xzly111/qiqiclaw");
          },
        },
        {
          label: "Report an Issue",
          click: (): void => {
            openExternalUrl("https://github.com/xzly111/qiqiclaw/issues");
          },
        },
      ],
    },
...
app.whenReady().then(() => {
  app.name = "QiQiClaw";
  electronApp.setAppUserModelId("com.qiqiclaw.desktop");
