...
import { getEnhancedPath, QIQICLAW_HOME } from "./installer";
import { stripAnsi, safeWriteFile } from "./utils";
import { getApiServerKey, getConnectionConfig, getModelConfig } from "./config";
import http from "http";

const QIQICLAW_OFFICE_REPO = "https://github.com/fathah/qiqiclaw-office";
const QIQICLAW_OFFICE_DIR = join(QIQICLAW_HOME, "qiqiclaw-office");
const DEV_PID_FILE = join(QIQICLAW_HOME, "claw3d-dev.pid");
const ADAPTER_PID_FILE = join(QIQICLAW_HOME, "claw3d-adapter.pid");
const PORT_FILE = join(QIQICLAW_HOME, "claw3d-port");
const WS_URL_FILE = join(QIQICLAW_HOME, "claw3d-ws-url");
...
/**
 * The model QiQiClaw Office should default to. Office runs against the same
...
/**
 * Build the `.env` QiQiClaw Desktop writes into the qiqiclaw-office directory.
 * Exported so the contents (notably `QIQICLAW_MODEL`, issue #256) can be
...
export function buildOfficeEnv(opts: {
  ...
  return [
    "# Auto-configured by QiQiClaw Desktop",
    `PORT=${opts.port}`,
    `HOST=127.0.0.1`,
    `NEXT_PUBLIC_GATEWAY_URL=${opts.url}`,
    `CLAW3D_GATEWAY_URL=${opts.url}`,
    `CLAW3D_GATEWAY_TOKEN=${opts.apiKey}`,
    `QIQICLAW_API_KEY=${opts.apiKey}`,
    `QIQICLAW_ADAPTER_PORT=18789`,
    `QIQICLAW_MODEL=${opts.model || "deepseek-chat"}`,
    `QIQICLAW_AGENT_NAME=QiQiClaw`,
    "",
  ].join("\n");
}
...
function writeClaw3dSettings(wsUrl?: string): void {
  ...
  try {
    if (existsSync(QIQICLAW_OFFICE_DIR)) {
      const envPath = join(QIQICLAW_OFFICE_DIR, ".env");
      safeWriteFile(
        envPath,
        buildOfficeEnv({
          port: getSavedPort(),
...
export interface Claw3dStatus {
  ...
  // Populated in SSH tunnel mode when a Claw3D / qiqiclaw-office service is
...
// listener). Used to detect remote Claw3D / qiqiclaw-office without dragging
...
export async function getClaw3dStatus(): Promise<Claw3dStatus> {
  const cloned = existsSync(join(QIQICLAW_OFFICE_DIR, "package.json"));
  const installed = existsSync(join(QIQICLAW_OFFICE_DIR, "node_modules"));
  ...
  // SSH tunnel mode: probe the remote host for a Claw3D / qiqiclaw-office
...
export function startDevServer(): boolean {
  ...
  if (!existsSync(join(QIQICLAW_OFFICE_DIR, "node_modules"))) return false;
  ...
  const env = {
    ...
    HOME: homedir(),
    TERM: "dumb",
    QIQICLAW_API_KEY: getApiServerKey(),
    PORT: String(port),
  };
...
export function startAdapter(): boolean {
  ...
  adapterError = "";
  adapterLogs = "";
  // The qiqiclaw-office adapter authenticates to the QiQiClaw gateway with
  // `Authorization: Bearer ${QIQICLAW_API_KEY}`. Without it, a gateway that
  ...
  proc.on("close", (code) => {
    if (code && code !== 0 && !adapterError) {
      adapterError = `QiQiClaw adapter exited with code ${code}`;
    }
    adapterProcess = null;
    cleanupPid(ADAPTER_PID_FILE);
  });
...
export function startAll(): { success: boolean; error?: string } {
  if (!existsSync(join(QIQICLAW_OFFICE_DIR, "node_modules"))) {
    return {
      success: false,
  ...
  if (!adapterOk) {
    return { success: false, error: "Failed to start QiQiClaw adapter" };
  }
