...
const PROFILES_DIR = join(QIQICLAW_HOME, "profiles");
...
export async function listProfiles(): Promise<ProfileInfo[]> {
  ...
  // Default profile is QIQICLAW_HOME itself
  const [
    defaultConfig,
    defaultHasEnv,
    defaultHasSoul,
    ...
    readProfileConfig(QIQICLAW_HOME),
    fileExists(join(QIQICLAW_HOME, ".env")),
    fileExists(join(QIQICLAW_HOME, "SOUL.md")),
    countSkills(QIQICLAW_HOME),
    isGatewayRunning(QIQICLAW_HOME),
  ]);
  ...
  // Named profiles under QIQICLAW_HOME/profiles/
  if (existsSync(PROFILES_DIR)) {
    try {
      ...
      const profilePromises = dirs.map(async (name) => {
        ...
        // Any subdirectory of QIQICLAW_HOME/profiles/ is treated as a profile.
...
export function createProfile(
  ...
  try {
    const args = clone
      ...
      : ["profile", "create", name];
    execFileSync(QIQICLAW_PYTHON, qiqiclawCliArgs(args), {
      cwd: QIQICLAW_REPO,
...
export function deleteProfile(name: string): {
  ...
  try {
    execFileSync(
      QIQICLAW_PYTHON,
      qiqiclawCliArgs(["profile", "delete", name, "--yes"]),
      {
        cwd: QIQICLAW_REPO,
        env: buildQiqiclawEnv(),
        stdio: "pipe",
...
export function setActiveProfile(name: string): void {
  ...
  try {
    execFileSync(QIQICLAW_PYTHON, qiqiclawCliArgs(["profile", "use", name]), {
      cwd: QIQICLAW_REPO,
      env: buildQiqiclawEnv(),
      stdio: "pipe",
      timeout: 10000,
