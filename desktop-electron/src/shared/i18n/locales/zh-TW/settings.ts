export default {
  ...
  analytics: {
    label: "傳送匿名使用情況分析",
    hint: "透過向專案的 PostHog 執行個體（位於歐盟）傳送匿名、彙總的使用資料，協助改進 QiQiClaw。您隨時可以關閉此功能。",
    disclosure: {
  ...
  poolHint:
    "為同一供應商新增多個 API Key，以便自動輪換和負載均衡。QiQiClaw 會在它們之間輪流使用。",
  add: "新增",
  remove: "移除",
  keyLabel: "金鑰",
  empty: "(空)",
  dataSection: "資料",
  dataHint: "匯出或匯入你的 QiQiClaw 設定、工作階段、技能和記憶。",
  backingUp: "正在備份...",
  exportBackup: "匯出備份",
  importing: "正在匯入...",
  importBackup: "匯入備份",
  ...
  migrationDesc:
    "在 <code>{{path}}</code> 發現 OpenClaw。你可以將設定、API Key、工作階段和技能遷移到 QiQiClaw。",
  migrationDismiss: "不再顯示",
  migrating: "遷移中...",
  migrateToQiQiClaw: "遷移到 QiQiClaw",
  skip: "跳過",
  ...
  modeLocal: "本機",
  modeRemote: "遠端",
  modeLocalHint: "使用本機安裝的 QiQiClaw",
  modeRemoteHint: "連線到網路或雲伺服器上的 QiQiClaw API",
  remoteUrl: "遠端伺服器位址",
  remoteUrlHint:
    "QiQiClaw API 伺服器位址（需開放 /health 和 /v1/chat/completions）",
  remoteApiKey: "API 金鑰",
  remoteApiKeyHint:
  ...
  serverConfigHint:
    "你已連線到遠端 QiQiClaw 伺服器。模型選擇、供應商 API Key 和憑證均在伺服器的 <code>~/.qiqiclaw/.env</code> 和 <code>config.yaml</code> 中管理。請在主機上編輯（例如 <code>docker exec -it hermes vi /opt/data/.env</code>）然後重新啟動容器。",
  connectionMode: "模式",
