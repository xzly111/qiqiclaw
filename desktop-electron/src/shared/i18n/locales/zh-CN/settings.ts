export default {
  ...
  analytics: {
    label: "发送匿名使用情况分析",
    hint: "通过向项目的 PostHog 实例（托管于欧盟）发送匿名、聚合的使用数据来帮助改进 QiQiClaw。您可以随时关闭。",
    disclosure: {
  ...
  customProviderHint:
    "使用任何兼容 OpenAI 的接口或中转站(LM Studio、Ollama、vLLM 等)",
  ...
  modelNotDiscovered: "该模型不在提供商返回的模型列表中，不会加入模型库",
  customBaseUrlHint: "兼容 OpenAI 的 API / 中转站地址",
  poolHint:
    "为同一提供商或中转站添加多个 API Key,以便自动轮换和负载均衡。选择 OpenAI 兼容 / 中转站时需填写 Base URL。",
  add: "添加",
  remove: "移除",
  keyLabel: "密钥",
  empty: "(空)",
  dataSection: "数据",
  dataHint: "导出或导入你的 QiQiClaw 配置、会话、技能和记忆。",
  backingUp: "正在备份...",
  exportBackup: "导出备份",
  importing: "正在导入...",
  importBackup: "导入备份",
  ...
  migrationDesc:
    "在 <code>{{path}}</code> 发现 OpenClaw。你可以将配置、API Key、会话和技能迁移到 QiQiClaw。",
  migrationDismiss: "不再显示",
  migrating: "迁移中...",
  migrateToQiQiClaw: "迁移到 QiQiClaw",
  skip: "跳过",
  ...
  modeLocal: "本地",
  modeRemote: "远程",
  modeLocalHint: "使用本机安装的 QiQiClaw",
  modeRemoteHint: "连接到网络或云服务器上的 QiQiClaw API",
  remoteUrl: "远程服务器地址",
  remoteUrlHint:
    "QiQiClaw API 服务器地址（需开放 /health 和 /v1/chat/completions）",
  remoteApiKey: "API 密钥",
  remoteApiKeyHint:
  ...
  serverConfigHint:
    "你已连接到远程 QiQiClaw 服务器。模型选择、提供商 API Key 和凭据均在服务器的 <code>~/.qiqiclaw/.env</code> 和 <code>config.yaml</code> 中管理。请在主机上编辑后重启服务。",
  connectionMode: "模式",
