...
/**
 * Coverage for the in-app OAuth login feature (`src/main/qiqiclaw-auth.ts`):
 *
 *   - provider allowlist (`isOAuthLoginProvider`)
 *   - `runQiqiclawAuthLogin` spawns `qiqiclaw auth add <provider> --type
 *     oauth`, streams output, and maps exit code → success/failure
 *   - the single-flight guard rejects a second concurrent login
 *   - `cancelQiqiclawAuthLogin` kills the in-flight subprocess
 */
...
// qiqiclaw-auth.ts only consumes `spawn` from child_process, so a minimal
...
describe("runQiqiclawAuthLogin", () => {
  ...
  it("refuses an unsupported provider without spawning", async () => {
    const result = await runQiqiclawAuthLogin("openrouter", () => {});
  ...
  it("spawns `auth add <provider> --type oauth` and streams output", async () => {
    const chunks: string[] = [];
    const promise = runQiqiclawAuthLogin("google-gemini-cli", (c) =>
      chunks.push(c),
    );
  ...
  it("passes the profile flag when a named profile is given", async () => {
    const promise = runQiqiclawAuthLogin("qwen-oauth", () => {}, "work");
    const args = spawnSpy.mock.calls[0][1] as string[];
    expect(args).toEqual([
      "-p",
      "work",
  ...
  it("reports failure on a non-zero exit code", async () => {
    const promise = runQiqiclawAuthLogin("qwen-oauth", () => {});
  ...
  it("includes CLI output when a sign-in process fails", async () => {
    const promise = runQiqiclawAuthLogin("qwen-oauth", () => {});
    lastProc().stderr.emit(
      "data",
      Buffer.from("Qwen CLI credentials not found.\n"),
    );
    lastProc().emit("close", 1, null);
    const result = await promise;
    expect(result.success).toBe(false);
    expect(result.error).toContain("Qwen CLI credentials not found");
  });

  it("reports cancellation when the process is killed by signal", async () => {
    const promise = runQiqiclawAuthLogin("minimax-oauth", () => {});
  ...
  it("rejects a second concurrent login while one is in flight", async () => {
    const first = runQiqiclawAuthLogin("openai-codex", () => {});
    const second = await runQiqiclawAuthLogin("minimax-oauth", () => {});
...
describe("cancelQiqiclawAuthLogin", () => {
  beforeEach(() => {
    spawnSpy.mockClear();
    fakeProcs.length = 0;
  });
  ...
  it("returns false when no login is running", () => {
    expect(cancelQiqiclawAuthLogin()).toBe(false);
  });

  it("kills the in-flight subprocess and returns true", async () => {
    const promise = runQiqiclawAuthLogin("openai-codex", () => {});
    const proc = lastProc();

    expect(cancelQiqiclawAuthLogin()).toBe(true);
