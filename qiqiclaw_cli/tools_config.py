"""
Unified tool configuration for QiQiClaw.

`qiqiclaw tools` and `qiqiclaw setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.qiqiclaw/config.yaml under
...
from qiqiclaw_cli.nous_subscription import (
...
from qiqiclaw_cli.cli_output import (  # noqa: E402 — late import block
...
CONFIGURABLE_TOOLSETS = [
    ...
    ("qiqiclaw_file_modules", "🗂️ QiQiClaw File Modules", "desktop all-files, processors, mounts"),
...
# They're still in _QIQICLAW_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
_DEFAULT_OFF_TOOLSETS = {"moa", "homeassistant", "rl", "spotify", "discord", "discord_admin"}

# Platform-scoped toolsets: only appear in the `qiqiclaw tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``qiqiclaw tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from qiqiclaw_cli.plugins import discover_plugins, get_plugin_toolsets
...
from qiqiclaw_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "QIQICLAW_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "QIQICLAW_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "QIQICLAW_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "QIQICLAW_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "QIQICLAW_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
    ...
    if post_setup_key in ("agent_browser", "browserbase"):
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                _print_success("    Node.js 依赖已安装")
            else:
                from qiqiclaw_constants import display_qiqiclaw_home
                _print_warning(f"    npm install 失败 - 请手动运行: cd {display_qiqiclaw_home()}/qiqiclaw && npm install")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    未找到 Node.js - 浏览器工具需要: npm install (在 qiqiclaw 目录中)")
            return
        ...
        try:
            ...
            _print_warning(f"    无法检查 Chromium 状态: {exc}")
            return

        if _chromium_installed():
            _print_success("    Chromium 浏览器已安装")
            return

        if _running_in_docker():
            _print_warning(
                "    Chromium 缺失，但您正在 Docker 中运行。"
            )
            _print_info(
                "    拉取最新镜像以获取捆绑的 Chromium:"
            )
            _print_info(
                "      docker pull ghcr.io/nousresearch/qiqiclaw:latest"
            )
            return

        if not npx_bin:
            _print_warning(
                "    未找到 npx - 请手动安装 Chromium: npx agent-browser install --with-deps"
            )
            return

        _print_info("    正在安装 Chromium (~170MB 一次性下载)...")
        import subprocess
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                _print_warning("    Chromium 安装失败:")
                tail = (result.stderr or result.stdout or "").strip().splitlines()[-3:]
                for line in tail:
                    _print_info(f"      {line[:200]}")
                _print_info("    请手动运行: npx agent-browser install --with-deps")
        except subprocess.TimeoutExpired:
            _print_warning("    Chromium 安装超时 (>10分钟)")
            _print_info("    请手动运行: npx agent-browser install --with-deps")
        except Exception as exc:
            _print_warning(f"    Chromium 安装失败: {exc}")
            _print_info("    请手动运行: npx agent-browser install --with-deps")
        ...
        if not camofox_dir.exists() and shutil.which("npm"):
            ...
            if result.returncode == 0:
                _print_success("    Camofox 已安装")
            else:
                _print_warning("    npm install 失败 - 请手动运行: npm install")
        if camofox_dir.exists():
            _print_info("    启动 Camofox 服务器:")
            _print_info("      npx @askjo/camofox-browser")
            _print_info("    首次运行会下载 Camoufox 引擎 (~300MB)")
            _print_info("    或使用 Docker: docker run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser")
        elif not shutil.which("npm"):
            _print_warning("    未找到 Node.js。通过 Docker 安装 Camofox:")
            _print_info("      docker run -p 9377:9377 -e CAMOFOX_PORT=9377 jo-inc/camofox-browser")

    elif post_setup_key == "kittentts":
        try:
            __import__("kittentts")
            _print_success("    kittentts 已安装")
            return
        except ImportError:
            pass
        import subprocess
        _print_info("    正在安装 kittentts (~25-80MB 模型, 仅 CPU)...")
        wheel_url = (
        ...
        try:
            ...
            if result.returncode == 0:
                _print_success("    kittentts 已安装")
                _print_info("    语音: Jasper, Bella, Luna, Bruno, Rosie, Hugo, Kiki, Leo")
                _print_info("    模型: KittenML/kitten-tts-nano-0.8-int8 (25MB), micro (41MB), mini (80MB)")
            else:
                _print_warning("    kittentts 安装失败:")
                _print_info(f"      {result.stderr.strip()[:300]}")
                _print_info(f"    请手动运行: python -m pip install -U '{wheel_url}' soundfile")
        except subprocess.TimeoutExpired:
            _print_warning("    kittentts 安装超时 (>5分钟)")
            _print_info(f"    请手动运行: python -m pip install -U '{wheel_url}' soundfile")

    elif post_setup_key == "piper":
        try:
            __import__("piper")
            _print_success("    piper-tts 已安装")
        except ImportError:
            import subprocess
            _print_info("    正在安装 piper-tts (~14MB wheel, 首次使用时下载语音)...")
            try:
                result = subprocess.run(
                ...
                if result.returncode == 0:
                    ...
                    _print_warning("    piper-tts 安装失败:")
                    _print_info(f"      {result.stderr.strip()[:300]}")
                    _print_info("    请手动运行: python -m pip install -U piper-tts")
                    return
            except subprocess.TimeoutExpired:
                _print_warning("    piper-tts 安装超时 (>5分钟)")
                _print_info("    请手动运行: python -m pip install -U piper-tts")
                return
        _print_info("    默认语音: en_US-lessac-medium (首次 TTS 调用时下载)")
        _print_info("    完整语音列表: https://github.com/OHF-Voice/piper1-gpl/blob/main/docs/VOICES.md")
        _print_info("    通过在 ~/.qiqiclaw/config.yaml 中设置 tts.piper.voice 来切换语音")

    elif post_setup_key == "spotify":
        # Run the full `qiqiclaw auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.qiqiclaw/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from qiqiclaw_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    无法加载 Spotify 认证: {exc}")
            _print_info("    请手动运行: qiqiclaw auth spotify")
            return
        _print_info("    正在启动 Spotify 登录...")
        try:
            login_spotify_command(SimpleNamespace(
                client_id=None, redirect_uri=None, scope=None,
                no_browser=False, timeout=None,
            ))
            _print_success("    Spotify 已认证")
        except SystemExit as exc:
            # User aborted the wizard, or OAuth failed — don't fail the
            # toolset enable; they can retry with `qiqiclaw auth spotify`.
            _print_warning(f"    Spotify 登录未完成: {exc}")
            _print_info("    稍后运行: qiqiclaw auth spotify")
        except Exception as exc:
            _print_warning(f"    Spotify 登录失败: {exc}")
            _print_info("    请手动运行: qiqiclaw auth spotify")
        ...
        try:
            ...
            if tinker_dir.exists() and (tinker_dir / "pyproject.toml").exists():
                ...
                if result.returncode == 0:
                    _print_success("    tinker-atropos 已安装")
                else:
        ...
        try:
            __import__("langfuse")
            _print_success("    langfuse SDK 已安装")
        except ImportError:
            import subprocess
            _print_info("    正在安装 langfuse SDK...")
            result = subprocess.run(
            ...
            if result.returncode == 0:
                ...
                _print_warning("    langfuse SDK 安装失败 — 请手动运行: pip install langfuse")
        ...
        try:
            from qiqiclaw_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            enabled = _get_enabled_set()
            if "observability/langfuse" in enabled or "langfuse" in enabled:
                _print_success("    插件 observability/langfuse 已启用")
            else:
                enabled.add("observability/langfuse")
                _save_enabled_set(enabled)
                _print_success("    插件 observability/langfuse 已启用")
        except Exception as exc:
            _print_warning(f"    无法自动启用插件: {exc}")
            _print_info("    请手动运行: qiqiclaw plugins enable observability/langfuse")
        _print_info("    重启 QiQiClaw 以使追踪生效。")
        _print_info("    验证: qiqiclaw plugins list")
    ...
    *,
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"qiqiclaw-{platform}"
    ...
    # "qiqiclaw-cli" (which include all _QIQICLAW_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # (e.g. "qiqiclaw-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
    ...
    # otherwise saving via `qiqiclaw tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"qiqiclaw-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("qiqiclaw-")}
    ...
    # shipped as a bundled plugin but user must opt in via `qiqiclaw tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `qiqiclaw tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by qiqiclaw tools — default enabled
                enabled_toolsets.add(pts)
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (qiqiclaw-cli, qiqiclaw-telegram, etc.)
    ...
    # Opening `qiqiclaw tools` is the user's opt-in to reconfigure tools, so treat
...
def _configure_tool_category(ts_key: str, cat: dict, config: dict):
    ...
    if cat.get("requires_python"):
        ...
        if sys.version_info < req:
            print()
            _print_error(f"  {name} 需要 Python {req[0]}.{req[1]}+ (当前: {sys.version_info.major}.{sys.version_info.minor})")
            _print_info("  升级 Python 并重新安装以启用此工具。")
            return
...
def _select_plugin_image_gen_provider(plugin_name: str, config: dict) -> None:
    ...
    _print_success(f"  image_gen.provider 设置为: {plugin_name}")
...
def _configure_provider(provider: dict, config: dict):
    ...
    if provider.get("web_backend"):
        ...
        _print_success(f"  Web 后端设置为: {provider['web_backend']}")
    ...
    if all_configured:
        _print_success(f"  {provider['name']} 已配置!")
        plugin_name = provider.get("image_gen_plugin_name")
        if plugin_name:
            _select_plugin_image_gen_provider(plugin_name, config)
            return
...
def _configure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        choices = [
            "OpenRouter — 使用 Gemini",
            "OpenAI 兼容端点 — base URL, API key, 和视觉模型",
            "跳过",
        ]
        idx = _prompt_choice("  配置视觉后端", choices, 2)
        if idx == 0:
            _print_info("  获取密钥: https://openrouter.ai/keys")
...
def _configure_tool_category_for_reconfig(ts_key: str, cat: dict, config: dict):
    ...
    if len(providers) == 1:
        ...
        idx = _prompt_choice("  选择提供商:", provider_choices, default_idx)
        _reconfigure_provider(providers[idx], config)
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `qiqiclaw tools` and `qiqiclaw setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print(color("Q QiQiClaw Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        print()
        return
    print(color("Q QiQiClaw 工具配置", Colors.CYAN, Colors.BOLD))
    print(color("  为每个平台启用或禁用工具。", Colors.DIM))
    print(color("  需要 API 密钥的工具将在启用时进行配置。", Colors.DIM))
    print(color("  指南: https://qiqiclaw.nousresearch.com/docs/user-guide/features/tools", Colors.DIM))
    print()
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            if to_configure:
                print()
                print(color(f"  配置 {len(to_configure)} 个工具:", Colors.YELLOW))
                for ts_key in to_configure:
                    label = next((l for k, l, _ in _get_effective_configurable_toolsets() if k == ts_key), ts_key)
                    print(color(f"    • {label}", Colors.DIM))
                print(color("  您可以跳过现在不需要的任何工具。", Colors.DIM))
                print()
                for ts_key in to_configure:
            ...
            save_config(config)
            print(color(f"  ✓ 已保存 {pinfo['label']} 工具配置", Colors.GREEN))
            print()
    ...
    while True:
        ...
        if idx == _global_idx:
            ...
            if new_enabled != all_current:
                ...
                save_config(config)
                print(color("  ✓ 已保存所有平台的配置", Colors.GREEN))
                # Update choice labels
                for ci, pk in enumerate(platform_keys):
                    new_count = len(_get_platform_tools(config, pk, include_default_mcp_servers=False))
                    total = len(_get_effective_configurable_toolsets())
                    platform_choices[ci] = f"配置 {PLATFORMS[pk]['label']}  ({new_count}/{total} 已启用)"
            else:
    ...
    print()
    from qiqiclaw_constants import display_qiqiclaw_home
    print(color(f"  工具配置已保存到 {display_qiqiclaw_home()}/config.yaml", Colors.DIM))
    print(color("  更改将在下次 'qiqiclaw' 或网关重启时生效。", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    print(color(f"  连接到 {len(enabled_names)} 个服务器: {', '.join(enabled_names)}", Colors.DIM))
    ...
    if not server_tools:
        ...
        _print_info("请检查服务器命令/URL 是否正确以及依赖项是否已安装。")
        return
    ...
    print(color(f"  在 {len(server_tools)} 个服务器中找到 {total_tools} 个工具", Colors.GREEN))
    print()
    ...
    for server_name, tools in server_tools.items():
        ...
        _print_success(
            f"  {server_name}: {enabled_count} 已启用, {disabled_count} 已禁用"
        )
        any_changes = True
    ...
    if any_changes:
        ...
        print(color("  MCP tools: no changes", Colors.DIM))
