...
from qiqiclaw_cli.config import get_qiqiclaw_home, get_config_path, read_raw_config
...
DEFAULT_NOUS_CLIENT_ID = "qiqiclaw-cli"
...
SPOTIFY_DOCS_URL = "https://qiqiclaw.nousresearch.com/docs/user-guide/features/spotify"
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from qiqiclaw_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.qiqiclaw/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `qiqiclaw model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = _legacy_env("OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.qiqiclaw/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_qiqiclaw_home() / "auth.json"
    # Seat belt: if pytest is running and QIQICLAW_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch QIQICLAW_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".qiqiclaw" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set QIQICLAW_HOME to a tmp_path in your test fixture, or run "
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in QiQiClaw.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `qiqiclaw logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'qiqiclaw doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'qiqiclaw model' for available providers, or run 'qiqiclaw doctor' to diagnose config issues."
    ...
    raise AuthError(
        "No inference provider configured. Run 'qiqiclaw model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.qiqiclaw/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = _legacy_env("QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.qiqiclaw/auth/google_oauth.json (managed by agent.google_oauth).
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `qiqiclaw auth list` / `qiqiclaw status`."""
    try:
...
# Spotify auth — PKCE tokens stored in ~/.qiqiclaw/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("QIQICLAW_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set QIQICLAW_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("QIQICLAW_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("QIQICLAW_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("QIQICLAW_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `qiqiclaw auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `qiqiclaw auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `qiqiclaw auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `qiqiclaw auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.qiqiclaw/.env, and return it.

    Raises SystemExit if the user aborts or submits an empty value.
    """
    from qiqiclaw_cli.config import save_env_value
    ...
    print("=" * 70)
    print()
    print("Spotify 要求每个用户注册自己的轻量级应用。")
    print("开发者应用。这大约需要两分钟，只需完成一次。")
    print("每台机器只需完成一次。")
    print()
    print(f"完整指南：{SPOTIFY_DOCS_URL}")
    print()
    print("Steps:")
    print(f"  1. 在浏览器中打开 {SPOTIFY_DASHBOARD_URL}...")
    print("  2. 点击\"创建应用\"并填写：")
    print("       App name:     anything (e.g. qiqiclaw)")
    print("       Description:  anything")
    print(f"       重定向 URI：{redirect_uri_hint}")
    print("       API/SDK：     Web API")
    print("  3. 同意条款，点击保存。")
    print("  4. 打开应用的设置页面并复制客户端 ID。")
    ...
    if not raw:
        print()
        print(f"未输入 Client ID。请参阅 {SPOTIFY_DOCS_URL} 获取完整指南。")
        raise SystemExit("Spotify setup cancelled: empty Client ID.")

    # Persist so subsequent `qiqiclaw auth spotify` runs skip the wizard.
    save_env_value("QIQICLAW_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("QIQICLAW_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("已保存 QIQICLAW_SPOTIFY_CLIENT_ID 到 ~/.qiqiclaw/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "QIQICLAW_SPOTIFY_CLIENT_ID is required".
    ...
    print("请确保此重定向 URI 已在您的 Spotify 应用设置中列入白名单。")
    print()
    print("打开此 URL 以授权 QiQiClaw：")
    print(authorize_url)
    print()
...
# =============================================================================
# OpenAI Codex auth — tokens stored in ~/.qiqiclaw/auth.json (not ~/.codex/)
#
# QiQi maintains its own Codex OAuth session separate from the Codex CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from QiQiClaw auth store (~/.qiqiclaw/auth.json).
    ...
    if not state:
        raise AuthError(
            "No Codex credentials stored. Run `qiqiclaw auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "Codex auth state is missing tokens. Run `qiqiclaw auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "Codex auth is missing access_token. Run `qiqiclaw auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "Codex auth is missing refresh_token. Run `qiqiclaw auth` to re-authenticate.",
            provider="openai-codex",
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None) -> None:
    """Save Codex OAuth tokens to QiQiClaw auth store (~/.qiqiclaw/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh Codex OAuth tokens without mutating QiQiClaw auth state."""
    ...
    if response.status_code != 200:
        ...
        if code == "refresh_token_reused":
            message = (
                ...
                "then run `qiqiclaw auth` to re-authenticate."
            )
            relogin_required = True
...
def _refresh_codex_auth_tokens(
    ...
    Saves the new tokens to QiQiClaw auth store automatically.
    """
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from QiQiClaw's own Codex token store."""
    data = _read_codex_tokens()
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = float(_legacy_env("CODEX_REFRESH_TIMEOUT_SECONDS", "20"))

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other QiQiClaw processes
...
def _refresh_access_token(
    *,
    ...
    # called POST /api/oauth/token with QiQiClaw's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, QiQiClaw's next refresh uses it, and the whole
    ...
    if "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "custom self-heal hook, or another QiQiClaw install sharing "
            "~/.qiqiclaw/auth.json) called POST /api/oauth/token with QiQiClaw's "
            "refresh token without persisting the rotated token back.\n"
            "Nous refresh tokens are single-use — only QiQiClaw may call the "
            "refresh endpoint. For health checks, use `qiqiclaw auth status` "
            "instead.\n"
            "Re-authenticate with: qiqiclaw auth add nous"
        )
    ...
    *,
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip QiQiClaw models — they're not reliable for agentic tool-calling
            if "qiqiclaw" in mid.lower():
                continue
...
def resolve_nous_access_token(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "QiQi is not logged into Nous Portal.",
                provider="nous",
                relogin_required=True,
            )
        ...
        portal_base_url = (
            ...
            or _legacy_env("PORTAL_BASE_URL")
...
def refresh_nous_oauth_from_state(
    ...
    *,
    ...
    return refresh_nous_oauth_pure(
        ...
        state.get("client_id", "qiqiclaw-cli"),
...
def persist_nous_credentials(
    ...
    *,
    ...
    Historically ``qiqiclaw auth add nous`` wrote a ``manual:device_code`` pool
    ...
    ``qiqiclaw auth add nous --label <name>``).  It gets embedded in the
...
def _is_terminal_nous_refresh_error(exc: Exception) -> bool:
    """Return true when a Nous refresh failure should disable cached creds."""
    if not isinstance(exc, AuthError):
        return False
    if exc.relogin_required:
        return True
    return (exc.code or "").strip().lower() in {
        "invalid_grant",
        "invalid_token",
        "reuse_detected",
    }
...
def _quarantine_nous_oauth_state(
    ...
    *,
    ...
    """Mark a singleton Nous OAuth state as unusable without deleting it."""
    state["quarantined"] = True
    state["quarantine_reason"] = reason
    state["quarantine_error"] = format_auth_error(exc)
...
def _quarantine_nous_pool_entries(
    ...
    *,
    ...
    """Mark Nous credential-pool rows as quarantined after terminal refresh errors."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return
    ...
    for entry in entries:
        ...
        entry["quarantine_error"] = message
        entry["quarantined_at"] = now


def _write_shared_nous_state(state: Dict[str, Any]) -> None:
    """Compatibility hook for proxy adapters; auth.json is the shared state."""
    _ = state
...
def resolve_nous_runtime_credentials(
    *,
    ...
    inference_auth_mode = (inference_auth_mode or NOUS_INFERENCE_AUTH_MODE_AUTO).strip().lower()
    if inference_auth_mode not in {
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("QiQi is not logged into Nous Portal.",
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if inference_auth_mode == NOUS_INFERENCE_AUTH_MODE_LEGACY:
                ...
                _oauth_trace("legacy_access_token_reuse", sequence_id=sequence_id)
    ...
    expires_in = (
        ...
        if expires_epoch is not None
            ...
            _coerce_ttl_seconds(state.get("expires_in"))
            if inference_auth_mode == NOUS_INFERENCE_AUTH_MODE_LEGACY
...
def get_codex_auth_status() -> Dict[str, Any]:
    ...
    Checks the credential pool first (where `qiqiclaw auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `qiqiclaw auth` and
    # `qiqiclaw model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        _legacy_env("COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = _legacy_env("COPILOT_ACP_ARGS", "").strip()
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set QIQICLAW_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `qiqiclaw logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from qiqiclaw_cli.models import _format_price_per_mtok
    ...
    try:
        ...
        if _unavailable:
            ...
            print()
            print(f"{_DIM}  ── 在 {_upgrade_url} 升级以使用付费模型 ──{_RESET}")
            print()
    ...
    if _unavailable:
        _upgrade_url = (portal_url or DEFAULT_NOUS_PORTAL_URL).rstrip("/")
        print()
        print(f"  {_DIM}── 不可用的模型（需要付费套餐 — 在 {_upgrade_url} 升级）──{_RESET}")
        for mid in _unavailable:
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from qiqiclaw_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'qiqiclaw model' or 'qiqiclaw setup' instead."""
    print("The 'qiqiclaw login' command has been removed.")
    print("Use 'qiqiclaw auth' to manage credentials,")
    print("'qiqiclaw model' to select a provider, or 'qiqiclaw setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """OpenAI Codex login via device code flow. Tokens stored in ~/.qiqiclaw/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing QiQiClaw-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("在 QiQiClaw 认证存储中找到现有的 Codex 凭据。")
                try:
                ...
                print("现有的 Codex 凭据已过期。正在开始全新登录...")
        except AuthError:
            pass
    ...
    if not force_new_login:
        ...
        if cli_tokens:
            ...
            print("QiQi 将创建自己的会话以避免与 Codex CLI / VS Code 冲突。")
            try:
            ...
            if do_import in ("y", "yes"):
                ...
                base_url = _legacy_env("CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("凭据已导入。注意：如果 Codex CLI 刷新其令牌，")
                print("QiQi 将继续使用自己的会话独立工作。")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — QiQiClaw gets its own OAuth session
    print()
    print("正在登录 OpenAI Codex...")
    print("（QiQiClaw 创建自己的会话 — 不会影响 Codex CLI 或 VS Code）")
    print()

    creds = _codex_device_code_login()

    # Save tokens to QiQiClaw auth store
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to QiQiClaw auth store (~/.qiqiclaw/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"正在通过 MiniMax ({region}) OAuth 启动 QiQiClaw 登录...")
    ...
    with httpx.Client(timeout=httpx.Timeout(timeout_seconds),
        ...
        print(f"  2. 如果提示，请输入代码：{user_code}")
        if open_browser:
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `qiqiclaw model` and select "
            "MiniMax (OAuth).",
...
def _nous_device_code_login(
    *,
    ...
    print(f"正在通过 {pconfig.name} 启动 QiQiClaw 登录...")
    print(f"门户：{portal_base_url}")
    if insecure:
        print("TLS 验证：已禁用（--insecure）")
    elif ca_bundle:
        print(f"TLS 验证：自定义 CA 包（{ca_bundle}）")
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        print(f"等待批准（每 {effective_interval} 秒轮询一次）...")
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("您的 Nous Portal 账户没有有效的订阅。")
            print(f"  Subscribe here: {portal_url}/billing")
            print()
            print("After subscribing, run `qiqiclaw model` again to finish setup.")
            raise SystemExit(1)
        raise
...
def _login_nous(args, pconfig: ProviderConfig) -> None:
    ...
    try:
        ...
        try:
            ...
            if model_ids:
                print(f"显示 {len(model_ids)} 个精选模型 — 使用\"输入自定义模型名称\"查看其他模型。")
        ...
        if not selected_model:
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("没有提供商变更。Nous 凭据已保存供将来使用。")
            print("  再次运行 `qiqiclaw model` 以切换到 Nous Portal。")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or config_matches:
        ...
        if provider_name == "OpenAI Codex":
            print("Logged out of OpenAI Codex.")
        else:
            print(f"已退出 {provider_name}。")
        if os.getenv("OPENROUTER_API_KEY"):
            print("QiQi 将使用 OpenRouter 进行推理。")
        else:
            print("运行 `qiqiclaw model` 或配置 API 密钥以使用 QiQiClaw。")
    else:
