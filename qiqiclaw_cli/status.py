"""
Status command for QiQiClaw CLI.

Shows the status of all QiQiClaw components.
"""
...
from qiqiclaw_constants import get_project_root

PROJECT_ROOT = get_project_root()

from qiqiclaw_cli.auth import AuthError, resolve_provider
from qiqiclaw_cli.colors import Colors, color
from qiqiclaw_cli.config import get_env_path, get_env_value, get_qiqiclaw_home, load_config
from qiqiclaw_cli.models import provider_label
from qiqiclaw_cli.nous_subscription import get_nous_subscription_features
from qiqiclaw_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``qiqiclaw config``'s
...
def show_status(args):
    """Show status of all QiQiClaw components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 Q QiQiClaw Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    nous_label = "logged in" if nous_logged_in else "not logged in (run: qiqiclaw auth add nous --type oauth)"
    print(
    ...
    print(
        ...
        f"{'logged in' if codex_logged_in else 'not logged in (run: qiqiclaw model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: qiqiclaw auth add minimax-oauth)'}"
    )
    ...
    if managed_nous_tools_enabled():
        ...
        for feature in features.items():
            if feature.managed_by_nous:
                ...
                state = "included in subscription, not selected"
        ...
        print()
        print(color("◆ Nous Tool Gateway", Colors.CYAN, Colors.BOLD))
        print("  Your free Nous account does not include Tool Gateway access.")
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: qiqiclaw model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from qiqiclaw_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'qiqiclaw[vercel]')"
        ...
        print("  Processes:    live processes do not survive cleanup, snapshot, or sandbox rebuild")
    ...
    try:
        from qiqiclaw_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            print("  Service:      installed but not managing the running gateway")
        elif _is_termux() and not snapshot.gateway_pids:
            print("  Start with:   qiqiclaw gateway")
            print("  Note:         Android may stop background tasks when Termux is suspended")
        ...
        if _is_termux():
            ...
            print("  Manager:      (unsupported on this platform)")
    ...
    jobs_file = get_qiqiclaw_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
            with open(jobs_file, encoding="utf-8") as f:
                ...
                print(f"  任务:         {len(enabled_jobs)} 个活动, 共 {len(jobs)} 个")
        except Exception:
            print("  任务:         (读取任务文件出错)")
    else:
        print("  任务:         0")
    ...
    sessions_file = get_qiqiclaw_home() / "sessions" / "sessions.json"
    if sessions_file.exists():
        import json
        try:
            with open(sessions_file, encoding="utf-8") as f:
                data = json.load(f)
                print(f"  活动:         {len(data)} 个会话")
        except Exception:
            print("  活动:         (读取会话文件出错)")
    else:
        print("  活动:         0")
    ...
    print(color("  运行 'qiqiclaw doctor' 进行详细诊断", Colors.DIM))
    print(color("  运行 'qiqiclaw setup' 进行配置", Colors.DIM))
    print()
