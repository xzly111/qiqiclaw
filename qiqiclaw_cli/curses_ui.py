"""Shared curses-based UI components for QiQiClaw CLI.

Used by `qiqiclaw tools` and `qiqiclaw skills` for interactive checklists.
...
def _reset_terminal() -> None:
    """Restore terminal to a sane state after curses exits.

    On WSL (and some other platforms), ``curses.endwin()`` does NOT
    reliably restore terminal settings such as echo, line buffering,
    and cooked mode — leaving the terminal seemingly hung or unresponsive
    after a curses TUI exits.

    This function runs ``stty sane`` to force the terminal back to a
    usable cooked-mode state.  It's a no-op if ``stty`` is not available
...
def _numbered_fallback(
    ...
    while True:
        ...
        print()
        try:
            val = input(color("  切换编号 (或回车确认): ", Colors.DIM)).strip()
            if not val:
                break
            idx = int(val) - 1
            if 0 <= idx < len(items):
