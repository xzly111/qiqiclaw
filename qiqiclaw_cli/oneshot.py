...
configured for "cli" in `qiqiclaw tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (QIQICLAW_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `qiqiclaw chat`:
...
Env var fallbacks (used when the corresponding arg is not passed):
    - QIQICLAW_INFERENCE_MODEL
    - QIQICLAW_INFERENCE_PROVIDER  (already read by resolve_runtime_provider)
"""
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"qiqiclaw -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "qiqiclaw -z: --toolsets all enables every toolset; "
    ...
    if unresolved:
        try:
            ...
            from qiqiclaw_cli.tools_config import _parse_enabled_flag
    ...
    if unknown:
        sys.stderr.write(f"qiqiclaw -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "qiqiclaw -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "qiqiclaw -z: --toolsets did not contain any valid toolsets.\n"
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to QIQICLAW_INFERENCE_MODEL
            env var, then config.yaml's model.default / model.model.
        provider: Optional provider override. Falls back to
            QIQICLAW_INFERENCE_PROVIDER env var, then config.yaml's model.provider,
            then "auto".
    ...
    env_model_early = _legacy_env("INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "qiqiclaw -z: --provider requires --model (or QIQICLAW_INFERENCE_MODEL). "
...
def _run_agent(
    ...
    # Imports are local so they don't run when qiqiclaw is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from qiqiclaw_cli.config import load_config
    from qiqiclaw_cli.models import detect_provider_for_model
    from qiqiclaw_cli.runtime_provider import resolve_runtime_provider
    from qiqiclaw_cli.tools_config import _get_platform_tools
    ...
    env_model = _legacy_env("INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from qiqiclaw_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or _legacy_env("INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                QIQICLAW_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via QIQICLAW_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via QIQICLAW_YOLO_MODE=1
