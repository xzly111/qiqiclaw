...
The board lives at ``$QIQICLAW_HOME/kanban.db`` (profile-agnostic on purpose:
...
coding workloads.  See ``docs/qiqiclaw-kanban-v1-spec.pdf`` for the full
...
def _normalize_board_slug(slug: Optional[str]) -> Optional[str]:
    """Lowercase and validate a board slug; return None for empty input."""
    if slug is None:
        return None
...
def kanban_home() -> Path:
    """Return the QiQiClaw home directory that anchors kanban state."""
    override = os.environ.get("QIQICLAW_KANBAN_HOME", "").strip()
    if override:
...
def get_current_board() -> str:
    env = os.environ.get("QIQICLAW_KANBAN_BOARD", "").strip()
    if env:
        try:
    ...
    try:
        ...
        if path.exists():
            normed = _normalize_board_slug(path.read_text(encoding="utf-8"))
            if normed and board_exists(normed):
                return normed
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    """Return the path to the active board's ``kanban.db``."""
    override = os.environ.get("QIQICLAW_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    override = os.environ.get("QIQICLAW_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def board_metadata_path(board: Optional[str] = None) -> Path:
    return board_dir(board) / "board.json"
...
def read_board_metadata(board: Optional[str] = None) -> dict:
    ...
    meta = {
        "slug": slug,
        "name": "QiQiClaw" if slug == DEFAULT_BOARD else _default_board_display_name(slug),
        "description": "",
        "icon": "",
        "color": "",
...
def write_board_metadata(
    ...
    *,
    ...
    if default_workdir is not _UNSET:
...
def list_boards(include_archived: bool = False) -> list[dict]:
    boards: list[dict] = [read_board_metadata(DEFAULT_BOARD)]
    root = boards_root()
    if root.exists():
        for child in sorted(root.iterdir(), key=lambda p: p.name):
            if not child.is_dir() or child.name in {DEFAULT_BOARD, "_archived"}:
                continue
            try:
        ...
        if include_archived:
            ...
            if archived_root.exists():
                for child in sorted(archived_root.iterdir(), key=lambda p: p.name):
                    if not child.is_dir():
                        continue
                    ...
                    try:
                        ...
                        raw_meta = {}
                    if not slug:
                        slug_part = child.name.rsplit("-", 1)[0]
                        try:
                    ...
                    if raw_meta:
                        raw_meta["slug"] = slug
                        raw_meta["archived"] = True
                        raw_meta["db_path"] = str(child / "kanban.db")
...
def remove_board(slug: str, *, archive: bool = True) -> dict:
    ...
    src = board_dir(normed)
    if archive:
        meta = write_board_metadata(normed, archived=True)
        archived_root = boards_root() / "_archived"
        archived_root.mkdir(parents=True, exist_ok=True)
        dest = archived_root / f"{normed}-{int(time.time())}"
        if src.exists():
            shutil.move(str(src), str(dest))
        if get_current_board() == normed:
            clear_current_board()
        return {"slug": normed, "action": "archived", "new_path": str(dest), "metadata": meta}
    if src.exists():
        shutil.rmtree(src)
    if get_current_board() == normed:
        clear_current_board()
...
def connect(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> sqlite3.Connection:
    ...
    """
    path = db_path or kanban_db_path(board=board)
    path.parent.mkdir(parents=True, exist_ok=True)
...
def init_db(db_path: Optional[Path] = None, *, board: Optional[str] = None) -> Path:
    ...
    Kept as a public entry point so CLI ``qiqiclaw kanban init`` and the
...
def create_task(
    ...
    *,
    ...
    each name to ``qiqiclaw --skills ...`` alongside the built-in
    ...
    # `qiqiclaw --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
    ...
    if workspace_path is None and workspace_kind in {"dir", "worktree"}:
        board_meta = read_board_metadata(board or get_current_board())
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``qiqiclaw kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``qiqiclaw kanban complete <ready-task>
...
def complete_task(
    ...
    *,
    ...
    completion (``qiqiclaw kanban complete <id>``) works without requiring
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``scratch``: a fresh dir under the active board's workspaces root.
    - ``dir:<path>``: the path stored in ``workspace_path``.  Created
      ...
      root.  Users who want a QIQICLAW_HOME-relative workspace should
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``qiqiclaw kanban
...
def _default_spawn(task: Task, workspace: str) -> Optional[int]:
    """Fire-and-forget ``qiqiclaw -p <profile> chat -q ...`` subprocess.
    ...
    env = dict(os.environ)
    if task.tenant:
        env["QIQICLAW_TENANT"] = task.tenant
    env["QIQICLAW_KANBAN_BOARD"] = board
    env["QIQICLAW_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["QIQICLAW_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    env["QIQICLAW_KANBAN_TASK"] = task.id
    env["QIQICLAW_KANBAN_WORKSPACE"] = workspace
    # QIQICLAW_PROFILE is the author the kanban_comment tool defaults to.
    # `qiqiclaw -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["QIQICLAW_PROFILE"] = task.assignee
    ...
    # Redirect output to a per-task log under the active board's log dir.
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`qiqiclaw` executable not found on PATH. "
            "Install QiQiClaw or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``qiqiclaw kanban daemon`` is systemd-friendly.
...
def gc_worker_logs(
    *, older_than_seconds: int = 30 * 24 * 3600,
    ...
    from qiqiclaw_constants import get_qiqiclaw_home
    log_dir = get_qiqiclaw_home() / "kanban" / "logs"
...
def list_profiles_on_disk() -> list[str]:
    ...
    Reads ``~/.qiqiclaw/profiles/`` directly so this module has no import
    dependency on ``qiqiclaw_cli.profiles`` (which pulls in a large chunk
    ...
    """
    try:
        home = Path.home() / ".qiqiclaw" / "profiles"
    except Exception:
        return []
    if not home.is_dir():
        return []
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``qiqiclaw kanban assignees`` for the terminal.
