...
Add, remove, or reorder entries here — both `qiqiclaw setup` and
`qiqiclaw` provider-selection will pick up the change automatically.
"""
...
_HERMES_USER_AGENT = f"qiqiclaw-cli/{_HERMES_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `qiqiclaw model`
    flow without maintaining a separate static list.
    """
    from qiqiclaw_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $QIQICLAW_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $QIQICLAW_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. QiQiClaw refreshes the cache from
...
def check_nous_free_tier() -> bool:
    ...
    try:
        from qiqiclaw_cli.auth import get_provider_auth_state, resolve_nous_runtime_credentials
...
# this list:  qiqiclaw model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `qiqiclaw model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `qiqiclaw model` TUI
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model a user would be offered first in the ``qiqiclaw model`` picker.

    Used as a fallback when the user has configured a provider but never
    selected a model (e.g. ``qiqiclaw auth add openai-codex`` without
    ``qiqiclaw model``).
    """
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    qiqiclaw is tool-calling-first — every provider path assumes the model
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from qiqiclaw_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — qiqiclaw
...
def get_curated_nous_model_ids() -> list[str]:
    ...
    """
    try:
        from qiqiclaw_cli.model_catalog import get_curated_nous_models
...
def fetch_ai_gateway_models(
    ...
    *,
    ...
    from qiqiclaw_constants import AI_GATEWAY_BASE_URL
...
def format_model_pricing_table(
    ...
    *models* is ``[(model_id, description), ...]``.
    ...
    # Header
    if has_cache:
        lines.append(f"{indent}{'模型':<{name_col}} {'输入':>{price_col}}  {'输出':>{price_col}}  {'缓存':>{cache_col}}  /百万token")
        lines.append(f"{indent}{'-' * name_col} {'-' * price_col}  {'-' * price_col}  {'-' * cache_col}")
    else:
        lines.append(f"{indent}{'模型':<{name_col}} {'输入':>{price_col}}  {'输出':>{price_col}}  /百万token")
...
def fetch_ai_gateway_pricing(
    ...
    *,
    ...
    """Fetch Vercel AI Gateway /v1/models and return qiqiclaw-shaped pricing.

    Vercel uses ``input`` / ``output`` field names; qiqiclaw's picker expects
...
def _resolve_nous_pricing_credentials() -> tuple[str, str]:
    ...
    try:
        from qiqiclaw_cli.auth import resolve_nous_runtime_credentials
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``qiqiclaw model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from qiqiclaw_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        nous:qiqiclaw-3                           →  ("nous", "qiqiclaw-3")
...
def _resolve_static_model_alias(
    ...
    try:
        from qiqiclaw_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to QiQiClaw' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``qiqiclaw_cli.auth.resolve_provider()`` to resolve it to a concrete
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether QiQiClaw should expose the /fast toggle for this model."""
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``qiqiclaw auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.qiqiclaw/.env``.
    ...
    """
    try:
        from qiqiclaw_cli.auth import resolve_api_key_provider_credentials
    ...
    try:
        ...
        from qiqiclaw_cli.copilot_auth import (
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    on the platform appear in ``/model`` without a QiQiClaw release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a QiQiClaw release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "nous":
        ...
        try:
            from qiqiclaw_cli.auth import fetch_nous_models, resolve_nous_runtime_credentials
            creds = resolve_nous_runtime_credentials()
            if creds:
...
def copilot_default_headers() -> dict[str, str]:
    ...
    """
    try:
        from qiqiclaw_cli.copilot_auth import copilot_request_headers
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: QiQiClaw' default Claude IDs elsewhere use
...
def _ollama_cloud_cache_path() -> Path:
    ...
    return get_qiqiclaw_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "lmstudio":
        ...
        try:
            ...
            return {
                ...
                "message": (
                    f"{exc} 请设置 `LM_API_KEY`（或更新它）以匹配服务器的 bearer token。"
                ),
            }
        if models is None:
            return {
                "accepted": False, "persist": False, "recognized": False,
                "message": f"Could not reach LM Studio `/api/v1/models` to validate `{requested}`.",
            }
        if not models:
            return {
                "accepted": False, "persist": False, "recognized": False,
                "message": (
                    f"LM Studio 可访问但未加载支持聊天的模型。"
                    f"请在 LM Studio 中加载 `{requested}`（开发者选项卡 → 加载模型）然后重试。"
                ),
            }
        ...
        return {
            "accepted": False, "persist": False, "recognized": False,
            "message": f"Model `{requested}` not found in LM Studio's model listing.",
        }
    ...
    if normalized == "custom":
        ...
        if api_models is not None:
            ...
            suggestion_text = ""
            if suggestions:
                suggestion_text = "\n  相似模型：" + ", ".join(f"`{s}`" for s in suggestions)

            message = (
                f"Note: `{requested}` was not found in this custom endpoint's model list "
                f"({probe.get('probed_url')})。如果服务器支持隐藏或别名模型，它可能仍然有效。"
                f"{suggestion_text}"
            )
            if probe.get("used_fallback"):
                message += (
                    f"\n  尝试 `{probe.get('resolved_base_url')}` 后端点验证成功。"
                    f"建议将其保存为您的基础 URL。"
                )
        ...
        message = (
            f"Note: could not reach this custom endpoint's model list `{probe.get('probed_url')}`."
            f"QiQi 仍将保存 `{requested}`，但端点应公开 `/models` 以供验证。"
        )
        if api_mode == "anthropic_messages":
            message += (
                "\n  许多 Anthropic 兼容代理未实现模型 API "
                "(GET /v1/models)。模型名称已接受但未验证。"
            )
        if probe.get("suggested_base_url"):
            message += f"\n  如果此服务器需要 `/v1`，请尝试基础 URL：`{probe.get('suggested_base_url')}`"
    ...
    if normalized == "openai-codex":
        ...
        if codex_models:
            ...
            return {
                ...
                "recognized": False,
                "message": (
                    f"Model `{requested}` not found in the OpenAI Codex model list."
                    f"{suggestion_text}"
                ),
            }
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint."
                    "\n  MiniMax 未公开 /models 端点，因此 QiQiClaw 无法验证模型名称。"
                    "\n  如果模型存在于服务器上，它可能仍然有效。"
                ),
            }
    ...
    if normalized == "anthropic":
        ...
        if anthropic_models is not None:
            ...
            return {
                ...
                "recognized": False,
                "message": (
                    f"Note: `{requested}` was not found in Anthropic's /v1/models list."
                    f"如果您有早期访问权限或快照 ID，它可能仍然有效。"
                    f"{suggestion_text}"
                ),
            }
    ...
    if api_mode == "anthropic_messages":
        ...
        return {
            ...
            "recognized": False,
            "message": (
                f"Note: could not validate `{requested}` against this endpoint's model list."
                f"许多 Anthropic 兼容代理未实现 GET /v1/models。"
                f"模型名称已接受但未验证。"
            ),
        }
    ...
    if api_models is not None:
        ...
        return {
            ...
            "recognized": False,
            "message": (
                f"Model `{requested}` not found in this provider's model list."
                f"{suggestion_text}"
            ),
        }
    ...
    if catalog_models:
        ...
        return {
            ...
            "recognized": False,
            "message": (
                f"Note: `{requested}` was not found in the curated catalog "
                f"for {provider_label}, "
                f"且 /models 端点无法访问。{suggestion_text}"
                f"\n  如果模型存在于提供商上，它可能仍然有效。"
            ),
        }
