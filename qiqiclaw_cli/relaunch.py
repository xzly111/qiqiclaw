"""
Unified self-relaunch for QiQiClaw CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``qiqiclaw sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``qiqiclaw`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from qiqiclaw_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched qiqiclaw."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_hermes_bin() -> Optional[str]:
    """Find the qiqiclaw entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("qiqiclaw")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m qiqiclaw_cli.main``.
    """
    argv0 = sys.argv[0]
    ...
    # PATH lookup
    path_bin = shutil.which("qiqiclaw")
    if path_bin:
        return path_bin
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with qiqiclaw.
    ...
    if bin_path:
        ...
        argv = [sys.executable, "-m", "qiqiclaw_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh qiqiclaw invocation."""
