"""
Gateway subcommand for QiQiClaw CLI.

Handles: qiqiclaw gateway [run|start|stop|restart|status|install|uninstall|setup]
"""
...
from qiqiclaw_constants import get_project_root, ensure_project_root_on_syspath
from qiqiclaw_cli._subprocess_compat import (
...
# display_qiqiclaw_home is imported lazily at call sites to avoid ImportError
# when hermes_constants is cached from a pre-update version during `qiqiclaw update`.
        ...
        for scope_args in [["systemctl", "--user"], ["systemctl"]]:
            try:
                result = subprocess.run(
                    scope_args + ["list-units", "qiqiclaw-gateway*",
...
def _command_looks_like_gateway_run(command: str) -> bool:
    """Return true only for a real gateway runtime process.

    Process-table scans are intentionally conservative.  Broad substring
    matches like ``"qiqiclaw gateway"`` also match the shell command currently
    running ``qiqiclaw gateway status`` or ``gateway stop``; that made status
    report a non-existent gateway and caused desktop clients to wait on an API
    port that was not listening.  Only the actual python/qiqiclaw process whose
    argv reaches ``gateway run`` is considered a gateway here.
    """
    try:
    ...
    if not (
        ...
        or exe in {"qiqiclaw", "qiqiclaw.exe", "qiqiclaw-gateway"}
    ):
        return False

    if "gateway/run.py" in command:
        return True
    if exe == "qiqiclaw-gateway":
        return True
    ...
    for idx, token in enumerate(tokens):
        ...
        tail = tokens[idx + 1 :]
        for part in tail:
            if part in {"run", "start"}:
                return True
            if part in {"status", "stop", "restart", "install", "uninstall", "setup", "migrate-legacy"}:
                return False
        return False
    return False
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    """
    pids: list[int] = []
    current_home = str(get_qiqiclaw_home().resolve())
    ...
    def _matches_current_profile(command: str) -> bool:
        if current_profile_name:
            return (
                ...
                or f"QIQICLAW_HOME={current_home}" in command
            )

        if "--profile " in command or " -p " in command:
            return False
        if "QIQICLAW_HOME=" in command and f"QIQICLAW_HOME={current_home}" not in command:
            return False
        return True
        ...
        if is_windows():
            ...
            for line in result.stdout.split("\n"):
                ...
                if line.startswith("CommandLine="):
                    ...
                    if _command_looks_like_gateway_run(current_cmd) and (
            ...
            for line in result.stdout.split("\n"):
                ...
                if pid is None:
                    continue
                if _command_looks_like_gateway_run(command) and (
...
def find_gateway_pids(exclude_pids: set | None = None, all_profiles: bool = False) -> list:
    ...
    Args:
        ...
        all_profiles: When ``True``, return gateway PIDs across **all**
            profiles (the pre-7923 global behaviour).  ``qiqiclaw update``
            needs this because a code update affects every profile.
            When ``False`` (default), only PIDs belonging to the current
            QiQiClaw profile are returned.
    """
...
def find_profile_gateway_processes(
    ...
    """Return running gateway PIDs mapped to QiQiClaw profiles via PID files."""
    _exclude = set(exclude_pids or set())
...
def _gateway_run_args_for_profile(profile: str) -> list[str]:
    args = [get_python_path(), "-m", "qiqiclaw_cli.main"]
    if profile != "default":
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    watcher = textwrap.dedent(
        ...
        def _detach_kwargs(with_breakaway=True):
            ...
            flags = 0x00000200 | 0x00000008 | 0x08000000
            if with_breakaway:
                flags |= 0x01000000
        ...
        try:
            ...
            subprocess.Popen(
                ...
                stdout=subprocess.DEVNULL,
                stderr=subprocess.DEVNULL,
                **_detach_kwargs(with_breakaway=False),
            )
        """
    ).strip()
...
def _start_detached_gateway_run() -> int | None:
    """Start ``gateway run`` detached when no service manager unit exists."""
    home = get_qiqiclaw_home()
    log_dir = home / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    log_file = log_dir / "gateway.log"
    args = [sys.executable, "-m", "qiqiclaw_cli.main", "gateway", "--accept-hooks", "run"]
    env = os.environ.copy()
...
def _wait_for_manual_gateway_start(pid: int, timeout: float = 15.0) -> bool:
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        f"⚠ {scope_label} 服务在 {int(timeout)} 秒内未激活。\n"
        f"  检查状态: {'sudo ' if system else ''}qiqiclaw gateway status\n"
        f"  检查日志:   journalctl {'--user ' if not system else ''}-u {svc} -l --since '2 min ago'"
    )
    return False
...
def _recover_pending_systemd_restart(system: bool = False, previous_pid: int | None = None) -> bool:
    ...
    if active_state == "failed" and (
        ...
        svc = get_service_name()
        scope_label = _service_scope_label(system).capitalize()
        print(f"↻ 清除挂起的 {scope_label.lower()} 服务重启的失败状态...")
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def get_gateway_runtime_snapshot(system: bool = False) -> GatewayRuntimeSnapshot:
    ...
    from qiqiclaw_constants import is_container
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  这通常是手动前台/tmux/nohup运行，因此 `qiqiclaw gateway`")
    print("  会拒绝启动另一个副本，直到此进程停止。")
...
def stop_profile_gateway() -> bool:
    """Stop only the gateway for the current profile (QIQICLAW_HOME-scoped).
...
from qiqiclaw_constants import is_container, is_termux, is_wsl
...
_SERVICE_BASE = "qiqiclaw-gateway"
SERVICE_DESCRIPTION = "QiQiClaw Gateway - Messaging Platform Integration"


def _profile_suffix() -> str:
    """Derive a service-name suffix from the current QIQICLAW_HOME.

    Returns ``""`` for the default root, the profile name for
    ``<root>/profiles/<name>``, or a short hash for any other path.
    Works correctly in Docker (QIQICLAW_HOME=/opt/data) and standard deployments.
    """
    import hashlib
    import re
    from qiqiclaw_constants import get_default_qiqiclaw_root
    home = get_qiqiclaw_home().resolve()
    default = get_default_qiqiclaw_root().resolve()
    if home == default:
        return ""
    ...
    try:
        ...
        pass
    # Fallback: short hash for arbitrary QIQICLAW_HOME paths
    return hashlib.sha256(str(home).encode()).hexdigest()[:8]


def _profile_arg(qiqiclaw_home: str | None = None) -> str:
    """Return ``--profile <name>`` only when QIQICLAW_HOME is a named profile.

    For ``~/.qiqiclaw/profiles/<name>``, returns ``"--profile <name>"``.
    For the default profile or hash-based custom paths, returns the empty string.

    Args:
        qiqiclaw_home: Optional explicit QIQICLAW_HOME path. Defaults to the current
            ``get_qiqiclaw_home()`` value. Should be passed when generating a
            service definition for a different user (e.g. system service).
    """
    import re
    from qiqiclaw_constants import get_default_qiqiclaw_root
    home = Path(qiqiclaw_home or str(get_qiqiclaw_home())).resolve()
    default = get_default_qiqiclaw_root().resolve()
    if home == default:
        return ""
    profiles_root = (default / "profiles").resolve()
...
def get_service_name() -> str:
    """Derive a systemd service name scoped to this QIQICLAW_HOME.

    Default ``~/.qiqiclaw`` returns ``qiqiclaw-gateway`` (backward compatible).
    Profile ``~/.qiqiclaw/profiles/coder`` returns ``qiqiclaw-gateway-coder``.
    Any other QIQICLAW_HOME appends a short hash for uniqueness.
    """
        ...
        try:
            ...
            if result.returncode == 0:
                if _wait_for_user_dbus_socket(timeout=5.0):
                    print(f"✓ Enabled linger for {username} — user D-Bus is now available")
                    return
...
# Legacy service names from older QiQiClaw installs that predate the
# qiqiclaw-gateway rename. Kept as an explicit allowlist (NOT a glob) so
# profile units (qiqiclaw-gateway-*.service) and unrelated third-party
# "qiqiclaw" units are never matched.
_LEGACY_SERVICE_NAMES: tuple[str, ...] = ("qiqiclaw.service",)

# ExecStart content markers that identify a unit as running our gateway.
# A legacy unit is only flagged when its file contains one of these.
_LEGACY_UNIT_EXECSTART_MARKERS: tuple[str, ...] = (
    "qiqiclaw_cli.main gateway",
    "qiqiclaw_cli/main.py gateway",
    "gateway/run.py",
    " qiqiclaw gateway ",
    "/qiqiclaw gateway ",
)
...
def _find_legacy_hermes_units() -> list[tuple[str, Path, bool]]:
    """Return ``[(unit_name, unit_path, is_system)]`` for legacy QiQiClaw gateway units.

    Detects unit files installed by older QiQiClaw versions that used a
    different service name (e.g. ``qiqiclaw.service`` before the rename to
    ``qiqiclaw-gateway.service``). When both a legacy unit and the current
    ``qiqiclaw-gateway.service`` are active, they fight over the same bot
    ...
    * Explicit allowlist of legacy names (no globbing). Profile units such
      as ``qiqiclaw-gateway-coder.service`` and unrelated third-party
      ``qiqiclaw-*`` services are never matched.
    * ExecStart content check — only flag units that invoke our gateway
      entrypoint. A user-created ``qiqiclaw.service`` running an unrelated
...
def has_legacy_qiqiclaw_units() -> bool:
    """Return True when any legacy QiQiClaw gateway unit files exist."""
    return bool(_find_legacy_hermes_units())


def print_legacy_unit_warning() -> None:
    """Warn about legacy QiQiClaw gateway unit files if any are installed.
    ...
    if not legacy:
        return
    print_warning("Legacy QiQiClaw gateway unit(s) detected from an older install:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
        print_info(f"    {path}  ({scope} scope)")
    print_info("  These run alongside the current qiqiclaw-gateway service and")
    print_info("  cause SIGTERM flap loops — both try to use the same bot token.")
    print_info("  Remove them with:")
    print_info("    qiqiclaw gateway migrate-legacy")
...
def remove_legacy_hermes_units(
    ...
    legacy = _find_legacy_hermes_units()
    if not legacy:
        print("No legacy QiQiClaw gateway units found.")
        return 0, []
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("已跳过。再次运行: qiqiclaw gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
    ...
    if system_units:
        if os.geteuid() != 0:
            ...
            print_info("  Re-run with: sudo qiqiclaw gateway migrate-legacy")
...
def print_systemd_scope_conflict_warning() -> None:
    ...
    print_info("    qiqiclaw gateway uninstall")
    print_info("    sudo qiqiclaw gateway uninstall --system")


def _require_root_for_system_service(action: str) -> None:
    if os.geteuid() != 0:
        print(f"系统网关 {action} 需要 root 权限。请使用 sudo 重新运行。")
        sys.exit(1)
...
def install_linux_gateway_from_setup(force: bool = False) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:
            print_warning("  System service install requires sudo, so QiQiClaw can't create it from this user session.")
            if run_as_user:
                print_info(f"  After setup, run: sudo qiqiclaw gateway install --system --run-as-user {run_as_user}")
            else:
                print_info("  After setup, run: sudo qiqiclaw gateway install --system --run-as-user <your-user>")
            print_info("  Then start it with: sudo qiqiclaw gateway start --system")
            return scope, False
...
def print_systemd_linger_guidance() -> None:
    ...
    linger_enabled, linger_detail = get_systemd_linger_status()
    if linger_enabled is True:
        print("✓ Systemd linger is enabled (服务在注销后继续运行)")
    elif linger_enabled is False:
        print("⚠ Systemd linger is disabled (注销时网关可能会停止)")
        print("  运行: sudo loginctl enable-linger $USER")
    else:
        print(f"⚠ 无法验证 systemd linger ({linger_detail})")
        print("  如果您希望网关用户服务在注销后继续运行，请运行:")
        print("  sudo loginctl enable-linger $USER")

def _launchd_user_home() -> Path:
    """Return the real macOS user home for launchd artifacts.

    Profile-mode QiQiClaw often sets ``HOME`` to a profile-scoped directory, but
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.qiqiclaw`` → ``ai.qiqiclaw.gateway.plist`` (backward compatible).
    Profile ``~/.qiqiclaw/profiles/coder`` → ``ai.qiqiclaw.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
    name = f"ai.qiqiclaw.gateway-{suffix}" if suffix else "ai.qiqiclaw.gateway"
...
def _remap_path_for_user(path: str, target_home_dir: str) -> str:
    ...
    to *target_home_dir*; otherwise the path is returned unchanged.
      ...
      /root/.qiqiclaw/qiqiclaw  -> /home/alice/.qiqiclaw/qiqiclaw
      /opt/qiqiclaw                 -> /opt/qiqiclaw  (kept as-is)
...
def _qiqiclaw_home_for_target_user(target_home_dir: str) -> str:
    """Remap the current QIQICLAW_HOME to the equivalent under a target user's home.

    When installing a system service via sudo, get_qiqiclaw_home() resolves to
    root's home.  This translates it to the target user's equivalent path:
      /root/.qiqiclaw                    -> /home/alice/.qiqiclaw
      /root/.qiqiclaw/profiles/coder     -> /home/alice/.qiqiclaw/profiles/coder
      /opt/custom-qiqiclaw               → /opt/custom-qiqiclaw  (kept as-is)
    """
    current_hermes = get_qiqiclaw_home().resolve()
    current_default = (Path.home() / ".qiqiclaw").resolve()
    target_default = Path(target_home_dir) / ".qiqiclaw"

    # Default ~/.qiqiclaw -> remap to target user's default
    if current_hermes == current_default:
        return str(target_default)

    # Profile or subdir of ~/.qiqiclaw -> preserve the relative structure
    try:
        relative = current_hermes.relative_to(current_default)
        return str(target_default / relative)
    except ValueError:
        # Completely custom path (not under ~/.qiqiclaw) -> keep as-is
...
def generate_systemd_unit(system: bool = False, run_as_user: str | None = None) -> str:
    ...
    if system:
        ...
        qiqiclaw_home = _qiqiclaw_home_for_target_user(home_dir)
        profile_arg = _profile_arg(qiqiclaw_home)
...
User={username}
Group={group_name}
ExecStart={python_path} -m qiqiclaw_cli.main{f" {profile_arg}" if profile_arg else ""} gateway run --replace
...
Environment="QIQICLAW_HOME={qiqiclaw_home}"
Restart=on-failure
RestartSec=30
...
"""
    ...
    qiqiclaw_home = str(get_qiqiclaw_home().resolve())
    profile_arg = _profile_arg(qiqiclaw_home)
...
def refresh_systemd_unit_if_needed(system: bool = False) -> bool:
    ...
    print(f"↻ Updated gateway {_service_scope_label(system)} service definition to match the current QiQiClaw install")
    return True
...
def _print_linger_enable_warning(username: str, detail: str | None = None) -> None:
    print()
    print("⚠ 未启用 Linger — 关闭终端时网关可能会停止。")
    if detail:
        print(f"  自动启用失败: {detail}")
    print()
    print("  在无头服务器（VPS、云实例）上运行:")
...
def _ensure_linger_enabled() -> None:
    ...
    if result.returncode == 0:
        print("✓ Linger enabled — 网关将在注销后继续运行")
        return
...
def _get_restart_drain_timeout() -> float:
    ...
    raw = _legacy_env("RESTART_DRAIN_TIMEOUT", "").strip()
    if not raw:
        cfg = read_raw_config()
...
def systemd_install(force: bool = False, system: bool = False, run_as_user: str | None = None):
    ...
    # Offer to remove legacy units (qiqiclaw.service from pre-rename installs)
    # before installing the new qiqiclaw-gateway.service. If both remain, they
    ...
    print()
    print("Next steps:")
    print(f"  {'sudo ' if system else ''}qiqiclaw gateway start{scope_flag}              # Start the service")
    print(f"  {'sudo ' if system else ''}qiqiclaw gateway status{scope_flag}             # Check status")
...
def systemd_restart(system: bool = False):
    ...
    if pid is not None and _request_gateway_self_restart(pid):
        ...
        print(f"⏳ {scope_label} 服务正在清空活动工作...")
        ...
        # unit immediately so `qiqiclaw gateway restart` behaves idempotently.
        _run_systemctl(
            ["reset-failed", svc],
            system=system,
            check=False,
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not unit_path.exists():
        ...
        print(f"  Run: {'sudo ' if system else ''}qiqiclaw gateway install{scope_flag}")
        return
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(f"  Run: {'sudo ' if system else ''}qiqiclaw gateway restart{scope_flag}  # auto-refreshes the unit")
        print()
    ...
    if status == "active":
        ...
        print(f"  Run: {'sudo ' if system else ''}qiqiclaw gateway start{scope_flag}")
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print("  ⚠ Planned restart is stuck in systemd failed state (exit code 75)")
        print(f"  运行: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}qiqiclaw gateway start{scope_flag}")
...
def get_launchd_label() -> str:
    ...
    suffix = _profile_suffix()
    return f"ai.qiqiclaw.gateway-{suffix}" if suffix else "ai.qiqiclaw.gateway"
...
def generate_launchd_plist() -> str:
    ...
    working_dir = str(PROJECT_ROOT)
    qiqiclaw_home = str(get_qiqiclaw_home().resolve())
    log_dir = get_qiqiclaw_home() / "logs"
    log_dir.mkdir(parents=True, exist_ok=True)
    label = get_launchd_label()
    profile_arg = _profile_arg(qiqiclaw_home)
    ...
    prog_args = [
        ...
        "<string>-m</string>",
        "<string>qiqiclaw_cli.main</string>",
    ]
    if profile_arg:
...
<dict>
    ...
    <dict>
        ...
        <key>QIQICLAW_HOME</key>
        <string>{qiqiclaw_home}</string>
    </dict>
...
def refresh_launchd_plist_if_needed() -> bool:
    ...
    print("↻ Updated gateway launchd service definition to match the current QiQiClaw install")
    return True
...
def launchd_install(force: bool = False):
    ...
    print()
    print("Next steps:")
    print("  qiqiclaw gateway status             # Check status")
    from qiqiclaw_constants import display_qiqiclaw_home as _dhh
...
def launchd_stop():
    ...
    # `qiqiclaw gateway start` re-bootstraps when it detects the job is unloaded.
    try:
...
def _wait_for_gateway_exit(timeout: float = 10.0, force_after: float | None = 5.0) -> bool:
    ...
    if remaining_pid is not None:
        print(f"⚠ 网关 PID {remaining_pid} 在 {timeout}s 后仍在运行 — 重启可能失败")
        return False
    return True
...
def launchd_status(deep: bool = False):
    ...
    if launchd_plist_is_current():
        print("✓ Service definition matches the current QiQiClaw install")
    else:
        print("⚠ Service definition is stale relative to the current QiQiClaw install")
        print("  Run: qiqiclaw gateway start")
    ...
    if deep:
        log_file = get_qiqiclaw_home() / "logs" / "gateway.log"
        if log_file.exists():
            print()
            print("Recent logs:")
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    print("┌─────────────────────────────────────────────────────────┐")
    print("│           Q QiQiClaw Gateway Starting...             │")
    print("├─────────────────────────────────────────────────────────┤")
...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Alternatively, provide user ID + password and QiQiClaw will log in directly",
        ...
        "vars": [
            ...
            {"name": "MATRIX_USER_ID", "prompt": "User ID (@bot:server — required for password login)", "password": False,
             "help": "Full Matrix user ID, e.g. @qiqiclaw:matrix.example.org"},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "2. Give it a username (e.g. qiqiclaw) and copy the bot token",
        ...
        "vars": [
            ...
            {"name": "MATTERMOST_HOME_CHANNEL", "prompt": "Home channel ID (for cron/notification delivery, or empty to set later with /set-home)", "password": False,
             "help": "Channel ID where QiQiClaw delivers cron results and notifications."},
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. Use a dedicated email account for your QiQiClaw agent",
        ...
        "vars": [
            {"name": "EMAIL_ADDRESS", "prompt": "Email address", "password": False,
             "help": "The email address QiQiClaw will use (e.g., qiqiclaw@gmail.com)."},
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "5. QiQiClaw connects via the BlueBubbles REST API and receives",
            "   incoming messages via a local webhook",
            "6. To authorize users, use DM pairing: qiqiclaw pairing generate bluebubbles",
    ...
    },
    {
        ...
        "setup_instructions": [
            ...
            "4. Enter them below and QiQiClaw will connect automatically over WebSocket",
        ],
        "vars": [
...
def _all_platforms() -> list[dict]:
    ...
    ``qiqiclaw setup gateway`` without needing the gateway to be running.
    ...
    # User-installed platform plugins under ~/.qiqiclaw/plugins/ still require
...
def _platform_status(platform: dict) -> str:
    ...
    if token_var == "WHATSAPP_ENABLED":
        if val and val.lower() == "true":
            session_file = get_qiqiclaw_home() / "whatsapp" / "session" / "creds.json"
...
def _gateway_platform_schema(platform: dict) -> dict:
    """Return a JSON-serializable platform setup descriptor.

    This is the non-interactive data layer behind ``qiqiclaw gateway setup``.
    The terminal wizard still owns interactive flows such as QR login; desktop
    and other UIs can consume this schema to render native forms for the fields
    that are plain env/config values.
    """
    entry = platform.get("_registry_entry")
    vars_schema = []
    for var in platform.get("vars") or []:
        vars_schema.append({
            "name": var.get("name", ""),
            "prompt": var.get("prompt") or var.get("name", ""),
    ...
    return {
        ...
        "vars": vars_schema,
        "has_interactive_setup": _builtin_setup_fn(key) is not None or (
            entry is not None and entry.setup_fn is not None
        ),
        "install_hint": platform.get("install_hint", ""),
        "source": "plugin" if entry is not None else "builtin",
    }


def gateway_setup_schema() -> dict:
    """Return the structured setup schema shared by CLI and desktop UIs."""
    platforms = [_gateway_platform_schema(p) for p in _all_platforms()]
    return {
        "version": 1,
...
def gateway_setup_status() -> dict:
    """Return structured gateway service/platform status for UI clients."""
    platforms = [_gateway_platform_schema(p) for p in _all_platforms()]
    try:
    ...
    return {
        ...
        "service_running": service_running,
        "gateway_pids": pids,
        "running": service_running or bool(pids),
        "supports_systemd": supports_systemd_services(),
        "supports_launchd": is_macos(),
        "platforms": platforms,
        "runtime_health": runtime_lines,
        "api_server": _api_server_status(),
    }


def _api_server_status() -> dict:
    """Return API server status using the same merged config as gateway run."""
    enabled = (get_env_value("API_SERVER_ENABLED") or "").lower() in (
        "true",
        "1",
        "yes",
    )
    key = get_env_value("API_SERVER_KEY") or ""
    host = get_env_value("API_SERVER_HOST") or "127.0.0.1"
    port_raw = get_env_value("API_SERVER_PORT") or ""
    ...
    try:
        ...
        if api_config is not None:
            enabled = enabled or bool(api_config.enabled)
            key = key or str(api_config.extra.get("key") or "")
            host = str(api_config.extra.get("host") or host)
            port_raw = str(api_config.extra.get("port") or port_raw)
    except Exception:
        pass
...
def print_gateway_setup_schema_json() -> None:
    print(json.dumps(gateway_setup_schema(), indent=2, ensure_ascii=False))


def print_gateway_setup_status_json() -> None:
    print(json.dumps(gateway_setup_status(), indent=2, ensure_ascii=False))
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                access_choices = [
                    ...
                    "Use DM pairing (unknown users request access, you approve with 'qiqiclaw pairing approve')",
                ...
                if access_idx == 0:
                    ...
                    print_info("  Approve with: qiqiclaw pairing approve <platform> <code>")
                else:
                    print_info("  Skipped — configure later with 'qiqiclaw gateway setup'")
            continue
...
def _setup_whatsapp():
    ...
    from qiqiclaw_cli.main import cmd_whatsapp
    import argparse
...
def _setup_dingtalk():
    ...
    if method == 0:
        ...
        try:
            from qiqiclaw_cli.dingtalk_auth import dingtalk_qr_auth
...
def _setup_weixin():
    ...
    print()
    print_info("  1. QiQiClaw will open Tencent iLink QR login in this terminal.")
    print_info("  2. Use WeChat to scan and confirm the QR code.")
    print_info("  3. QiQiClaw will store the returned account_id/token in ~/.qiqiclaw/.env.")
    ...
    if not check_weixin_requirements():
        ...
        print_info("  Install them, then rerun `qiqiclaw gateway setup`.")
        return
    ...
    import asyncio
    try:
        credentials = asyncio.run(qr_login(str(get_qiqiclaw_home())))
    except KeyboardInterrupt:
        print()
    ...
    if access_idx == 0:
        ...
        print_info("  Unknown DM users can request access and you approve them with `qiqiclaw pairing approve`.")
    elif access_idx == 1:
...
def _setup_feishu():
    ...
    if access_idx == 0:
        ...
        print_success("  DM pairing enabled.")
        print_info("  Unknown users can request access; approve with `qiqiclaw pairing approve`.")
    elif access_idx == 1:
...
def _builtin_setup_fn(key: str):
    ...
    Late-bound to avoid a circular import with ``qiqiclaw_cli.setup`` (which
    imports from this module for the remaining bespoke flows).
    """
    from qiqiclaw_cli import setup as _s
    return {
...
def _configure_platform(platform: dict) -> None:
    ...
    is needed here. User-installed platform plugins under ~/.qiqiclaw/plugins/
    ...
    if required:
        print_info(f"  Set these env vars in ~/.qiqiclaw/.env: {', '.join(required)}")
    else:
...
def gateway_setup():
    ...
    print(color("│             Q QiQiClaw Gateway Setup                  │", Colors.MAGENTA))
    print(color("├─────────────────────────────────────────────────────────┤", Colors.MAGENTA))
    print(color("│  Configure messaging platforms and the gateway service. │", Colors.MAGENTA))
    print(color("│  随时按 Ctrl+C 退出。                                  │", Colors.MAGENTA))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA))
            ...
            if prompt_yes_no("  Restart the gateway to pick up changes?", True):
                try:
                    if supports_systemd_services():
                        ...
                        print_info("Start manually: qiqiclaw gateway")
                except UserSystemdUnavailableError as e:
            ...
            if supports_systemd_services() or is_macos():
                ...
                if prompt_yes_no(f"  Install the gateway as a {platform_name} service?{wsl_note} (runs in background, starts on boot)", True):
                    try:
                        ...
                        print_info("  You can try manually: qiqiclaw gateway install")
                else:
                    print_info("  You can install later: qiqiclaw gateway install")
                    if supports_systemd_services():
                        print_info("  Or as a boot-time service: sudo qiqiclaw gateway install --system")
                    print_info("  Or run in foreground:  qiqiclaw gateway run")
            elif is_wsl():
                print_info("  WSL detected but systemd is not running.")
                print_info("  Run in foreground: qiqiclaw gateway run")
                print_info("  For persistence:   tmux new -s qiqiclaw 'qiqiclaw gateway run'")
                ...
                if is_termux():
                    ...
                    print_info("  Run in foreground: qiqiclaw gateway run")
                    print_info(f"  Or start it manually in the background (best effort): nohup qiqiclaw gateway run >{_dhh()}/logs/gateway.log 2>&1 &")
                else:
                    print_info("  Service install not supported on this platform.")
                    print_info("  Run in foreground: qiqiclaw gateway run")
    else:
        print()
        print_info("No platforms configured. Run 'qiqiclaw gateway setup' when ready.")
...
def _gateway_command_inner(args):
    ...
    if subcmd == "setup-schema":
        print_gateway_setup_schema_json()
        return

    if subcmd == "setup-status":
        print_gateway_setup_status_json()
        return
    ...
    if subcmd == "install":
        ...
        if is_termux():
            ...
            print("Run manually: qiqiclaw gateway")
            sys.exit(1)
        if supports_systemd_services():
            if is_wsl():
                print_warning("WSL detected — systemd services may not survive WSL restarts.")
                print_info("  Consider running in foreground instead: qiqiclaw gateway run")
                print_info("  Or use tmux/screen for persistence: tmux new -s qiqiclaw 'qiqiclaw gateway run'")
                print()
            ...
            print("启用 systemd (在 /etc/wsl.conf 中添加 systemd=true 并重启 WSL)")
            print("or run the gateway in foreground mode:")
            print()
            print("  qiqiclaw gateway run                              # direct foreground")
            print("  tmux new -s qiqiclaw 'qiqiclaw gateway run'         # persistent via tmux")
            print("  nohup qiqiclaw gateway run > ~/.qiqiclaw/logs/gateway.log 2>&1 &  # background")
            sys.exit(1)
        elif is_container():
            print("容器内不需要安装服务。")
            print("容器运行时就是您的服务管理器 — 请使用 Docker 重启策略:")
            print()
            print("  docker run --restart unless-stopped ...   # 崩溃/重启时自动重启")
            print("  docker restart <container>                # 手动重启")
            print()
            print("要运行网关: qiqiclaw gateway run")
            sys.exit(0)
        else:
            print("Service installation not supported on this platform.")
            print("Run manually: qiqiclaw gateway run")
            sys.exit(1)
        ...
        system = getattr(args, 'system', False)
        if is_termux():
            print("Termux 不支持网关服务卸载，因为没有托管服务可以移除。")
            print("Stop manual runs with: qiqiclaw gateway stop")
            sys.exit(1)
        if supports_systemd_services():
        ...
        if is_termux():
            print("Termux 不支持网关服务启动，因为没有系统服务管理器。")
            print("Run manually: qiqiclaw gateway")
            sys.exit(1)
        if supports_systemd_services():
            selected_system = _select_systemd_scope(system)
            if get_systemd_unit_path(system=selected_system).exists():
                systemd_start(system=system)
            else:
                pid = _start_detached_gateway_run()
                if not pid:
                    print_error("Failed to start gateway in background.")
                    sys.exit(1)
                if _wait_for_manual_gateway_start(pid):
                    from qiqiclaw_constants import display_qiqiclaw_home as _dhh
                    print_success(f"Gateway started in background (PID: {pid})")
                    print_info(f"Logs: {_dhh()}/logs/gateway.log")
                else:
            ...
            print("  docker restart <container>   # 重启正在运行的容器")
            print()
        ...
        if stop_all:
            ...
            if total:
                print(f"✓ 已停止所有配置文件中的 {total} 个网关进程")
            else:
                print("✗ 未找到网关进程")
        else:
            ...
            if not service_available:
                ...
                print(f"✓ 已停止 {get_service_name()} 服务")
        ...
        if not service_available:
            ...
            if service_configured:
                ...
                print("  Fix the service, then retry: qiqiclaw gateway start")
                sys.exit(1)
        ...
        if supports_systemd_services() and (get_systemd_unit_path(system=False).exists() or get_systemd_unit_path(system=True).exists()):
            ...
            if pids:
                print(f"✓ 网关正在运行 (PID: {', '.join(map(str, pids))})")
                print("  (手动运行，不作为系统服务)")
                runtime_lines = _runtime_health_lines()
                if runtime_lines:
                    print()
                ...
                if is_termux():
                    ...
                    print("WSL note:")
                    print("  网关正在前台/手动模式下运行 (推荐用于 WSL)。")
                    print("  Use tmux or screen for persistence across terminal closes.")
                else:
                    print("To install as a service:")
                    print("  qiqiclaw gateway install")
                    print("  sudo qiqiclaw gateway install --system")
            else:
                ...
                print()
                print("To start:")
                print("  qiqiclaw gateway run      # Run in foreground")
                if is_termux():
                    print("  nohup qiqiclaw gateway run > ~/.qiqiclaw/logs/gateway.log 2>&1 &  # Best-effort background start")
                elif is_wsl():
                    print("  tmux new -s qiqiclaw 'qiqiclaw gateway run'         # persistent via tmux")
                    print("  nohup qiqiclaw gateway run > ~/.qiqiclaw/logs/gateway.log 2>&1 &  # background")
                else:
                    print("  qiqiclaw gateway install  # Install as user service")
                    print("  sudo qiqiclaw gateway install --system  # Install as boot-time system service")

    elif subcmd == "migrate-legacy":
        # Stop, disable, and remove legacy QiQiClaw gateway unit files from
        # pre-rename installs (e.g. qiqiclaw.service). Profile units and
        # unrelated third-party services are never touched.
