...
on Japanese locales, etc.).  QiQiClaw's banners, tool output feed, and slash
command listings all contain Unicode: box-drawing characters, mathematical and
geometric symbols, and user-supplied text in any language.  Printing those to a
cp1252 console raises ``UnicodeEncodeError: 'charmap' codec can't encode
character...`` and kills the whole CLI before the REPL even opens.
...
Entry points (``cli.py`` ``main``, ``qiqiclaw_cli/main.py`` CLI dispatch,
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent -- if there's no attached console (e.g. QiQiClaw is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``QIQICLAW_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("QIQICLAW_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the QiQiClaw-managed Git install directories so
    ...
    # already-running shells don't see the change -- which means qiqiclaw
    ...
    # install without QiQiClaw-managed PortableGit).
    ...
    # stdin is re-configured for completeness; QiQiClaw's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and QiQiClaw's
    ``qiqiclaw config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching QiQiClaw (User env var in Windows Settings, or
    export in a PowerShell profile) and QiQiClaw picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known QiQiClaw-managed tool directories to os.environ['PATH'].
    ...
    ``%USERPROFILE%\\.qiqiclaw\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only -- already-running shells (including the one the
    user invokes ``qiqiclaw`` from right after install) retain their old PATH.

    Any subprocess QiQiClaw spawns -- bash, ``rg``, ``grep``, ``npm`` -- inherits
    ...
    Patch-up strategy: add the known QiQiClaw-managed tool directories to our
    ...
    qiqiclaw_home = os.environ.get("QIQICLAW_HOME") or (
        os.path.join(user_profile, ".qiqiclaw") if user_profile else ""
    )
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(qiqiclaw_home, "git", "cmd") if qiqiclaw_home else "",
        os.path.join(qiqiclaw_home, "git", "bin") if qiqiclaw_home else "",
        os.path.join(qiqiclaw_home, "git", "usr", "bin") if qiqiclaw_home else "",
        # QiQiClaw venv Scripts directory -- host of the qiqiclaw.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes qiqiclaw, but harmless to include.
        os.path.join(qiqiclaw_home, "qiqiclaw", "venv", "Scripts")
        if qiqiclaw_home
        else "",
    ...
    for d in candidate_dirs:
        if d and os.path.isdir(d) and d.lower() not in existing_lower:
            prepend.append(d)
