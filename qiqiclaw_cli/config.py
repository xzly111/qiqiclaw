...
- qiqiclaw config          - Show current configuration
- qiqiclaw config edit     - Open config in editor
- qiqiclaw config set      - Set a specific value
- qiqiclaw config wizard   - Re-run setup wizard
"""
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `qiqiclaw plugins enable
    # observability/langfuse` or `qiqiclaw tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "QIQICLAW_LANGFUSE_ENV",
    "QIQICLAW_LANGFUSE_RELEASE",
    "QIQICLAW_LANGFUSE_SAMPLE_RATE",
    "QIQICLAW_LANGFUSE_MAX_CHARS",
    "QIQICLAW_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from qiqiclaw_cli.default_soul import DEFAULT_SOUL_MD
...
def get_managed_system() -> Optional[str]:
    ...
    managed_marker = get_qiqiclaw_home() / ".managed"
...
def is_managed() -> bool:
    """Check if QiQiClaw is running in package-manager-managed mode.

    Two signals: the QIQICLAW_MANAGED env var (set by the systemd service),
    or a .managed marker file in QIQICLAW_HOME (set by the NixOS activation
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "qiqiclaw update"


def format_managed_message(action: str = "modify this QiQiClaw installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = _legacy_env("MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"无法{action}：此 QiQiClaw 安装由 NixOS 管理 "
            f"(QIQICLAW_MANAGED={env_hint})。\n"
            "请在 configuration.nix 中编辑 services.qiqiclaw.settings 并运行：\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this QiQiClaw installation is managed by Homebrew "
            f"(QIQICLAW_MANAGED={env_hint})。\n"
            "请使用：\n"
            "  brew upgrade qiqiclaw"
        )

    return (
        f"无法{action}：此 QiQiClaw 安装由 {managed_system} 管理。\n"
        "请使用您的包管理器升级或重新安装 QiQiClaw。"
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from QIQICLAW_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hermes_bin
    or None if container mode is not active, we're already inside the
    container, or QIQICLAW_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if _legacy_env("DEV") == "1":
        return None
    ...
    container_mode_file = get_qiqiclaw_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "qiqiclaw")
    exec_user = info.get("exec_user", "qiqiclaw")
    hermes_bin = info.get("hermes_bin", "/data/current-package/bin/qiqiclaw")
...
# Re-export from qiqiclaw_constants — canonical definition lives there.
from qiqiclaw_constants import get_project_root, get_qiqiclaw_home  # noqa: F811,E402
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the qiqiclaw group can
    share state with the gateway service.

    The mode can be overridden via the QIQICLAW_HOME_MODE environment variable
    (e.g. QIQICLAW_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse QIQICLAW_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("QIQICLAW_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When QiQiClaw runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if _legacy_env("CONTAINER") or _legacy_env("SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set QIQICLAW_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into QIQICLAW_HOME if the user doesn't have one yet."""
...
def ensure_qiqiclaw_home():
    """Ensure ~/.qiqiclaw directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_qiqiclaw_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_qiqiclaw_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"QIQICLAW_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the QiQiClaw-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, QiQiClaw auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), QiQiClaw sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when QiQiClaw
        ...
        #  "/home/user/.qiqiclaw/cache/documents:/output"]
        ...
        # bundled QiQiClaw image, which drops to the `qiqiclaw` user via gosu).
    ...
    "browser": {
        ...
        "camofox": {
            # When true, QiQiClaw sends a stable profile-scoped userId to Camofox
    ...
    "checkpoints": {
        ...
        # ~/.qiqiclaw/checkpoints/ (one per cd'd working directory). Field
        # reports put the typical offender at 1000+ repos / ~12 GB. When
        # auto_prune is on, qiqiclaw sweeps at startup (at most once per
    ...
    # single read_file page exceeds these limits, QiQiClaw truncates the
    ...
    "auxiliary": {
        ...
        # model; override via `qiqiclaw model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `qiqiclaw --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `qiqiclaw -c` muscle memory.  Default off so existing
        # users aren't surprised.  QIQICLAW_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        "bell_on_complete": False,
        "show_reasoning": True,
        "streaming": True,
        "startup_animation": True,
        "startup_animation_speed": "fast",
        "semantic_highlight": True,
        "status_bar_style": "full",  # full | compact
        "tool_line_style": "card",   # card | legacy
        "final_response_markdown": "strip",  # render | strip | raw
        "inline_diffs": True,     # Show inline diff previews for write actions (write_file, patch, skill_manage)
        "show_cost": True,       # Show $ cost in the status bar
        "skin": "reasonix-modern",
        ...
        # e.g. `model · 68% · ~/projects/qiqiclaw`. Per-platform overrides go under
    ...
    "tts": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.qiqiclaw/cache/piper-voices/
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.qiqiclaw/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        "max_spawn_depth": 2,        # depth cap (1 = flat, 2 = orchestrator→leaf [default], 3 = three-level)
    ...
    # response. If not, QiQiClaw feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before QiQiClaw auto-pauses the goal and
    ...
    # always goes to ~/.qiqiclaw/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${QIQICLAW_SKILL_DIR} and ${QIQICLAW_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `qiqiclaw curator status` for the last run summary.
    "curator": {
        "enabled": True,
    ...
    # This section is only needed for qiqiclaw-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "Q *QiQiClaw*" header.
    ...
    # stored approval from ~/.qiqiclaw/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or QIQICLAW_ACCEPT_HOOKS=1.
    ...
    "cron": {
        ...
        # Also overridable via QIQICLAW_CRON_MAX_PARALLEL env var.
    ...
    # todos to ready, and fires `qiqiclaw -p <assignee> chat -q ...` for
    ...
    "code_execution": {
        ...
        #     qiqiclaw's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.qiqiclaw/logs/.
    ...
    # update model picker lists without shipping a qiqiclaw release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "https://qiqiclaw.nousresearch.com/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `qiqiclaw model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.qiqiclaw/state.db.
    ...
    "sessions": {
        ...
        # default of ``qiqiclaw sessions prune``.
        "retention_days": 90,
    ...
    # ``qiqiclaw update`` behaviour.
    "updates": {
        # Run a full ``qiqiclaw backup``-style zip of QIQICLAW_HOME before every
        # ``qiqiclaw update``.  Backups land in ``<QIQICLAW_HOME>/backups/`` and
        # can be restored with ``qiqiclaw import <path>``.  Off by default —
        # on large QIQICLAW_HOME directories the zip can add minutes to every
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'qiqiclaw model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit Nous Subscriber access token for tool-gateway requests (optional; otherwise read from the QiQiClaw auth store)",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @qiqiclaw:example.org)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #qiqiclaw)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: qiqiclaw-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'qiqiclaw' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote QiQiClaw API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote QiQiClaw API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote QiQiClaw API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # QIQICLAW_TOOL_PROGRESS and QIQICLAW_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "QIQICLAW_TOOL_PROGRESS": {
    ...
    "QIQICLAW_PREFILL_MESSAGES_FILE": {
    ...
    "QIQICLAW_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.qiqiclaw.config`` entries, then checks
    ...
    try:
        ...
        # should never break `qiqiclaw update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older QiQiClaw versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``qiqiclaw_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'qiqiclaw setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — QiQiClaw won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'qiqiclaw doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("QIQICLAW_HOME", "~/.qiqiclaw")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        if fixes and not quiet:
            print(f"  ✓ 修复了 .env 文件（修复了 {fixes} 个损坏的条目）")
    except Exception:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("QIQICLAW_TOOL_PROGRESS")
            old_mode = get_env_value("QIQICLAW_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in ("false", "0", "no"):
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from QIQICLAW_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in ("new", "all"):
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from QIQICLAW_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("QIQICLAW_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from QIQICLAW_TIMEZONE)")
            else:
                config["timezone"] = ""
            ...
            if not quiet:
                ...
                print(f"  ✓ 已将时区添加到 config.yaml：{tz_display}")
    ...
    if current_ver < 9:
        try:
            ...
            if old_token:
                ...
                if not quiet:
                    print("  ✓ 已从 .env 中清除 ANTHROPIC_TOKEN（不再使用）")
        except Exception:
            pass
    ...
    if current_ver < 12:
        ...
        if isinstance(custom_list, list) and custom_list:
            ...
            if migrated_count > 0:
                ...
                save_config(config)
                if not quiet:
                    print(f"  ✓ 已迁移 {migrated_count} 个自定义提供商到 providers: 部分")
        ...
        for dead_var in ("LLM_MODEL", "OPENAI_MODEL"):
            try:
                ...
                if old_val:
                    ...
                    if not quiet:
                        print(f"  ✓ 已从 .env 中清除 {dead_var}（不再使用 — config.yaml 是真实来源）")
            except Exception:
                pass
    ...
    if current_ver < 14:
        ...
        if isinstance(raw_stt, dict) and "model" in raw_stt:
            ...
            save_config(config)
            if not quiet:
                print(f"  ✓ 已将旧版 stt.model 迁移到特定提供商配置")
    ...
    if current_ver < 15:
        ...
        if "interim_assistant_messages" not in display:
            ...
            save_config(config)
            if not quiet:
                print("  ✓ 已添加 display.interim_assistant_messages=true")
    ...
    if current_ver < 16:
        ...
        if isinstance(old_overrides, dict) and old_overrides:
            ...
            if not quiet:
                ...
                print(f"  ✓ 已将 tool_progress_overrides → display.platforms 迁移：{migrated}")
    ...
    if current_ver < 17:
        ...
        if isinstance(comp, dict):
            ...
            if migrated_keys or s_model is not None or s_provider is not None or s_base_url is not None:
                ...
                if not quiet:
                    if migrated_keys:
                        print(f"  ✓ 已将 compression.summary_* → auxiliary.compression 迁移：{', '.join(migrated_keys)}")
                    else:
                        print("  ✓ 已移除未使用的 compression.summary_* 键")
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$QIQICLAW_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_qiqiclaw_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `qiqiclaw plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `qiqiclaw curator status` has no
    ...
    #   3. Creates `~/.qiqiclaw/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_qiqiclaw_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_qiqiclaw_home() / "logs" / "curator"
    ...
    if interactive and missing_env:
        ...
        for var in missing_env:
            if var.get("url"):
                print(f"  在此获取您的密钥：{var['url']}")

            if var.get("password"):
                # Visible input — better UX, supports paste
            ...
            if value:
                ...
                results["warnings"].append(f"已跳过 {var['name']} - 某些功能可能无法使用")
            print()
            ...
            if answer in ("y", "yes"):
                ...
                for name, info in new_and_unset:
                    if info.get("url"):
                        ...
                        print(f"  在此获取您的密钥：{info['url']}")
                    else:
                        print(f"  {info.get('description', name)}")
                    if info.get("password"):
                        # Visible input — better UX, supports paste
                        value = input(f"  {info.get('prompt', name)}（按回车跳过）: ").strip()
                    else:
                        value = input(f"  {info.get('prompt', name)}（按回车跳过）: ").strip()
                    if value:
                ...
                print("  稍后使用以下命令设置：qiqiclaw config set <key> <value>")
    ...
    # metadata.qiqiclaw.config in their SKILL.md frontmatter.
            ...
            for var in missing_skill_config:
                ...
                if value:
                    ...
                    results["warnings"].append(
                        f"已跳过 {var['key']} — 技能 '{var.get('skill', '?')}' 可能稍后会请求它"
                    )
                print()
            save_config(config)
        else:
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _qiqiclaw_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.qiqiclaw/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.qiqiclaw/config.yaml.
    ...
    (which change ``QIQICLAW_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    ensure_qiqiclaw_home()
    config_path = get_config_path()
    path_key = str(config_path)
    ...
    if cache_key is not None and cache_key[1] == 0 and not is_managed():
        try:
            save_config(DEFAULT_CONFIG)
            st = config_path.stat()
            cache_key = (st.st_mtime_ns, st.st_size)
        except Exception as e:
            logger.debug("Could not seed empty config.yaml: %s", e)
...
#   openai-codex (OAuth — qiqiclaw auth) — OpenAI Codex
#   nous         (OAuth — qiqiclaw auth) — Nous Portal
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.qiqiclaw/config.yaml."""
    if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.qiqiclaw/.env.
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real QiQiClaw env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.qiqiclaw/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.qiqiclaw/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.qiqiclaw/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.qiqiclaw/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to QiQi — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known QiQiClaw vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.qiqiclaw/.env or environment."""
...
def show_config():
    ...
    print(color("│              Q QiQiClaw 配置                         │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(f"  推理：        {'开启' if display.get('show_reasoning', False) else '关闭'}")
    print(f"  提示音：      {'开启' if display.get('bell_on_complete', False) else '关闭'}")
    ump = display.get('user_message_preview', {}) if isinstance(display.get('user_message_preview', {}), dict) else {}
    ump_first = ump.get('first_lines', 2)
    ump_last = ump.get('last_lines', 2)
    print(f"  用户预览：    前 {ump_first} 行，后 {ump_last} 行")
    ...
    if terminal.get('backend') == 'docker':
        ...
        print(f"  Vercel 运行时：{terminal.get('vercel_runtime', 'node24')}")
        print(f"  Vercel 认证：  {'已配置' if get_env_value('VERCEL_OIDC_TOKEN') or (get_env_value('VERCEL_TOKEN') and get_env_value('VERCEL_PROJECT_ID') and get_env_value('VERCEL_TEAM_ID')) else '（未设置）'}")
    ...
    print(color("  qiqiclaw config edit     # Edit config file", Colors.DIM))
    print(color("  qiqiclaw config set <key> <value>", Colors.DIM))
    print(color("  qiqiclaw setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    if key.upper() in api_keys or key.upper().endswith(('_API_KEY', '_TOKEN')) or key.upper().startswith('TERMINAL_SSH'):
        ...
        print(f"✓ 已在 {get_env_path()} 中设置 {key}")
        return
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: qiqiclaw config set <key> <value>")
            print()
            print("Examples:")
            print("  qiqiclaw config set model anthropic/claude-sonnet-4")
            print("  qiqiclaw config set terminal.backend docker")
            print("  qiqiclaw config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            print(f"\n  将添加 {len(missing_config)} 个新配置选项（使用默认值）")
        ...
        if optional_missing:
            print(f"\n  ℹ️  未配置 {len(optional_missing)} 个可选的 API 密钥：")
        ...
        if missing_config:
            print()
            print(color(f"  有 {len(missing_config)} 个新配置选项可用", Colors.YELLOW))
            print("    运行 'qiqiclaw config migrate' 来添加它们")
        ...
        print()
        print("可用命令：")
        print("  qiqiclaw config           显示当前配置")
        print("  qiqiclaw config edit      在编辑器中打开配置")
        print("  qiqiclaw config set <key> <value>   Set a config value")
        print("  qiqiclaw config check     检查缺失/过时的配置")
        print("  qiqiclaw config migrate   使用新选项更新配置")
        print("  qiqiclaw config path      显示配置文件路径")
        print("  qiqiclaw config env-path  显示 .env 文件路径")
        sys.exit(1)
