...
into prompt_toolkit's event loop. Each function takes the QiQiClawCLI instance
...
from qiqiclaw_cli.banner import cprint, _DIM, _RST
from qiqiclaw_cli.config import save_env_value_secure
...
def clarify_callback(cli, question, choices):
    ...
    if hasattr(cli, "_app") and cli._app:
        cli._app.invalidate()
    cprint(f"\n{_DIM}(澄清在 {timeout}秒后超时 — 代理将自行决定){_RST}")
    return (
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.qiqiclaw/.env and never exposed to the model.
    """
    if not getattr(cli, "_app", None):
        ...
        try:
            value = getpass.getpass(f"{prompt} (empty Enter to skip): ")
        ...
        cprint(f"\n{_DIM}  ✓ 密钥已存储到 {_dhh}/.env 作为 {var_name}{_RST}")
        return {
            **stored,
            "skipped": False,
