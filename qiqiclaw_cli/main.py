#!/usr/bin/env python3
"""
QiQiClaw CLI - Main entry point.

Usage:
    qiqiclaw                     # Interactive chat (default)
    qiqiclaw chat                # Interactive chat
    qiqiclaw gateway             # Run gateway in foreground
    qiqiclaw gateway start       # Start gateway as service
    qiqiclaw gateway stop        # Stop gateway service
    qiqiclaw gateway status      # Show gateway status
    qiqiclaw gateway install     # Install gateway service
    qiqiclaw gateway uninstall   # Uninstall gateway service
    qiqiclaw setup               # Interactive setup wizard
    qiqiclaw logout              # Clear stored authentication
    qiqiclaw status              # Show status of all components
    qiqiclaw cron                # Manage cron jobs
    qiqiclaw cron list           # List cron jobs
    qiqiclaw cron status         # Check if cron scheduler is running
    qiqiclaw doctor              # Check configuration and dependencies
    qiqiclaw honcho setup                    # Configure Honcho AI memory integration
    qiqiclaw honcho status                   # Show Honcho config and connection status
    qiqiclaw honcho sessions                 # List directory → session name mappings
    qiqiclaw honcho map <name>               # Map current directory to a session name
    qiqiclaw honcho peer                     # Show peer names and dialectic settings
    qiqiclaw honcho peer --user NAME         # Set user peer name
    qiqiclaw honcho peer --ai NAME           # Set AI peer name
    qiqiclaw honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    qiqiclaw honcho mode                     # Show current memory mode
    qiqiclaw honcho mode [hybrid|honcho|local]  # Set memory mode
    qiqiclaw honcho tokens                   # Show token budget settings
    qiqiclaw honcho tokens --context N       # Set session.context() token cap
    qiqiclaw honcho tokens --dialectic N     # Set dialectic result char cap
    qiqiclaw honcho identity                 # Show AI peer identity representation
    qiqiclaw honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    qiqiclaw honcho migrate                  # Step-by-step migration guide: OpenClaw native → QiQiClaw + Honcho
    qiqiclaw version             Show version
    qiqiclaw update              Update to latest version
    qiqiclaw uninstall           Uninstall QiQiClaw
    qiqiclaw acp                 Run as an ACP server for editor integration
    qiqiclaw sessions browse     Interactive session picker with search

    qiqiclaw claw migrate --dry-run  # Preview migration without changes
"""

try:
    from qiqiclaw_constants import _legacy_env, _set_legacy_env
    import qiqiclaw_bootstrap  # noqa: F401 — UTF-8 bootstrap (Windows only)
except ImportError:
    pass
...
_existing_main_module = getattr(builtins, "qiqiclaw_main", None)
if _existing_main_module is None:
    builtins.qiqiclaw_main = sys.modules[__name__]
else:
    # Some tests deliberately remove qiqiclaw_cli.main from sys.modules and
    # import it again. Keep package-level patch targets and legacy qiqiclaw_main
    # users on the original module object so monkeypatches don't split across
    # two independently loaded copies.
    sys.modules[__name__] = _existing_main_module
    _pkg = sys.modules.get(__package__)
    if _pkg is not None:
        setattr(_pkg, "main", _existing_main_module)
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to QIQICLAW_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (qiqiclaw tools, qiqiclaw setup, qiqiclaw model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"错误：'qiqiclaw {command_name}' 需要交互式终端。\n"
            f"它不能通过管道或非交互式子进程运行。\n"
            f"请直接在终端中运行。",
            file=sys.stderr,
        )
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any qiqiclaw module import.
#
# Many modules cache QIQICLAW_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("QIQICLAW_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.qiqiclaw/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set QIQICLAW_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If QIQICLAW_HOME is already set and no explicit flag was given, trust it.
    # This lets child processes (relaunch, subprocess) inherit the parent's
    # profile choice without having to pass --profile again.
    if profile_name is None and os.environ.get("QIQICLAW_HOME"):
        return

    # 2. If no flag, check active_profile in the qiqiclaw root
    if profile_name is None:
        try:
            from qiqiclaw_constants import get_default_qiqiclaw_root

            active_path = get_default_qiqiclaw_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set QIQICLAW_HOME
    if profile_name is not None:
        try:
            from qiqiclaw_cli.profiles import resolve_profile_env

            qiqiclaw_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"错误：{exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent qiqiclaw from starting
            print(
                f"警告：配置文件覆盖失败（{exc}），使用默认值",
                file=sys.stderr,
            )
            return
        os.environ["QIQICLAW_HOME"] = qiqiclaw_home
...
# Load .env from ~/.qiqiclaw/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from qiqiclaw_cli.config import get_qiqiclaw_home
from qiqiclaw_cli.env_loader import load_qiqiclaw_dotenv

load_qiqiclaw_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml to QIQICLAW_REDACT_SECRETS env
# var BEFORE hermes_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "QIQICLAW_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early
        _cfg_path = get_qiqiclaw_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    _set_legacy_env("REDACT_SECRETS", str(_early_redact).lower())
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `qiqiclaw` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from qiqiclaw_logging import setup_logging as _setup_logging
...
try:
    from qiqiclaw_cli.config import load_config as _load_config_early
    from qiqiclaw_constants import apply_ipv4_preference as _apply_ipv4
...
from qiqiclaw_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from qiqiclaw_cli.config import get_env_path, get_qiqiclaw_home, load_config
    from qiqiclaw_cli.auth import get_auth_status

    # Determine whether QiQiClaw itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from qiqiclaw_cli.config import DEFAULT_CONFIG
    ...
    auth_file = get_qiqiclaw_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if QiQiClaw has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants QiQiClaw to use their tokens.
...
def _session_browse_picker(sessions: list, allow_actions: bool = False) -> dict:
    """Interactive curses-based session browser with live search filtering.

    Args:
        sessions: List of session dicts from list_sessions_rich()
        allow_actions: If True, enable d/r/n keys for delete/rename/new actions

    Returns:
        Dict with keys:
        - action: "open" | "delete" | "rename" | "new" | "quit"
        - session_id: Selected session ID (for open/delete/rename)
    ...
    if not sessions:
        ...
        return {"action": "new"} if allow_actions else {"action": "quit"}
    ...
    try:
        ...
        def _curses_browse(stdscr):
            ...
            while True:
                ...
                # Header line
                if rename_mode[0]:
                    header = f"  Rename session — new title: {rename_buffer[0]}█  (Enter to save, Esc to cancel)"
                    ...
                    if allow_actions:
                        header = "  Browse sessions — ↑↓ navigate  Enter select  d delete  r rename  n new  / filter  q quit"
                    else:
                ...
                if rename_mode[0]:
                    if key in (curses.KEY_ENTER, 10, 13):
                        new_name = rename_buffer[0].strip()
                        if new_name and filtered:
                            result_holder[0] = {
                                "action": "rename",
                                "session_id": filtered[cursor]["id"],
                                "new_name": new_name
                            }
                            return
                        rename_mode[0] = False
                        ...
                        rename_buffer[0] = rename_buffer[0][:-1]
                    elif 32 <= key <= 126:
                        rename_buffer[0] += chr(key)
                    continue
                ...
                if key in (curses.KEY_UP,):
                    ...
                    if filtered:
                        result_holder[0] = {"action": "open", "session_id": filtered[cursor]["id"]}
                    return
                elif key == 27:  # Esc
                    if search_text:
                    ...
                    return
                elif key == ord("d") and allow_actions and not search_text and filtered:
                    # Delete selected session
                    result_holder[0] = {"action": "delete", "session_id": filtered[cursor]["id"]}
                    return
                elif key == ord("r") and allow_actions and not search_text and filtered:
                    # Enter rename mode
                    rename_mode[0] = True
                    current_title = filtered[cursor].get("title") or ""
                    rename_buffer[0] = current_title
                elif key == ord("n") and allow_actions and not search_text:
                    # New session
                    result_holder[0] = {"action": "new"}
                    return
                elif key == ord("/") and not search_text:
                    # Start search mode (just fall through to add character)
                    pass
                elif 32 <= key <= 126:
    ...
    if allow_actions:
        print("\n  浏览会话（输入数字恢复会话，d<N> 删除，n 新建，q 取消）\n")
    else:
        print("\n  浏览会话（输入数字恢复会话，q 取消）\n")
    ...
    while True:
        try:
            if allow_actions:
                val = input(f"\n  Select [1-{len(sessions)}] or d<N>/n/q: ").strip()
            else:
            ...
            if allow_actions and val.lower() == "n":
                return {"action": "new"}

            if allow_actions and val.lower().startswith("d"):
                try:
                    idx = int(val[1:]) - 1
                    if 0 <= idx < len(sessions):
                        return {"action": "delete", "session_id": sessions[idx]["id"]}
                    print(f"  无效选择。请输入 d1-d{len(sessions)}。")
                    continue
                except (ValueError, IndexError):
                    print("  无效输入。删除格式: d<数字>，例如 d1")
                    continue

            idx = int(val) - 1
            if 0 <= idx < len(sessions):
                return {"action": "open", "session_id": sessions[idx]["id"]}
            print(f"  无效选择。请输入 1-{len(sessions)} 或 q 取消。")
        except ValueError:
            print("  无效输入。请输入数字或 q 取消。")
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        ...
        cli_args: the original CLI arguments (everything after 'qiqiclaw')
    """
    ...
    if not runtime:
        print(
            f"错误：在 PATH 中未找到 {backend}。无法路由到容器。",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if probe.returncode != 0:
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    f"错误：通过 {backend} 未找到容器 '{container_name}'。\n"
                    f"\n"
                    f"容器可能以 root 身份运行。您的用户无法看到它\n"
                    f"因为 {backend} 使用每用户命名空间。为 {backend} 授予无密码\n"
                    f"sudo 权限 — 需要 -n（非交互式）标志\n"
                    f"因为密码提示会挂起或破坏管道命令。\n"
                    f"\n"
                    f"在 NixOS 上：\n"
                    f"\n"
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"或运行：sudo qiqiclaw {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"错误：通过 {backend} 未找到容器 '{container_name}'。\n"
                f"容器可能在 root 下运行。尝试：sudo qiqiclaw {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(session_id: Optional[str], active_session_file: Optional[str] = None) -> None:
    ...
    print(f"  qiqiclaw --tui --resume {target}")
    if title:
        print(f'  qiqiclaw --tui -c "{title}"')
    print()
    print(f"会话：        {target}")
    if title:
        print(f"标题：          {title}")
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @qiqiclaw/ink is missing or node_modules is behind package-lock.json.
    ...
    """
    ink = root / "node_modules" / "@qiqiclaw" / "ink" / "package.json"
    if not ink.is_file():
        return True
...
def _find_bundled_tui(tui_dir: Path) -> Optional[Path]:
    """Directory whose dist/entry.js we should run: QIQICLAW_TUI_DIR first, else repo ui-tui."""
    env = _legacy_env("TUI_DIR", None)
    if env:
        p = Path(env)
...
def _hermes_ink_bundle_stale(tui_dir: Path) -> bool:
    ink_root = tui_dir / "packages" / "qiqiclaw-ink"
...
def _ensure_tui_node() -> None:
    ...
    ``QIQICLAW_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if _legacy_env("SKIP_NODE_BOOTSTRAP", None):
        return
    ...
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "QIQICLAW_HOME": qiqiclaw_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(qiqiclaw_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev -> tsx src; else node dist (QIQICLAW_TUI_DIR or ui-tui, build when stale)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = _legacy_env("NODE", None)
    ...
    # pre-built dist + node_modules (nix / full QIQICLAW_TUI_DIR) skips npm.
    if not tui_dev:
        ext_dir = _legacy_env("TUI_DIR", None)
        if ext_dir:
            p = Path(ext_dir)
            if (p / "dist" / "entry.js").exists() and not _tui_need_npm_install(p):
                node = _node_bin("node")
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from qiqiclaw_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="qiqiclaw-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["QIQICLAW_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["QIQICLAW_PYTHON_SRC_ROOT"] = os.environ.get(
        "QIQICLAW_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("QIQICLAW_PYTHON", sys.executable)
    env.setdefault("QIQICLAW_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")
    if model:
        env["QIQICLAW_MODEL"] = model
        env["QIQICLAW_INFERENCE_MODEL"] = model
    if provider:
        env["QIQICLAW_TUI_PROVIDER"] = provider
        env["QIQICLAW_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["QIQICLAW_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    ...
    if resume_session_id:
        env["QIQICLAW_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or _legacy_env("TUI", None) == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print(f"未找到匹配 '{continue_val}' 的会话。")
                print("使用 'qiqiclaw sessions list' 查看可用的会话。")
                sys.exit(1)
        else:
    ...
    # ── Session picker (ported from Reasonix) ──
    # Auto-show session picker when starting chat without explicit session selection
    no_picker = getattr(args, "no_session_picker", False)
    quiet_mode = getattr(args, "query", None) is not None  # -q/--query skips picker

    if not resume_val and not continue_val and not no_picker and not quiet_mode and not use_tui:
        db = None
        try:
            from qiqiclaw_state import SessionDB
            ...
            if sessions:
                result = _session_browse_picker(sessions, allow_actions=True)

                if result["action"] == "open":
                    args.resume = result["session_id"]
                elif result["action"] == "delete":
                    db.delete_session(result["session_id"])
                    db.close()
                    db = None
                    # Recursively show picker again after deletion
                    import copy
                    args_copy = copy.copy(args)
                    return cmd_chat(args_copy)
                elif result["action"] == "rename":
                    try:
                        db.set_session_title(result["session_id"], result["new_name"])
                    except Exception as e:
                        print(f"重命名失败: {e}")
                    db.close()
                    db = None
                    # Show picker again after rename
                    import copy
                    args_copy = copy.copy(args)
                    return cmd_chat(args_copy)
                elif result["action"] == "quit":
                    sys.exit(0)
                # "new" action falls through to start new session

        except Exception:
            pass  # Picker failure doesn't block chat startup
        finally:
            if db is not None:
                try:
                    db.close()
    ...
    # First-run guard: block only non-interactive runs that would immediately
    # need a provider. Interactive chat can still open so the visual shell,
    # help, setup, and model commands remain reachable on a fresh install.
    from qiqiclaw_cli.setup import (
        is_interactive_stdin,
        print_noninteractive_setup_guidance,
    )

    has_interactive_stdin = is_interactive_stdin()
    if not _has_any_provider_configured() and (
        quiet_mode or not has_interactive_stdin
    ):
        print()
        print(
            "It looks like QiQiClaw isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  qiqiclaw setup")
        print()
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("您可以随时运行 'qiqiclaw setup' 进行配置。")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from qiqiclaw_cli.banner import prefetch_update_check
    ...
    # user's ~/.qiqiclaw/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        _set_legacy_env("IGNORE_USER_CONFIG", "1")

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        _set_legacy_env("IGNORE_RULES", "1")

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        _set_legacy_env("SESSION_SOURCE", args.source)
...
def cmd_gateway(args):
    ...
    from qiqiclaw_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from qiqiclaw_cli.config import get_env_value, save_env_value

    print()
    print("Q QiQiClaw WhatsApp Setup")
    print("=" * 50)
    ...
    if not current_mode:
        print()
        print("How will you use WhatsApp with QiQiClaw?")
        print()
    ...
    if not (bridge_dir / "node_modules").exists():
        ...
        if not npm:
            print("  ✗ 未在 PATH 中找到 npm — 请先安装 Node.js")
            return
        try:
            result = subprocess.run(
    ...
    session_dir = get_qiqiclaw_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("\n✓ WhatsApp 已配置并配对！")
            print("  Start the gateway with: qiqiclaw gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  qiqiclaw gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with 'Q QiQiClaw'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  qiqiclaw gateway")
        ...
        print()
        print("  或安装为服务：qiqiclaw gateway install")
    else:
        print("⚠ 配对可能未完成。运行 'qiqiclaw whatsapp' 重试。")


def cmd_setup(args):
    """Interactive setup wizard."""
    from qiqiclaw_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``qiqiclaw model``) and the setup wizard
    ...
    from qiqiclaw_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or _legacy_env("INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'qiqiclaw model' for "
                "available providers, or run 'qiqiclaw doctor' to diagnose config "
                "issues."
            )
            print(f"警告：{warning} 回退到自动提供商检测。")
    if active is None:
        try:
    ...
    from qiqiclaw_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from qiqiclaw_cli.config import read_raw_config
    ...
    if _has_saved_custom_list:
        ordered.append(("remove-custom", "Remove saved custom provider"))
    ...
    if selected_provider == "openrouter":
        ...
        if provider_info is None:
            print(
                "Warning: selected saved custom provider is no longer available. "
                "警告：所选保存的自定义提供商不再可用。"
                "它可能已从 config.yaml 中删除。未进行更改。"
            )
            return
    ...
    # a leftover OPENAI_BASE_URL in ~/.qiqiclaw/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.qiqiclaw/.env if the active provider is not 'custom'.
    ...
    """
    from qiqiclaw_cli.config import get_env_value, save_env_value, load_config
...
#
# QiQi uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `qiqiclaw model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `qiqiclaw model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  QiQiClaw only falls back to a lightweight backend (OpenRouter,")
        print("  Nous Portal）如果主模型不可用。覆盖一个")
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``qiqiclaw model`` flow, then route aux tasks to them here.
    """
    from qiqiclaw_cli.config import load_config
...
def _aux_flow_provider_model(
    ...
    from qiqiclaw_cli.auth import _prompt_model_selection
    from qiqiclaw_cli.models import get_pricing_for_provider
...
def _aux_flow_custom_endpoint(task: str, task_cfg: dict) -> None:
    ...
    print("  提供一个 OpenAI 兼容的基础 URL（例如 http://localhost:11434/v1）")
    print()
    try:
    ...
    try:
        api_key = input("API key (optional, blank = use OPENAI_API_KEY): ").strip()
    except (KeyboardInterrupt, EOFError):
        print()
        return
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from qiqiclaw_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from qiqiclaw_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    if not api_key:
        ...
        try:
            ...
            key = input("AI Gateway API key (or Enter to cancel): ").strip()
        except (KeyboardInterrupt, EOFError):
            print()
            return
        if not key:
    ...
    from qiqiclaw_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from qiqiclaw_cli.nous_subscription import prompt_enable_tool_gateway

    state = get_provider_auth_state("nous")
    if not state or not state.get("access_token"):
        print("未登录 Nous Portal。正在开始登录...")
        print()
        try:
    ...
    from qiqiclaw_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from qiqiclaw_cli.auth import DEFAULT_NOUS_PORTAL_URL

            _url = (_nous_portal_url or DEFAULT_NOUS_PORTAL_URL).rstrip("/")
            print(f"在 {_url} 升级以访问付费模型。")
        return
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from qiqiclaw_cli.codex_models import get_codex_model_ids
    ...
    _codex_token = None
    # Prefer credential pool (where `qiqiclaw auth` stores device_code tokens),
    ...
    if not _codex_token:
        try:
            from qiqiclaw_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from qiqiclaw_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    if not state or not state.get("access_token"):
        print("未登录 MiniMax。正在开始 OAuth 登录...")
        print()
        try:
            mock_args = argparse.Namespace(
    ...
    from qiqiclaw_cli.models import _PROVIDER_MODELS
    ...
    print(f"\u2713 \u4f7f\u7528 MiniMax \u6a21\u578b\uff1a{selected}")
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    Flow:
      ...
      5. Save to ~/.qiqiclaw/config.yaml.
    """
    from qiqiclaw_cli.auth import (
    ...
    print()
    print("⚠  Google 认为将 Gemini CLI OAuth 客户端用于第三方")
...
def _model_flow_custom(config):
    ...
    """
    from qiqiclaw_cli.auth import _save_model_choice, deactivate_provider
    from qiqiclaw_cli.config import get_env_value, load_config, save_config
    ...
    from qiqiclaw_cli.models import probe_api_models

    probe = probe_api_models(effective_key, effective_url)
    if probe.get("used_fallback") and probe.get("resolved_base_url"):
        print(
            f"警告：端点验证在 {probe['resolved_base_url']}/models 处成功，"
            f"而不是您输入的确切 URL。将保存有效的基础 URL。"
        )
        ...
        print(
            f"通过 {probe.get('probed_url')} 验证端点 "
            f"（可见 {len(probe.get('models') or [])} 个模型）"
        )
    else:
        print(
            f"警告：无法通过 {probe.get('probed_url')} 验证此端点。"
            f"QiQi 仍将保存它。"
        )
    ...
    try:
        if len(detected_models) == 1:
            print(f"  检测到模型：{detected_models[0]}")
    ...
    if model_name:
        ...
        print("端点已保存。在聊天中使用 `/model` 或 `qiqiclaw model` 设置模型。")
...
def _save_custom_provider(
    ...
    cfg["custom_providers"] = providers
    save_config(cfg)
    print(f'  💾 已保存到自定义提供商为 "{name}"（在 config.yaml 中编辑）')
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from qiqiclaw_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from qiqiclaw_cli.config import get_env_value, save_env_value, load_config, save_config
    from qiqiclaw_cli import azure_detect
    import getpass
    ...
    print("=" * 50)
    print()
    print("Azure Foundry 可以使用 OpenAI 风格或其他格式托管模型")
    print("Anthropic-style API endpoints.  QiQiClaw will probe your")
    ...
    if api_mode:
        ...
        print(f"✓ 检测到 API 传输方式：{mode_label}")
        if detection.reason:
            print(f"    ({detection.reason})")
        if discovered_models:
            print(f"✓ 在此端点上找到 {len(discovered_models)} 个已部署的模型")
    else:
        print(f"⚠ 自动检测未完成：{detection.reason}")
        print()
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from qiqiclaw_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from qiqiclaw_cli.config import save_env_value, load_config, save_config
    from qiqiclaw_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from qiqiclaw_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from qiqiclaw_cli.copilot_auth import validate_copilot_token
        ...
        if source in ("GITHUB_TOKEN", "GH_TOKEN"):
            print(f"  GitHub 令牌：{api_key[:8]}... ✓ ({source})")
    ...
    if live_models:
        ...
        print(f"  从 GitHub Copilot 找到 {len(model_list)} 个模型")
    else:
        ...
        if model_list:
            ...
            print('    如果您没有看到您的模型，请使用"输入自定义模型名称"。')
    ...
    if selected:
        ...
        if reasoning_efforts:
            if selected_effort == "none":
                ...
                print(f"推理强度已设置为：{selected_effort}")
    else:
        print("No change.")
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    print("  GitHub Copilot ACP delegates QiQiClaw turns to `copilot --acp`.")
    print("  QiQiClaw currently starts its own ACP subprocess for each request.")
    print("  QiQiClaw uses your selected model as a hint for the Copilot ACP session.")
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set QIQICLAW_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _model_flow_kimi(config, current_model=""):
    ...
    if is_coding_plan:
        ...
        effective_base = pconfig.inference_base_url
        print(f"  使用 Moonshot 端点 → {effective_base}")
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if existing_key:
        print(f"  Bedrock API 密钥：{existing_key[:12]}... ✓")
    else:
    ...
    print(f"  显示 {len(model_list)} 个精选模型")
    ...
    if selected:
        ...
        # Save the API key env var name so qiqiclaw knows where to find it
...
def _model_flow_bedrock(config, current_model=""):
    ...
    try:
        ...
        print("  ✗ boto3 未安装。使用以下命令安装：")
        print("    pip install boto3")
        print()
        return

    if not has_aws_credentials():
        print("  ⚠ 未通过环境变量检测到 AWS 凭证。")
    ...
    print("       支持 EC2 实例角色、SSO、环境变量、aws configure")
    print("    2. Bedrock API Key")
    print("       直接输入您的 Bedrock API 密钥 — 也支持")
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "gemini" and existing_key:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   QiQiClaw typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with QiQiClaw, enable billing on your "
            ...
            if tier == "paid":
                ...
                print("  Tier check: could not verify (continuing).")
            print()
    ...
    if provider_id == "lmstudio":
        from qiqiclaw_cli.auth import AuthError
        from qiqiclaw_cli.models import fetch_lmstudio_models
        ...
        try:
            ...
            model_list = []
        if model_list:
            print(f"  从 LM Studio 找到 {len(model_list)} 个模型")
    elif provider_id == "ollama-cloud":
        from qiqiclaw_cli.models import fetch_ollama_cloud_models
        ...
        if model_list:
            print(f"  从 Ollama Cloud 找到 {len(model_list)} 个模型")
    else:
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from qiqiclaw_constants import display_qiqiclaw_home as _dhh_fn

            print(
                f"    QiQiClaw will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
    ...
    try:
        print()
        print("  正在运行 'claude setup-token' — 请按照下面的提示操作。")
        ...
        if manual_token:
            ...
            print("  ✓ Setup-token saved. 设置令牌已保存。")
            return True
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'qiqiclaw model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate QiQiClaw CLI with a provider."""
    from qiqiclaw_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from qiqiclaw_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from qiqiclaw_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from qiqiclaw_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from qiqiclaw_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from qiqiclaw_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    Dispatches ``qiqiclaw slack <subcommand>``. Currently supports:
    ...
    if sub in (None, ""):
        ...
        print(
            "usage: qiqiclaw slack <subcommand>\n"
            "\n"
            "subcommands:\n"
            "  manifest   Generate a Slack app manifest with every gateway\n"
            "             command registered as a native slash\n"
            "\n"
            "Run `qiqiclaw slack manifest -h` for details.",
            file=sys.stderr,
        )
        return 1

    if sub == "manifest":
        from qiqiclaw_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from qiqiclaw_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from qiqiclaw_cli.hooks import hooks_command
    hooks_command(args)


def cmd_doctor(args):
    """Check configuration and dependencies."""
    from qiqiclaw_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from qiqiclaw_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from qiqiclaw_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from qiqiclaw_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up QiQiClaw home directory to a zip file."""
    if getattr(args, "quick", False):
        from qiqiclaw_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from qiqiclaw_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a QiQiClaw backup from a zip file."""
    from qiqiclaw_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"QiQiClaw v{__version__} ({__release_date__})")
    ...
    try:
        from qiqiclaw_cli.banner import check_for_updates
        from qiqiclaw_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from qiqiclaw_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``qiqiclaw update --gateway`` so interactive prompts (stash restore,
    ...
    prompt_path.unlink(missing_ok=True)
    response_path.unlink(missing_ok=True)
    print(f"  （{int(timeout)}秒后无响应，使用默认值：{default!r}）")
    return default


def _check_web_dist() -> bool:
    """Return True when the minimal dashboard frontend exists."""
    web_index = Path(__file__).parent / "web_dist" / "index.html"
    return web_index.exists()


def _web_ui_dist_dir(web_dir: Path) -> Path:
    """Return the legacy Vite output dir for a given ``web/`` source tree."""
    return web_dir.parent / "qiqiclaw_cli" / "web_dist"


def _web_ui_build_needed(web_dir: Path) -> bool:
    """Return True when a legacy web UI source tree needs rebuilding."""
    dist_dir = _web_ui_dist_dir(web_dir)
    manifest = dist_dir / ".vite" / "manifest.json"
    index_html = dist_dir / "index.html"

    sentinel = manifest if manifest.exists() else index_html if index_html.exists() else None
    if sentinel is None:
        return True
    ...
    for candidate in watch_files:
        try:
            if candidate.exists() and candidate.stat().st_mtime > sentinel_mtime:
                return True
        except OSError:
            continue
    ...
    if web_dir.exists():
        for path in web_dir.rglob("*"):
            ...
            if rel.parts and rel.parts[0] in {"node_modules", "dist"}:
                continue
            try:
                if path.stat().st_mtime > sentinel_mtime:
                    return True
            except OSError:
                continue
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``qiqiclaw update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False, env: Optional[dict[str, str]] = None) -> bool:
    ...
    if web_dir.exists():
        ...
        if not npm:
            if fatal:
                print("QiQiClaw web frontend build requires npm.")
            return False
        ...
        if install_result.returncode != 0:
            if fatal:
                print("QiQiClaw web frontend dependency install failed.")
            return False
    ...
    if _check_web_dist():
        return True
    if fatal:
        print("QiQiClaw web frontend not found.")
        print("Create: qiqiclaw_cli/web_dist/index.html")
    return False


def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``qiqiclaw dashboard`` processes other than ourselves.

    ``qiqiclaw dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``qiqiclaw update`` replaces files on disk, the running
    ...
    patterns = [
        "qiqiclaw dashboard",
        "qiqiclaw_cli.main dashboard",
        "qiqiclaw_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    ...
    try:
        if sys.platform == "win32":
            ...
            # (rather than `pgrep -f "qiqiclaw.*dashboard"`) keeps us consistent
            # with `qiqiclaw_cli.gateway._scan_gateway_pids` and avoids the
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``qiqiclaw dashboard`` processes.

    Called at the end of ``qiqiclaw update`` (default ``reason``) and also
    from ``qiqiclaw dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    qiqiclaw dashboard --port <port>")
...
def _update_via_zip(args):
    """Update QiQiClaw by downloading a ZIP archive.
    ...
    print("→ 正在下载最新版本...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="qiqiclaw-update-")
        zip_path = os.path.join(tmp_dir, f"qiqiclaw-{branch}.zip")
        ...
        # GitHub ZIPs extract to qiqiclaw-<branch>/
        extracted = os.path.join(tmp_dir, f"qiqiclaw-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "qiqiclaw-update-autostash-%Y%m%d-%H%M%S"
    )
    print("→ 检测到本地更改 — 更新前暂存...")
    subprocess.run(
...
def _print_stash_cleanup_guidance(
    ...
    print("  使用 git stash list --format='%gd %H %s' 查找已保存的条目")
    if stash_selector:
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if QiQiClaw behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes qiqiclaw completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but QiQiClaw couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but QiQiClaw couldn't drop the saved stash entry."
            )
    ...
    print("⚠ Local changes restored on top of the updated codebase.")
    print("  Review `git diff` / `git status` if QiQiClaw behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `qiqiclaw update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_qiqiclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_qiqiclaw_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(
    ...
    *,
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official QiQiClaw repository.")
        print("  This means you may miss updates from xzly111/qiqiclaw.")
        print()
        try:
            response = (
        ...
        if response in ("", "y", "yes"):
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/xzly111/qiqiclaw.git"
                )
                has_upstream = True
            else:
                print("  ✗ 无法添加上游远程仓库。跳过上游同步。")
                return
        else:
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/xzly111/qiqiclaw.git' to add later."
            )
    ...
    if _sync_fork_with_upstream(git_cmd, cwd):
        ...
        print("    您的本地仓库已更新，但您在 GitHub 上的 fork 可能落后。")
...
def _invalidate_update_cache():
    ...
    ``qiqiclaw update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from qiqiclaw_constants import get_default_qiqiclaw_root

    default_home = get_default_qiqiclaw_root()
...
def _load_installable_optional_extras() -> list[str]:
    ...
    # Entries look like "qiqiclaw[matrix]" or "package-name[extra]".
...
def _update_node_dependencies(*, env: Optional[dict[str, str]] = None) -> None:
    ...
    for label, path in paths:
        ...
        print(f"  ⚠ npm install 在 {label} 中失败")
...
class _UpdateOutputStream:
    """Stream wrapper used during ``qiqiclaw update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.qiqiclaw/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``qiqiclaw update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``qiqiclaw update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.qiqiclaw/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``qiqiclaw update --gateway``) the update is already
    ...
    try:
        ...
        # qiqiclaw_cli.config.get_qiqiclaw_home to simulate setup failure.
        from qiqiclaw_cli.config import get_qiqiclaw_home as _get_qiqiclaw_home

        logs_dir = _get_qiqiclaw_home() / "logs"
...
def _cmd_update_check():
    """Implement ``qiqiclaw update --check``: fetch and report without installing."""
    git_dir = PROJECT_ROOT / ".git"
    if not git_dir.exists():
        print("✗ 不是 git 仓库 — 无法检查更新。")
        sys.exit(1)
    ...
    if fetch_result.returncode != 0:
        ...
        if "Could not resolve host" in stderr or "unable to access" in stderr:
            ...
            print("✗ 认证失败 — 请检查您的 git 凭据或 SSH 密钥。")
        else:
            print("✗ 从 origin 获取失败。")
            if stderr:
    ...
    if behind == 0:
        ...
        commits_word = "commit" if behind == 1 else "commits"
        print(f"Q 有可用更新：落后 origin/main {behind} 个{commits_word}。")
        from qiqiclaw_cli.config import recommended_update_command
        print(f"  运行 '{recommended_update_command()}' 进行安装。")
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``qiqiclaw update`` without requiring a reinstall.  The
    ...
    ``qiqiclaw`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/qiqiclaw.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v qiqiclaw'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/qiqiclaw, code at /usr/local/lib/qiqiclaw).
    fhs_link = Path("/usr/local/bin/qiqiclaw")
    ...
    path_comment = (
        "# QiQiClaw — ensure /usr/local/bin is on PATH "
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of QIQICLAW_HOME before running the update.
    ...
    QIQICLAW_HOME directories.  The ``--backup`` flag on ``qiqiclaw update``
    ...
    try:
        from qiqiclaw_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(f"⚠ 更新前备份：无法加载备份模块（{exc}）；继续更新。")
        print()
        return

    print("◆ Creating pre-update backup... 正在创建更新前备份...")
    t0 = _time.monotonic()
    try:
    ...
    # Render path using display_qiqiclaw_home so the user sees ~/.qiqiclaw/...
    try:
        from qiqiclaw_constants import get_qiqiclaw_home, display_qiqiclaw_home
        home = get_qiqiclaw_home()
        try:
            display_path = f"{display_qiqiclaw_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  qiqiclaw import {out_path}")
    print(f"  Disable: omit --backup (backup is off by default)")
    print(f"            set updates.pre_update_backup: false in config.yaml")
    print()


def cmd_update(args):
    """Update QiQiClaw to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from qiqiclaw_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update QiQiClaw")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    if sys.platform == "win32" and git_probe.returncode == 0:
        subprocess.run(
            [
                "git",
                "-c",
    ...
    try:
        ...
        if fetch_result.returncode != 0:
            stderr = fetch_result.stderr.strip()
            if "Could not resolve host" in stderr or "unable to access" in stderr:
                print("✗ Network error — 无法访问远程仓库。")
                print(f"  {stderr.splitlines()[0]}" if stderr else "")
            elif (
                "Authentication failed" in stderr or "could not read Username" in stderr
            ):
                print(
                    "✗ Authentication failed — 请检查您的 git 凭据或 SSH 密钥。"
                )
            else:
                print(f"✗ 从 origin 获取更新失败。")
                if stderr:
                    print(f"  {stderr.splitlines()[0]}")
            sys.exit(1)
        ...
        # the old bytecode (e.g. get_qiqiclaw_home added to hermes_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # attributes like display_qiqiclaw_home() added since the last release.
        try:
            import importlib
            import qiqiclaw_constants as _hc
        ...
        try:
            from qiqiclaw_cli.profiles import (
                list_profiles,
        ...
        if needs_migration:
            ...
            if missing_config:
                print(f"  ℹ️  {len(missing_config)} 个新配置选项可用")
            ...
            if assume_yes:
                ...
                print(
                    "    Run 'qiqiclaw config migrate' later to apply any new config/env options."
                )
                response = "n"
            else:
                try:
            ...
            if response in ("", "y", "yes"):
                ...
                if (gateway_mode or assume_yes) and missing_env:
                    print("  ℹ API keys require manual entry: qiqiclaw config migrate")
            else:
                print()
                print("已跳过。稍后运行 'qiqiclaw config migrate' 进行配置。")
        else:
            print("  ✓ 配置已是最新")
        ...
        # When running as ``qiqiclaw update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_qiqiclaw_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            try:
                from qiqiclaw_constants import (
            ...
            # Discover all qiqiclaw-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. qiqiclaw-gateway.service or qiqiclaw-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    plural = "" if unmapped_count == 1 else "s"
                    print(f"  → Stopped {unmapped_count} manual gateway{plural}")
                    print("    Restart manually: qiqiclaw gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: qiqiclaw -p <profile> gateway run  for each profile)"
                        )
        ...
        # Warn if legacy QiQiClaw gateway unit files are still installed.
        # When both qiqiclaw.service (from a pre-rename install) and the
        # current qiqiclaw-gateway.service are enabled, they SIGTERM-fight
        # for the same bot token (see PR #11909). Flagging here means
        # every `qiqiclaw update` surfaces the issue until the user migrates.
        try:
            from qiqiclaw_cli.gateway import (
                has_legacy_qiqiclaw_units,
                _find_legacy_hermes_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_qiqiclaw_units():
                print()
                print("⚠ Legacy QiQiClaw gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_hermes_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (qiqiclaw.service) fight the current")
                print("  qiqiclaw-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    qiqiclaw gateway migrate-legacy")
                print()
        ...
        print("  qiqiclaw model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``qiqiclaw -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `qiqiclaw profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_qiqiclaw_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → qiqiclaw -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.qiqiclaw)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  qiqiclaw profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     qiqiclaw -p {name} chat")
                else:
        ...
        from qiqiclaw_cli.profiles import profile_exists
        ...
        if remove:
            ...
            if wrapper_path:
                ...
                if custom_name:
                    wrapper_path.write_text(f'#!/bin/sh\nexec qiqiclaw -p {name} "$@"\n')
                print(f"✓ Alias created: {wrapper_path}")
                if not _is_wrapper_dir_in_path():
                    print(f"⚠ {_get_wrapper_dir()} is not in your PATH.")

    elif action == "rename":
        from qiqiclaw_cli.profiles import rename_profile
        ...
        from qiqiclaw_cli.profiles import export_profile
        ...
        from qiqiclaw_cli.profiles import import_profile
...
def _report_dashboard_status() -> int:
    """Print ``qiqiclaw dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``qiqiclaw dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No qiqiclaw dashboard processes running.")
        return 0

    print(f"{len(pids)} qiqiclaw dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No qiqiclaw dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `qiqiclaw update`.
    ...
    if _legacy_env("WEB_DIST") is None:
        if not _check_web_dist():
            if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
                sys.exit(1)
        elif not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from qiqiclaw_cli.web_server import start_server

    embedded_chat = args.tui or _legacy_env("DASHBOARD_TUI", None) == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from qiqiclaw_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter QiQiClaw log files."""
    from qiqiclaw_cli.logs import tail_log, list_logs
...
def cmd_langgraph(args):
    """Run the LangGraph-backed QiQiClaw workflow."""
    from qiqiclaw_cli.langgraph_runner import run_cli
...
def main():
    ...
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for Nous login (default: qiqiclaw-cli)",
    )
    model_parser.add_argument(
    ...
    # =========================================================================
    from qiqiclaw_cli.fallback_cmd import cmd_fallback
    ...
    fallback_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "https://qiqiclaw.nousresearch.com/docs/user-guide/features/fallback-providers"
        ),
    )
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `qiqiclaw model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_setup_schema = gateway_subparsers.add_parser(
        "setup-schema",
        help="Emit structured gateway setup schema for desktop/UI clients",
    )
    gateway_setup_schema.add_argument("--json", action="store_true", default=True)

    gateway_setup_status = gateway_subparsers.add_parser(
        "setup-status",
        help="Emit structured gateway setup status for desktop/UI clients",
    )
    gateway_setup_status.add_argument("--json", action="store_true", default=True)

    # gateway migrate-legacy
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy qiqiclaw.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy QiQiClaw gateway unit files "
            "(e.g. qiqiclaw.service) left over from older installs. Profile "
            "units (qiqiclaw-gateway-<profile>.service) and unrelated "
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure QiQiClaw with an interactive wizard. "
        "Run a specific section: qiqiclaw setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        help="(Default on existing installs.) Re-run the full wizard, "
             ...
             "compatibility — a bare 'qiqiclaw setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_parser = subparsers.add_parser(
        ...
        description="Slack integration helpers for QiQiClaw.",
    )
    ...
    slack_manifest.add_argument(
        ...
        help="Write manifest to a file instead of stdout. With no PATH "
             "writes to $QIQICLAW_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
        help='Bot display name (default: "QiQiClaw")',
    )
    slack_manifest.add_argument(
        "--description",
        default=None,
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for QiQiClaw CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: qiqiclaw-cli)"
    )
    ...
    auth_spotify = auth_subparsers.add_parser("spotify", help="Authenticate QiQiClaw with Spotify via PKCE")
    auth_spotify.add_argument("spotify_action", nargs="?", choices=["login", "status", "logout"], default="login")
    auth_spotify.add_argument("--client-id", help="Spotify app client_id (or set QIQICLAW_SPOTIFY_CLIENT_ID)")
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of QiQiClaw components",
    )
    ...
    # =========================================================================
    from qiqiclaw_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.qiqiclaw/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.qiqiclaw/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with QiQiClaw setup",
    )
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your QiQiClaw setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for QiQiClaw. Use 'qiqiclaw debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    qiqiclaw debug share              Upload debug report and print URL
    qiqiclaw debug share --lines 500  Include more log lines
    qiqiclaw debug share --expire 30  Keep paste for 30 days
    qiqiclaw debug share --local      Print report locally (no upload)
    qiqiclaw debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'qiqiclaw debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up QiQiClaw home directory to a zip file",
        description="Create a zip archive of your entire QiQiClaw configuration, "
        "skills, sessions, and data (excludes the qiqiclaw codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/qiqiclaw-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a QiQiClaw backup from a zip file",
        description="Extract a previously created QiQiClaw backup into your "
        "QiQiClaw home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage QiQiClaw configuration",
    )
    ...
    def cmd_pairing(args):
        from qiqiclaw_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.qiqiclaw/skills/.bundled_manifest) "
            "so future 'qiqiclaw update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from qiqiclaw_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from qiqiclaw_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/qiqiclaw-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `qiqiclaw plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from qiqiclaw_cli.plugins_cmd import plugins_command
    ...
    try:
        from qiqiclaw_cli.curator import register_cli as _register_curator_cli
    ...
    _memory_setup_parser = memory_sub.add_parser(
        "setup", help="Interactive provider selection and configuration"
    )
    _memory_setup_parser.add_argument(
        "provider",
        nargs="?",
        default=None,
        help="Provider to configure directly (e.g. honcho), skipping the picker",
    )
    _memory_status_parser = memory_sub.add_parser(
        "status", help="Show current memory provider config"
    )
    _memory_status_parser.add_argument(
        "--json",
        action="store_true",
        help="Emit structured memory status for desktop/UI clients",
    )
    _memory_provider_parser = memory_sub.add_parser(
        "provider",
        help="Select external memory provider, or empty/builtin for built-in only",
    )
    _memory_provider_parser.add_argument("provider", nargs="?", default="")
    _memory_provider_parser.add_argument(
        "--json",
        action="store_true",
        help="Emit structured result for desktop/UI clients",
    )
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_qiqiclaw_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_qiqiclaw_home()}/memories/\n")
        else:
            from qiqiclaw_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Run 'qiqiclaw tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # qiqiclaw tools list [--platform cli]
    ...
    # qiqiclaw tools disable <name...> [--platform cli]
    ...
    # qiqiclaw tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from qiqiclaw_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from qiqiclaw_cli.tools_config import tools_command
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run QiQiClaw as an MCP server",
        description=(
            "Manage MCP server connections and run QiQiClaw as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'qiqiclaw mcp add' to connect to a new server, or\n"
            "'qiqiclaw mcp serve' to expose QiQiClaw conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run QiQiClaw as an MCP server (expose conversations to other agents)",
    )
    ...
    def cmd_mcp(args):
        from qiqiclaw_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if args.session_id:
                ...
                if not resolved_session_id:
                    print(f"会话 '{args.session_id}' 未找到。")
                    return
            ...
            # Launch qiqiclaw --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from qiqiclaw_cli.relaunch import relaunch
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to QiQiClaw",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to QiQiClaw",
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.qiqiclaw/ (by default a "
        "single restore-point archive is written to ~/.qiqiclaw/backups/ "
        "before apply; restorable with 'qiqiclaw import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from qiqiclaw_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update QiQiClaw to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall QiQiClaw",
        description="Remove QiQiClaw from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run QiQiClaw as an ACP (Agent Client Protocol) server",
        description="Start QiQiClaw in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch QiQiClaw as an ACP server."""
        try:
            ...
            print("ACP 依赖未安装。")
            print("使用以下命令安装：pip install -e '.[acp]'")
            sys.exit(1)
    ...
    proxy_parser = subparsers.add_parser(
        "proxy",
        help="Run a local OpenAI-compatible proxy for OAuth providers",
        description="Expose OAuth providers (Claude Pro, ChatGPT Pro, SuperGrok) as localhost OpenAI-compatible endpoints",
    )
    proxy_parser.add_argument("--port", type=int, default=8080, help="Port to listen on (default: 8080)")
    proxy_parser.add_argument("--host", default="127.0.0.1", help="Host to bind to (default: 127.0.0.1)")

    def cmd_proxy(args):
        """Launch the OpenAI-compatible local proxy server."""
        try:
            from qiqiclaw_cli.proxy.cli import run_proxy
            run_proxy(host=args.host, port=args.port)
        except ImportError:
            print("Proxy module not available. Check your installation.")
            sys.exit(1)
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated QiQiClaw instances",
    )
    ...
    # langgraph command
    # =========================================================================
    langgraph_parser = subparsers.add_parser(
        "langgraph",
        help="Run a LangGraph-orchestrated QiQiClaw workflow",
        description=(
            "Use LangGraph for stateful workflow orchestration while delegating "
            "reasoning, memory, skills, MCP tools, and model routing to QiQiClaw."
        ),
    )
    langgraph_parser.add_argument("prompt", nargs="+", help="Prompt to route through the workflow")
    langgraph_parser.add_argument("-m", "--model", default=None, help="Model override for the QiQiClaw node")
    langgraph_parser.add_argument("--provider", default=None, help="Provider override for the QiQiClaw node")
    langgraph_parser.add_argument(
        "-t",
        "--toolsets",
        default=None,
        help="Comma-separated toolsets to enable for the QiQiClaw node",
    )
    langgraph_parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Compile and execute the graph with a deterministic local runner instead of calling a model",
    )
    langgraph_parser.add_argument(
        "--source-path",
        default=None,
        help="Path to a langgraph-main checkout to drive (default: bundled vendor/langgraph-main)",
    )
    langgraph_parser.add_argument(
        "--no-local-source",
        action="store_true",
        help="Use the installed langgraph package instead of the local langgraph-main checkout",
    )
    langgraph_parser.add_argument("--json", action="store_true", help="Print final graph state as JSON")
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the QiQiClaw web dashboard for managing config, API keys, and sessions",
    )
    dashboard_parser.add_argument(
        "--port",
        type=int,
        default=9120,
        help="Port shared with QiQiClaw Desktop (default 9120)",
    )
    dashboard_parser.add_argument(
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `qiqiclaw --tui` via PTY/WebSocket). "
            "Alternatively set QIQICLAW_DASHBOARD_TUI=1."
        ),
    )
    ...
    # for `qiqiclaw dashboard` cmdlines and SIGTERM them directly — the same
    # path `qiqiclaw update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running qiqiclaw dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running qiqiclaw dashboard processes and exit",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter QiQiClaw log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    qiqiclaw logs                    Show last 50 lines of agent.log
    qiqiclaw logs -f                 Follow agent.log in real time
    qiqiclaw logs errors             Show last 50 lines of errors.log
    qiqiclaw logs gateway -n 100     Show last 100 lines of gateway.log
    qiqiclaw logs --level WARNING    Only show WARNING and above
    qiqiclaw logs --session abc123   Filter by session ID
    qiqiclaw logs --component tools  Only show tool-related lines
    qiqiclaw logs --since 1h         Lines from the last hour
    qiqiclaw logs --since 30m -f     Follow, starting from 30 min ago
    qiqiclaw logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``qiqiclaw -c Pokemon Agent Dev`` → ``qiqiclaw -c 'Pokemon Agent Dev'``
    ...
    from qiqiclaw_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'qiqiclaw -c model' where 'model' is consumed as the
    ...
    # so introspection/management commands (qiqiclaw hooks list, cron
    ...
    if (
        ...
        try:
            from qiqiclaw_cli.plugins import discover_plugins
            discover_plugins()
        except Exception:
            logger.debug(
    ...
    if getattr(args, "oneshot", None):
        from qiqiclaw_cli.oneshot import run_oneshot
