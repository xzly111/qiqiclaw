"""
Backup and import commands for QiQiClaw CLI.

`qiqiclaw backup` creates a zip archive of the entire ~/.qiqiclaw/ directory
(excluding the qiqiclaw repo and transient files).

`qiqiclaw import` restores from a backup zip, overlaying onto the current
QIQICLAW_HOME root.
"""
...
from qiqiclaw_constants import get_default_qiqiclaw_root, get_qiqiclaw_home, display_qiqiclaw_home
...
_EXCLUDED_DIRS = {
    "qiqiclaw",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to qiqiclaw root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the QiQiClaw home directory."""
    hermes_root = get_default_qiqiclaw_root()

    if not hermes_root.is_dir():
        print(f"错误：未找到 QiQiClaw 主目录 {hermes_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"qiqiclaw-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"qiqiclaw-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_qiqiclaw_home()} ...")
    ...
    for dirpath, dirnames, filenames in os.walk(hermes_root, followlinks=False):
        ...
        for fname in filenames:
            ...
            # Skip the output zip itself if it happens to be inside qiqiclaw root
            try:
    ...
    print(f"\nRestore with: qiqiclaw import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a QiQiClaw backup.
    ...
    # Look for telltale files that a qiqiclaw home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a QiQiClaw backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.qiqiclaw/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a qiqiclaw dir name
        if prefix in (".qiqiclaw", "qiqiclaw"):
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_qiqiclaw_home()}")
        ...
        if profiles_dir.is_dir():
            try:
                ...
                # qiqiclaw_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: qiqiclaw profile list  (after installing qiqiclaw)")

        # Guidance
        print()
        if not (hermes_root / "qiqiclaw").is_dir():
            print("注意：qiqiclaw 代码库未包含在备份中。")
            print("  If this is a fresh install, run: qiqiclaw update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  qiqiclaw -p {pname} gateway install")

        print("完成。您的 QiQiClaw 配置已恢复。")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and qiqiclaw backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to QIQICLAW_HOME).
...
# — `qiqiclaw update` snapshots this set before pulling so approved-user lists
...
def _quick_snapshot_root(qiqiclaw_home: Optional[Path] = None) -> Path:
    home = qiqiclaw_home or get_qiqiclaw_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    qiqiclaw_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(qiqiclaw_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(qiqiclaw_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for qiqiclaw backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_qiqiclaw_home()}/state-snapshots/")
...
def _pre_update_backup_dir(qiqiclaw_home: Optional[Path] = None) -> Path:
    home = qiqiclaw_home or get_qiqiclaw_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of QIQICLAW_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<QIQICLAW_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``qiqiclaw update``) should continue even if the backup fails.
    """
    hermes_root = qiqiclaw_home or get_default_qiqiclaw_root()
    if not hermes_root.is_dir():
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `qiqiclaw claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of QIQICLAW_HOME under ``backups/`` before a
    ``qiqiclaw claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``qiqiclaw import <archive>``.  Writes to
    ``<QIQICLAW_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `qiqiclaw import` and the
