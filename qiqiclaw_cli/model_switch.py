...
- ``qiqiclaw_cli.providers``        -- canonical provider identity + overlays
- ``qiqiclaw_cli.model_normalize``  -- per-provider name formatting
...
from qiqiclaw_cli.model_normalize import (
...
_QIQICLAW_MODEL_WARNING = (
    "The selected QiQiClaw chat model is NOT agentic and is not designed "
    "for QiQiClaw tool use. It lacks the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Nous Research QiQiClaw 3 / QiQiClaw 4 chat families.
# The previous substring check (`"qiqiclaw" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``qiqiclaw-brain:qwen3-14b-ctx16k`` that just
# happen to carry "qiqiclaw" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   NousResearch/QiQiClaw-3-Llama-3.1-70B, qiqiclaw-4-405b, openrouter/hermes3:70b
# Negative examples it must NOT match:
#   qiqiclaw-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_NOUS_HERMES_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])(?:qiqi\s*claw|qiqiclaw|hermes)[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_nous_hermes_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real Nous QiQiClaw 3/4 chat model.
...
def _check_hermes_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a Nous QiQiClaw 3/4 chat model."""
    if is_nous_hermes_non_agentic(model_name):
        return _QIQICLAW_MODEL_WARNING
    return ""
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from qiqiclaw_cli.model_switch import
...
def switch_model(
    ...
    from qiqiclaw_cli.runtime_provider import resolve_runtime_provider
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'qiqiclaw model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'qiqiclaw doctor' — config issues detected:"
        ...
        if not new_model:
            if pdef.base_url:
                from qiqiclaw_cli.runtime_provider import _auto_detect_local_model
    ...
    # Mirrors the same logic in qiqiclaw_cli.runtime_provider.resolve_runtime_provider;
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from qiqiclaw_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from qiqiclaw_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by qiqiclaw provider ID
    ...
    # --- 1. Check QiQiClaw-mapped providers ---
    for hermes_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        # section 2 (QIQICLAW_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                from qiqiclaw_cli.auth import _load_auth_store
        ...
        # show up in the picker without requiring a QiQiClaw release.
    ...
    # --- 2. Check QiQiClaw-only providers (nous, openai-codex, copilot, opencode-go) ---
    from qiqiclaw_cli.providers import QIQICLAW_OVERLAYS
    from qiqiclaw_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → QiQiClaw provider ID.
    # QIQICLAW_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use QiQiClaw IDs ("copilot").
    _mdev_to_hermes = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in QIQICLAW_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve QiQiClaw slug — e.g. "github-copilot" → "copilot"
        ...
        # reflects the active region (eu.*, ap.*) not the static us.* list.
            ...
            # Use curated list — look up by QiQiClaw slug, fall back to overlay key
            model_ids = curated.get(hermes_slug, []) or curated.get(pid, [])
    ...
    # in PROVIDER_TO_MODELS_DEV or QIQICLAW_OVERLAYS (keeps /model in sync
    # with `qiqiclaw model`).
    try:
        from qiqiclaw_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is QiQiClaw's canonical write key (matches
            ...
            # QiQi writes ``models:`` as a dict keyed by model id
            # (see qiqiclaw_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # QiQiClaw's own writer uses; a hyphen variant is accepted
            ...
            # active model. QiQiClaw's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, qiqiclaw_cli/config.py) already consume that dict.
