"""qiqiclaw hooks — inspect and manage shell-script hooks.

Usage::

    qiqiclaw hooks list
    qiqiclaw hooks test <event> [--for-tool X] [--payload-file F]
    qiqiclaw hooks revoke <command>
    qiqiclaw hooks doctor

Consent records live under ``~/.qiqiclaw/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.qiqiclaw/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``qiqiclaw hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("用法: qiqiclaw hooks {list|test|revoke|doctor}")
        print("运行 'qiqiclaw hooks --help' 查看详情。")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.qiqiclaw/config.yaml.")
        print("查看 `qiqiclaw hooks --help` 或")
            ...
            if is_approved:
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            f"      ⚠ 脚本在批准后已修改 "
                            f"(之前 {mtime_at}, 现在 {mtime_now}) — "
                            f"运行 `qiqiclaw hooks doctor` 重新验证"
                        )
        print()
...
# stdin a script sees under `qiqiclaw hooks test` and `qiqiclaw hooks doctor`
...
def _cmd_test(args) -> None:
    from qiqiclaw_cli.config import load_config
    from qiqiclaw_cli.plugins import VALID_HOOKS
    from agent import shell_hooks
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (QiQiClaw wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `qiqiclaw hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `qiqiclaw hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `qiqiclaw hooks doctor`.")
