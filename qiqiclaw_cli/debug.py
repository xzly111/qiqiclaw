"""``qiqiclaw debug`` — debug tools for QiQiClaw.

Currently supports:
    qiqiclaw debug share    Upload debug report (system info + logs) to a
...
def _pending_file() -> Path:
    """Path to ``~/.qiqiclaw/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``qiqiclaw debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``qiqiclaw debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_PRIVACY_NOTICE = """\
⚠️  这将上传以下内容到公共粘贴服务：
  • 系统信息（操作系统、Python 版本、QiQiClaw 版本、提供商、已配置的 API 密钥
    — 不包括实际密钥）
  • 最近的日志行（agent.log、errors.log、gateway.log — 可能包含
    对话片段和文件路径）
  • 完整的 agent.log 和 gateway.log（每个最多 512 KB — 可能包含
    对话内容、工具输出和文件路径）
...
_GATEWAY_PRIVACY_NOTICE = (
    "⚠️ **隐私提示：** 这将上传系统信息 + 最近的日志尾部 "
    "（可能包含对话片段）到公共粘贴服务。"
    "网关不包含完整日志 — 使用 CLI 中的 `qiqiclaw debug share` "
    "上传完整日志。\n"
    "粘贴将在 6 小时后自动删除。"
)
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "qiqiclaw/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``qiqiclaw debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.qiqiclaw/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``qiqiclaw debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"qiqiclaw debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "qiqiclaw/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----QiQiClawDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from qiqiclaw_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_qiqiclaw_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``qiqiclaw dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``qiqiclaw dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if not local_only:
        print("Privacy notice: this uploads debug data to a public paste service.")
    ...
    try:
        ...
        print("\nFull report is printed below — please copy/paste manually:\n")
        print(report)
        sys.exit(1)
    ...
    print(f"\n⏱  auto-delete scheduled in 6 hours.")

    # Manual delete fallback
    print(f"Delete now: qiqiclaw debug delete <url>")

    print(f"\nShare these links with the QiQiClaw team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: qiqiclaw debug delete <url> [<url> ...]")
        print("  删除由 'qiqiclaw debug share' 上传的 paste.rs 粘贴。")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``qiqiclaw debug`` call.
    ...
    # best-effort — any failure is swallowed so ``qiqiclaw debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: qiqiclaw debug <command>")
        print()
        print("命令:")
        print("  share    上传调试报告到粘贴服务并打印 URL")
        print("  delete   删除之前上传的粘贴")
        print()
        print("选项 (share):")
        print("  --lines N    包含的日志行数 (默认: 200)")
        print("  --expire N   粘贴过期天数 (默认: 7)")
        print("  --local      本地打印报告而不上传")
        print()
        print("选项 (delete):")
        print("  <url> ...    要删除的一个或多个粘贴 URL")
