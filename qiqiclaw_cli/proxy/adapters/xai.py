...
from qiqiclaw_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
from qiqiclaw_cli.proxy.adapters.base import UpstreamAdapter, UpstreamCredential
...
class XAIGrokAdapter(UpstreamAdapter):
    ...
    auth_hint = "qiqiclaw auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`qiqiclaw auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`qiqiclaw auth reset xai-oauth` or re-authenticate with "
                    "`qiqiclaw auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `qiqiclaw auth add xai-oauth --type oauth`."
            )
