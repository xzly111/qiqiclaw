"""CLI handlers for the ``qiqiclaw proxy`` subcommand."""
...
from qiqiclaw_cli.proxy.adapters import ADAPTERS, get_adapter
from qiqiclaw_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "qiqiclaw proxy requires aiohttp. Install one of:\n"
...
def cmd_proxy_start(args: Any) -> int:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"qiqiclaw login {adapter.name}")
        print(
    ...
    print(
        f"Starting QiQi Proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("QiQi Proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: qiqiclaw proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``qiqiclaw proxy <subcommand>``."""
    ...
    print(
        "qiqiclaw proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  qiqiclaw proxy start [--provider nous|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  qiqiclaw proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  qiqiclaw proxy providers\n"
