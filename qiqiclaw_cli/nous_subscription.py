...
from qiqiclaw_cli.auth import get_nous_auth_status
from qiqiclaw_cli.config import get_env_value, load_config
...
def get_nous_subscription_features(
    ...
    # Tool Gateway via `qiqiclaw model`, so direct credentials should NOT
...
def prompt_enable_tool_gateway(config: Dict[str, object]) -> set[str]:
    ...
    try:
        from qiqiclaw_cli.setup import prompt_choice
    except Exception:
        return set()
    ...
    if already_managed:
        for k in already_managed:
            desc_parts.append(f"  ✓ {_GATEWAY_TOOL_LABELS[k]} — 正在使用工具网关")
    if unconfigured:
        for k in unconfigured:
            desc_parts.append(f"  ○ {_GATEWAY_TOOL_LABELS[k]} — 未配置")
    if has_direct:
        for k in has_direct:
            desc_parts.append(f"  ○ {_GATEWAY_TOOL_LABELS[k]} — 正在使用 {_GATEWAY_DIRECT_LABELS[k]}")
    ...
    if unconfigured and has_direct:
        choices.append("为所有工具启用（保留现有密钥，但不使用）")
        choice_keys.append("all")

        choices.append("仅为没有现有密钥的工具启用")
        ...
        choices.append("启用工具网关（保留现有密钥，但不使用）")
        choice_keys.append("all")
    ...
    if changed:
        from qiqiclaw_cli.config import save_config
        save_config(config)
