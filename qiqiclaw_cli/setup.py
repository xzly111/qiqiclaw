"""
Interactive setup wizard for QiQiClaw.
...
from qiqiclaw_cli.nous_subscription import get_nous_subscription_features
from tools.tool_backend_helpers import managed_nous_tools_enabled
from utils import base_url_hostname
from qiqiclaw_constants import get_optional_skills_dir, get_project_root

logger = logging.getLogger(__name__)

PROJECT_ROOT = get_project_root()

_DOCS_BASE = "https://qiqiclaw.nousresearch.com/docs"
...
# display_qiqiclaw_home imported lazily at call sites (stale-module safety during qiqiclaw update)
...
from qiqiclaw_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("Q QiQiClaw 设置 — 非交互模式", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure QiQiClaw using environment variables or config commands:")
    print_info("  qiqiclaw config set model.provider custom")
    print_info("  qiqiclaw config set model.base_url http://localhost:8080/v1")
    print_info("  qiqiclaw config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'qiqiclaw setup' in an interactive terminal to use the full wizard.")
    print()
...
def prompt(question: str, default: str = None, password: bool = False) -> str:
    ...
    try:
        # API key input: use visible input() so users can see what they type
        # and paste from clipboard. getpass() disables echo and blocks paste
        # in some terminals (Windows Terminal / WSL / conhost).
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'qiqiclaw setup')")


def _print_setup_summary(config: dict, qiqiclaw_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'qiqiclaw setup' to configure"))
    ...
    if subscription_features.image_gen.managed_by_nous:
        ...
        try:
            ...
            from qiqiclaw_cli.plugins import _ensure_plugins_discovered
    ...
    if subscription_features.tts.managed_by_nous:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'qiqiclaw setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'qiqiclaw setup tts'"))
    else:
    ...
    if subscription_features.modal.managed_by_nous:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'qiqiclaw setup terminal'"))
    ...
    # Spotify (OAuth via qiqiclaw auth spotify — check auth.json, not env vars)
    try:
        from qiqiclaw_cli.auth import get_provider_auth_state
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'qiqiclaw setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {qiqiclaw_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('qiqiclaw setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('qiqiclaw setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('qiqiclaw setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('qiqiclaw setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('qiqiclaw setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('qiqiclaw config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('qiqiclaw config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('qiqiclaw config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('qiqiclaw', Colors.GREEN)}              Start chatting")
    print(f"   {color('qiqiclaw gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('qiqiclaw doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `qiqiclaw tools` and `qiqiclaw setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``qiqiclaw model``)
    ...
    provider added to ``qiqiclaw model`` is automatically available here.
    ...
    # Delegate to the shared qiqiclaw model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from qiqiclaw_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: qiqiclaw model")
    ...
    if not quick and _supports_same_provider_pool_setup(selected_provider):
        try:
            ...
            from qiqiclaw_cli.auth_commands import auth_add_command
            ...
            print_info(
                "QiQi can keep multiple credentials for one provider and rotate between"
            )
            print_info(
    ...
    if _vision_needs_setup:
        ...
        print_info("doesn't currently provide one QiQiClaw can auto-use for vision,")
        ...
        if _vision_idx == 0:  # OpenRouter
            ...
            print_info("Skipped — add later with 'qiqiclaw setup' or configure AUXILIARY_VISION_* settings")
...
def _install_kittentts_deps() -> bool:
    ...
    try:
        ...
        print_error(f"安装失败 kittentts: {e}")
...
def _setup_tts_provider(config: dict):
    ...
    if selected == "nous-openai":
        ...
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.qiqiclaw/.env."
            )
        ...
        if not existing:
            ...
            if api_key:
                ...
                print_warning(
                    ...
                    f"qiqiclaw setup model or {_dhh()}/.env to use xAI TTS. "
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'qiqiclaw setup tts')."""
    _setup_tts_provider(config)
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where QiQiClaw runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        print_info("  When using QiQiClaw via Telegram/Discord, this is where")
        print_info(
        ...
        print_info("Requires the optional SDK: pip install 'qiqiclaw[vercel]'")
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                print_warning("Install failed — run manually: pip install 'qiqiclaw[vercel]'")
                if result.stderr:
                    print_info(f"  Error: {result.stderr.strip().splitlines()[-1]}")
...
def _apply_default_agent_settings(config: dict):
    ...
    save_env_value("QIQICLAW_MAX_ITERATIONS", "90")
    ...
    print_info("  Run `qiqiclaw setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    # ── Max Iterations ──
    current_max = get_env_value("QIQICLAW_MAX_ITERATIONS") or str(
    ...
    try:
        ...
        if max_iter > 0:
            save_env_value("QIQICLAW_MAX_ITERATIONS", str(max_iter))
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where QiQiClaw delivers cron job results,")
...
def _setup_slack():
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `qiqiclaw update`)",
                True,
            ):
    ...
    print()
    print_info("   Full guide: https://qiqiclaw.nousresearch.com/docs/user-guide/messaging/slack/")
    print()
...
def _write_slack_manifest_and_instruct():
    """Generate the Slack manifest, write it under QIQICLAW_HOME, and print
    ...
    """
    try:
        from qiqiclaw_cli.slack_cli import _build_full_manifest
        from qiqiclaw_constants import get_qiqiclaw_home

        manifest = _build_full_manifest(
            bot_name="QiQiClaw",
            bot_description="Your QiQiClaw agent on Slack",
        )
        ...
        print_info(
            "   Re-run `qiqiclaw slack manifest --write` anytime to refresh after "
            "QiQi adds new commands."
        )
...
def _setup_matrix():
    ...
    if token or get_env_value("MATRIX_PASSWORD"):
        ...
        print()
        print_info("📬 Home Room: where QiQiClaw delivers cron job results and notifications.")
...
def _setup_mattermost():
    ...
    print()
    print_info("📬 Home Channel: where QiQiClaw delivers cron job results and notifications.")
    print_info("   To get a channel ID: click channel name → View Info → copy the ID")
    print_info("   You can also set this later by typing /set-home in a Mattermost channel.")
    home_channel = prompt("Home channel ID (leave empty to set later with /set-home)")
    if home_channel:
        save_env_value("MATTERMOST_HOME_CHANNEL", home_channel)
    print_info("   Open config in your editor:  qiqiclaw config edit")
...
def _setup_bluebubbles():
    ...
    print_info("Connects QiQiClaw to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from qiqiclaw_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: https://qiqiclaw.nousresearch.com/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   https://qiqiclaw.nousresearch.com/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  qiqiclaw config edit")
    print_info("   Open config in your editor:  qiqiclaw config edit")
...
def setup_gateway(config: dict):
    ...
    from qiqiclaw_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with QiQiClaw from anywhere.")
    ...
    # Build checklist, pre-selecting already-configured platforms. Always show
    # this menu so users can add or reconfigure gateway platforms from the
    # setup wizard, matching the standalone gateway setup flow.
    has_configured_platform = any(
        _is_progress(_platform_status(p)) for p in platforms
    )
    items = []
    pre_selected = []
    ...
    selected = prompt_checklist(
        "Select messaging platforms to configure:",
        items,
        pre_selected,
    )

    if not selected:
        if not has_configured_platform and prompt_yes_no(
            "  No platform was selected. Choose one platform to configure now?",
            True,
        ):
            single_items = [
            ...
            single_items.append("Skip messaging setup")
            choice = prompt_choice(
                "Select a messaging platform to configure:",
                single_items,
                0,
            )
    ...
    if not selected:
        print_info("No platforms selected. Run 'qiqiclaw setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     qiqiclaw config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_qiqiclaw_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                print_info("  Or run in foreground:  qiqiclaw gateway")
        else:
            from qiqiclaw_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   qiqiclaw gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   qiqiclaw gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `qiqiclaw setup tools` and `qiqiclaw tools` use the same flow:
...
def _model_section_has_credentials(config: dict) -> bool:
    ...
    Sources of truth:
      * ``PROVIDER_REGISTRY`` in ``qiqiclaw_cli.auth`` — lists every supported
    ...
    """
    try:
        from qiqiclaw_cli.auth import get_active_provider
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from qiqiclaw_cli.config
    ...
    if section_key == "model":
        ...
        from qiqiclaw_cli.gateway import _all_platforms, _platform_status
        ...
        configured = [
            ...
            if "_registry_entry" not in plat
            if plat.get("token_var") and get_env_value(plat["token_var"])
...
def _load_openclaw_migration_module():
    """Load the openclaw_to_qiqiclaw migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_qiqiclaw", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between OpenClaw and QiQiClaw.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure QiQiClaw to use your OpenClaw messaging channels",
    "telegram": "⚠ Telegram — this will point QiQiClaw at your OpenClaw Telegram bot",
    "slack": "⚠ Slack — this will point QiQiClaw at your OpenClaw Slack workspace",
    "discord": "⚠ Discord — this will point QiQiClaw at your OpenClaw Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point QiQiClaw at your OpenClaw WhatsApp connection",
    "config": "⚠ Config values — OpenClaw settings may not map 1:1 to QiQiClaw equivalents",
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing QiQiClaw config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: OpenClaw config values may have different semantics in QiQiClaw.", Colors.YELLOW))
        print(color("  For example, OpenClaw's tool_call_execution: \"auto\" ≠ QiQiClaw's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from OpenClaw may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(qiqiclaw_home: Path) -> bool:
    ...
    print_info("QiQi can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: qiqiclaw claw migrate --dry-run"
        )
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: qiqiclaw claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing QiQiClaw configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing QiQiClaw config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
    ...
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in QiQiClaw (use qiqiclaw claw migrate --overwrite to force).")
    if skipped:
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      qiqiclaw setup           — full or quick (auto-detected)
      qiqiclaw setup model     — just model/provider
      qiqiclaw setup tts       — just text-to-speech
      qiqiclaw setup terminal  — just terminal backend
      qiqiclaw setup gateway   — just messaging platforms
      qiqiclaw setup tools     — just tool configuration
      qiqiclaw setup agent     — just agent settings
    """
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     Q QiQiClaw Setup — {label:<29s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             Q QiQiClaw Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your QiQiClaw installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, qiqiclaw_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have QiQiClaw configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'qiqiclaw setup model|terminal|")
        ...
        migration_ran = _offer_openclaw_migration(qiqiclaw_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice("How would you like to set up QiQiClaw?", [
            "Quick setup — provider, model & messaging (recommended)",
            "Full setup — configure everything",
        ], 0)

        if setup_mode == 0:
            _run_first_time_quick_setup(config, qiqiclaw_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {qiqiclaw_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'qiqiclaw config edit'")
    ...
    save_config(config)
    _print_setup_summary(config, qiqiclaw_home)
...
def _offer_launch_chat():
    ...
    print()
    if not prompt_yes_no("Launch qiqiclaw chat now?", True):
        return
...
def _run_first_time_quick_setup(config: dict, qiqiclaw_home, is_existing: bool):
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'qiqiclaw setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    qiqiclaw setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  qiqiclaw setup gateway")
    print()
...
def _run_quick_setup(config: dict, qiqiclaw_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'qiqiclaw setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect QiQiClaw to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'qiqiclaw setup gateway'.")
