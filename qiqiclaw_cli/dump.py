"""
Dump command for QiQiClaw CLI.

Outputs a compact, plain-text summary of the user's QiQiClaw setup
...
from qiqiclaw_cli.config import get_qiqiclaw_home, get_env_path, get_project_root, load_config
...
def _redact(value: str) -> str:
    ...
    ``qiqiclaw dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from qiqiclaw_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(qiqiclaw_home: Path) -> int:
    """Count installed skills."""
    skills_dir = qiqiclaw_home / "skills"
...
def _cron_summary(qiqiclaw_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = qiqiclaw_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"qiqiclaw_home:      {display_qiqiclaw_home()}")
    ...
    toolsets = config.get("toolsets", ["qiqiclaw-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(qiqiclaw_home)}")
    lines.append(f"  skills:             {_count_skills(qiqiclaw_home)}")
