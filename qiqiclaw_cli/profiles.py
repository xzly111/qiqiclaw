"""
Profile management for multiple isolated QiQiClaw instances.

Each profile is a fully independent QIQICLAW_HOME directory with its own
config.yaml, .env, memory, sessions, skills, gateway, cron, and logs.
Profiles live under ``~/.qiqiclaw/profiles/<name>/`` by default.

The "default" profile is ``~/.qiqiclaw`` itself — backward compatible,
zero migration needed.

Usage::

    qiqiclaw profile create coder          # fresh profile + bundled skills
    qiqiclaw profile create coder --clone  # also copy config, .env, SOUL.md, skills
    qiqiclaw profile create coder --clone-all  # full copy of source profile
    coder chat                           # use via wrapper alias
    qiqiclaw -p coder chat                 # or via flag
    qiqiclaw profile use coder             # set as sticky default
    qiqiclaw profile delete coder          # remove profile + alias + service
"""
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    ``~/.qiqiclaw`` contains ``profiles/<name>/`` for sibling named profiles.
...
# Directories/files to exclude when exporting the default (~/.qiqiclaw) profile.
# The default profile contains infrastructure (repo checkout, worktrees, DBs,
# caches, binaries) that named profiles don't have.  We exclude those so the
# export is a portable, reasonable-size archive of actual profile data.
_DEFAULT_EXPORT_EXCLUDE_ROOT = frozenset({
    # Infrastructure
    "qiqiclaw",         # repo checkout (multi-GB)
    ...
    "errors.log",
    ".qiqiclaw_history", ".hermes_history",
...
_RESERVED_NAMES = frozenset({
    "qiqiclaw", "default", "test", "tmp", "root", "sudo",
})

# QiQi subcommands that cannot be used as profile names/aliases
...
def _get_profiles_root() -> Path:
    ...
    Anchored to the qiqiclaw root, NOT to the current QIQICLAW_HOME
    (which may itself be a profile).  This ensures ``coder profile list``
    can see all profiles.

    In Docker/custom deployments where QIQICLAW_HOME points outside
    ``~/.qiqiclaw``, profiles live under ``QIQICLAW_HOME/profiles/`` so
    they persist on the mounted volume.
    """
    return _get_default_qiqiclaw_home() / "profiles"


def _get_default_qiqiclaw_home() -> Path:
    """Return the default (pre-profile) QIQICLAW_HOME path.

    In standard deployments this is ``~/.qiqiclaw``.
    In Docker/custom deployments where QIQICLAW_HOME is outside ``~/.qiqiclaw``
    (e.g. ``/opt/data``), returns QIQICLAW_HOME directly.
    """
    from qiqiclaw_constants import get_default_qiqiclaw_root
    return get_default_qiqiclaw_root()


def _get_active_profile_path() -> Path:
    """Return the path to the sticky active_profile file."""
    return _get_default_qiqiclaw_home() / "active_profile"
...
def validate_profile_name(name: str) -> None:
    ...
    if name == "default":
        return  # special alias for ~/.qiqiclaw
...
def get_profile_dir(name: str) -> Path:
    """Resolve a profile name to its QIQICLAW_HOME directory."""
    if name == "default":
        return _get_default_qiqiclaw_home()
...
def check_alias_collision(name: str) -> Optional[str]:
    ...
    Checks: reserved names, qiqiclaw subcommands, existing binaries in PATH.
    """
    if name in _RESERVED_NAMES:
        return f"'{name}' is a reserved name"
    if name in _HERMES_SUBCOMMANDS:
        return f"'{name}' conflicts with a qiqiclaw subcommand"
...
def list_profiles() -> List[ProfileInfo]:
    ...
    # Default profile
    default_home = _get_default_qiqiclaw_home()
...
def create_profile(
    ...
    if name == "default":
        raise ValueError(
            "Cannot create a profile named 'default' — it is the built-in profile (~/.qiqiclaw)."
        )
    ...
    if clone_all and source_dir:
        # Full copy of source profile (exclude sibling ~/.qiqiclaw/profiles/)
        shutil.copytree(
            source_dir,
            profile_dir,
...
def seed_profile_skills(profile_dir: Path, quiet: bool = False) -> Optional[dict]:
    ...
    Uses subprocess because sync_skills() caches QIQICLAW_HOME at module level.
    ...
    try:
        result = subprocess.run(
            ...
            env={**os.environ, "QIQICLAW_HOME": str(profile_dir)},
        ...
        if not quiet:
            print(f"⚠ 技能初始化返回退出代码 {result.returncode}")
...
def delete_profile(name: str, yes: bool = False) -> Path:
    ...
    if name == "default":
        raise ValueError(
            "Cannot delete the default profile (~/.qiqiclaw).\n"
            "To remove everything, use: qiqiclaw uninstall"
        )
    ...
    if has_wrapper:
        items.append(f"命令别名 ({wrapper_path})")
    ...
    if not yes:
        print()
        try:
            confirm = input(f"输入 '{name}' 以确认: ").strip()
    ...
    if has_wrapper:
        if remove_wrapper_script(name):
            print(f"✓ 已删除 {wrapper_path}")

    # 4. Remove profile directory
    try:
        shutil.rmtree(profile_dir)
        print(f"✓ 已删除 {profile_dir}")
    except Exception as e:
        print(f"⚠ 无法删除 {profile_dir}: {e}")
...
def _cleanup_gateway_service(name: str, profile_dir: Path) -> None:
    ...
    # Temporarily set QIQICLAW_HOME so _profile_suffix resolves correctly
    old_home = os.environ.get("QIQICLAW_HOME")
    try:
        os.environ["QIQICLAW_HOME"] = str(profile_dir)
        from qiqiclaw_cli.gateway import get_service_name, get_launchd_plist_path
        ...
        if _platform.system() == "Linux":
            ...
            if plist_path.exists():
                ...
                print(f"✓ Launchd 服务已删除")
    except Exception as e:
        print(f"⚠ 服务清理: {e}")
    finally:
        if old_home is not None:
            os.environ["QIQICLAW_HOME"] = old_home
        elif "QIQICLAW_HOME" in os.environ:
            del os.environ["QIQICLAW_HOME"]
...
def set_active_profile(name: str) -> None:
    ...
    Writes to ``~/.qiqiclaw/active_profile``. Use ``"default"`` to clear.
    """
    validate_profile_name(name)
    if name != "default" and not profile_exists(name):
        raise FileNotFoundError(
            f"Profile '{name}' does not exist. "
            f"Create it with: qiqiclaw profile create {name}"
        )
...
def get_active_profile_name() -> str:
    """Infer the current profile name from QIQICLAW_HOME.

    Returns ``"default"`` if QIQICLAW_HOME is not set or points to ``~/.qiqiclaw``.
    Returns the profile name if QIQICLAW_HOME points into ``~/.qiqiclaw/profiles/<name>``.
    Returns ``"custom"`` if QIQICLAW_HOME is set to an unrecognized path.
    """
    from qiqiclaw_constants import get_qiqiclaw_home
    qiqiclaw_home = get_qiqiclaw_home()
    resolved = qiqiclaw_home.resolve()

    default_resolved = _get_default_qiqiclaw_home().resolve()
...
def export_profile(name: str, output_path: str) -> Path:
    ...
    if name == "default":
        # The default profile IS ~/.qiqiclaw itself — its parent is ~/ and its
        # directory name is ".qiqiclaw", not "default".  We stage a clean copy
...
def import_profile(archive_path: str, name: Optional[str] = None) -> Path:
    ...
    if not inferred_name:
        raise ValueError(
            ...
            "Specify it explicitly: qiqiclaw profile import <archive> --name <name>"
        )
    ...
    # dir.  Importing as "default" would target ~/.qiqiclaw itself — disallow
    # that and guide the user toward a named profile.
    if inferred_name == "default":
        raise ValueError(
            "Cannot import as 'default' — that is the built-in root profile (~/.qiqiclaw). "
            "Specify a different name: qiqiclaw profile import <archive> --name <name>"
        )
...
def _migrate_honcho_profile_host(old_name: str, new_name: str, new_dir: Path) -> None:
    ...
    old_host = f"qiqiclaw.{old_name}"
    new_host = f"qiqiclaw.{new_name}"

    candidates = [
        new_dir / "honcho.json",
        _get_default_qiqiclaw_home() / "honcho.json",
    ...
    for path in candidates:
        ...
        if new_host in hosts:
            print(f"⚠ Honcho 主机块未迁移: {new_host} 已存在于 {path}")
            continue
        ...
        print(f"✓ Honcho 主机已更新: {old_host} → {new_host}")
...
def generate_bash_completion() -> str:
    """Generate a bash completion script for qiqiclaw profile names."""
    return '''# QiQiClaw profile completion
# Add to ~/.bashrc: eval "$(qiqiclaw completion bash)"
...
def generate_zsh_completion() -> str:
    """Generate a zsh completion script for qiqiclaw profile names."""
    return '''#compdef qiqiclaw
# QiQiClaw profile completion
# Add to ~/.zshrc: eval "$(qiqiclaw completion zsh)"
...
def resolve_profile_env(profile_name: str) -> str:
    """Resolve a profile name to a QIQICLAW_HOME path string.

    Called early in the CLI entry point, before any qiqiclaw modules
    are imported, to set the QIQICLAW_HOME environment variable.
    """
    ...
    if profile_name != "default" and not profile_dir.is_dir():
        raise FileNotFoundError(
            ...
            f"Create it with: qiqiclaw profile create {profile_name}"
        )
