"""
QiQiClaw Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with qiqiclaw;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.qiqiclaw/plugins/<name>/``
3. **Project plugins** – ``./.qiqiclaw/plugins/<name>/`` (opt-in via
   ``QIQICLAW_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``qiqiclaw.plugins``
   entry-point group.
...
def get_bundled_plugins_dir() -> Path:
    ...
    """
    env_override = _legacy_env("BUNDLED_PLUGINS")
    if env_override:
...
ENTRY_POINTS_GROUP = "qiqiclaw.plugins"
...
class PluginManifest:
    ...
    #              in ~/.qiqiclaw/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``qiqiclaw plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``qiqiclaw honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``qiqiclaw <subcommand>``
    ...
    def register_skill(
        ...
        ``~/.qiqiclaw/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        #
        # Repo-shipped plugins live next to qiqiclaw_cli/. Two layouts are
        ...
        # 2. User plugins (~/.qiqiclaw/plugins/)
        user_dir = get_qiqiclaw_home() / "plugins"
        manifests.extend(self._scan_directory(user_dir, source="user"))

        # 3. Project plugins (./.qiqiclaw/plugins/)
        if _env_enabled("QIQICLAW_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".qiqiclaw" / "plugins"
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with qiqiclaw and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform QiQiClaw ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `qiqiclaw plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="qiqiclaw-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``qiqiclaw tools`` TUI so plugin-provided toolsets appear
