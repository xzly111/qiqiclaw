"""
Skills configuration for QiQiClaw.
`qiqiclaw skills` enters this module.

Toggle individual skills or categories on/off, globally or per-platform.
Config stored in ~/.qiqiclaw/config.yaml under:
...
from qiqiclaw_cli.config import cfg_get, load_config, save_config
from qiqiclaw_cli.colors import Colors, color
from qiqiclaw_cli.platforms import PLATFORMS as _PLATFORMS
...
def skills_command(args=None):
    """Entry point for `qiqiclaw skills`."""
    from qiqiclaw_cli.curses_ui import curses_checklist
    ...
    print()
    print(color(f"  配置对象：{platform_label}", Colors.DIM))
    print()
    print("  1. 切换单个技能")
    print("  2. 按类别切换")
    print()
