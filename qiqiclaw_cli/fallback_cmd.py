"""
qiqiclaw fallback — manage the fallback provider chain.
...
Subcommands:
  qiqiclaw fallback [list]   Show the current fallback chain (default when no subcommand)
  qiqiclaw fallback add      Pick provider + model via the same picker as `qiqiclaw model`,
                           then append the selection to the chain
  qiqiclaw fallback remove   Pick an entry to delete from the chain
  qiqiclaw fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.qiqiclaw/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from qiqiclaw_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `qiqiclaw model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one:  qiqiclaw fallback add")
        print()
        return
    ...
    print()
    print("  Tried in order when the primary provider fails (rate limits, 5xx, connection errors).")
    print("  Docs: https://qiqiclaw.nousresearch.com/docs/user-guide/features/fallback-providers")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `qiqiclaw model`, then append the selection to the chain."""
    from qiqiclaw_cli.main import _require_tty, select_provider_and_model
    ...
    print()
    print("  添加备用提供商。下面的选择器与")
    print("  `qiqiclaw model` 使用的相同 — 选择您想要作为备用的提供商 + 模型。")
    print()
    ...
    print(f"  Chain now has {len(chain)} {'entry' if len(chain) == 1 else 'entries'}.")
    print()
    print("  运行 `qiqiclaw fallback list` 查看，或 `qiqiclaw fallback remove` 删除。")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``qiqiclaw fallback [subcommand]``."""
