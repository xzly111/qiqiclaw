"""``qiqiclaw logs`` — view and filter QiQiClaw log files.
...
Usage examples::
    ...
    qiqiclaw logs                    # last 50 lines of agent.log
    qiqiclaw logs -f                 # follow agent.log in real time
    qiqiclaw logs errors             # last 50 lines of errors.log
    qiqiclaw logs gateway -n 100    # last 100 lines of gateway.log
    qiqiclaw logs --level WARNING    # only WARNING+ lines
    qiqiclaw logs --session abc123   # filter by session ID substring
    qiqiclaw logs --component tools  # only tool-related lines
    qiqiclaw logs --since 1h         # lines from the last hour
    qiqiclaw logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    if filename is None:
        print(f"未知的日志: {log_name!r}。可用的日志: {', '.join(sorted(LOG_FILES))}")
        sys.exit(1)

    log_path = get_qiqiclaw_home() / "logs" / filename
    if not log_path.exists():
        print(f"未找到日志文件: {log_path}")
        print(f"(日志在 QiQiClaw 运行时创建 — 请先尝试 'qiqiclaw chat')")
        sys.exit(1)
    ...
    if since:
        ...
        if since_dt is None:
            print(f"无效的 --since 值: {since!r}。请使用类似 '1h'、'30m'、'2d' 的格式。")
            sys.exit(1)
    ...
    if component:
        ...
        if component_lower not in COMPONENT_PREFIXES:
            ...
            print(f"未知的组件: {component!r}。可用的组件: {available}")
            sys.exit(1)
    ...
    if follow:
        print(f"--- {display_qiqiclaw_home()}/logs/{filename}{filter_desc} (按 Ctrl+C 停止) ---")
    else:
        print(f"--- {display_qiqiclaw_home()}/logs/{filename}{filter_desc} (最后 {num_lines} 行) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"在 {display_qiqiclaw_home()}/logs/ 没有日志目录")
        return

    print(f"{display_qiqiclaw_home()}/logs/ 中的日志文件:\n")
    found = False
    ...
    if not found:
        print("  (还没有日志文件 — 运行 'qiqiclaw chat' 来生成日志)")
