"""``qiqiclaw plugins`` CLI subcommand — install, update, remove, and list plugins.

Plugins are installed from Git repositories into ``~/.qiqiclaw/plugins/``.
...
from qiqiclaw_cli.config import cfg_get
...
def _plugins_dir() -> Path:
    ...
    plugins = get_qiqiclaw_home() / "plugins"
    ...
    for example_file in plugin_dir.glob("*.example"):
        ...
        if not real_path.exists():
            try:
                ...
                console.print(
                    f"[dim]  从 {example_file.name} 创建了 {real_name}[/dim]"
                )
            except OSError as e:
                console.print(
                    f"[yellow]Warning:[/yellow] 复制 {example_file.name} 失败: {e}"
                )
...
def _prompt_plugin_env_vars(manifest: dict, console) -> None:
    ...
    from qiqiclaw_cli.config import get_env_value, save_env_value  # noqa: F811
    ...
    for spec in missing:
        ...
        try:
            ...
            console.print(f"\n[dim]  已跳过 (您可以稍后在 {display_qiqiclaw_home()}/.env 中设置这些变量)[/dim]")
            return

        if value:
            save_env_value(name, value)
            os.environ[name] = value
            console.print(f"  [green]✓[/green] 已保存到 {display_qiqiclaw_home()}/.env")
        else:
            console.print(f"  [dim]  已跳过 (稍后在 {display_qiqiclaw_home()}/.env 中设置 {name})[/dim]")
...
def cmd_install(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            ...
            console.print("[red]错误:[/red] git 未安装或不在 PATH 中。")
            sys.exit(1)
        ...
        if mv is not None:
            ...
            if mv_int > _SUPPORTED_MANIFEST_VERSION:
                ...
                console.print(
                    ...
                    f"运行 [bold]{recommended_update_command()}[/bold] 以获取更新的安装程序。"
                )
                sys.exit(1)
        ...
        if target.exists():
            if not force:
                console.print(
                    ...
                    f"使用 [bold]--force[/bold] 移除并重新安装，或使用 "
                    f"[bold]qiqiclaw plugins update {plugin_name}[/bold] 拉取最新版本。"
                )
                sys.exit(1)
    ...
    if not (target / "plugin.yaml").exists() and not (target / "__init__.py").exists():
        console.print(
            ...
            f"或 __init__.py。它可能不是有效的 QiQiClaw 插件。"
        )
    ...
    if should_enable:
        ...
        console.print(
            f"[green]✓[/green] 插件 [bold]{installed_name}[/bold] 已启用。"
        )
    else:
        console.print(
            f"[dim]插件已安装但未启用。"
            f"运行 `qiqiclaw plugins enable {installed_name}` 以激活。[/dim]"
        )

    console.print("[dim]重启网关以使插件生效:[/dim]")
    console.print("[dim]  qiqiclaw gateway restart[/dim]")
    console.print()
...
def cmd_update(name: str) -> None:
    ...
    if not (target / ".git").exists():
        console.print(
            f"[red]错误:[/red] 插件 '{name}' 不是从 git 安装的 "
            f"(没有 .git 目录)。无法更新。"
        )
        sys.exit(1)
    ...
    try:
        ...
        console.print("[red]错误:[/red] Git pull 在 60 秒后超时。")
        sys.exit(1)

    if result.returncode != 0:
        console.print(f"[red]错误:[/red] Git pull 失败:\n{result.stderr.strip()}")
        sys.exit(1)
...
def cmd_enable(name: str) -> None:
    ...
    _save_disabled_set(disabled)
    console.print(
        f"[green]✓[/green] 插件 [bold]{name}[/bold] 已启用。"
        "将在下次会话中生效。"
    )
...
def cmd_disable(name: str) -> None:
    ...
    _save_disabled_set(disabled)
    console.print(
        f"[green]✓[/green] 插件 [bold]{name}[/bold] 已禁用。"
        "将在下次会话中生效。"
    )
...
def cmd_list() -> None:
    ...
    if not entries:
        ...
        console.print("[dim]Install with:[/dim] qiqiclaw plugins install owner/repo")
        return
    ...
    console.print(table)
    console.print()
    console.print("[dim]Interactive toggle:[/dim] qiqiclaw plugins")
    console.print("[dim]Enable/disable:[/dim] qiqiclaw plugins enable/disable <name>")
...
def plugins_command(args) -> None:
    """Dispatch qiqiclaw plugins subcommands."""
