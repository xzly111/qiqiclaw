"""Slash command definitions and autocomplete for the QiQiClaw CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    # Session
    CommandDef("new", "开始新会话（全新的会话 ID 和历史记录）", "Session",
               aliases=("reset",)),
    CommandDef("clear", "清屏并开始新会话", "Session",
               cli_only=True),
    CommandDef("redraw", "强制完全重绘界面（从终端漂移中恢复）", "Session",
               cli_only=True),
    CommandDef("history", "显示对话历史记录", "Session",
               cli_only=True),
    CommandDef("save", "保存当前对话到文件", "Session",
               cli_only=True),
    CommandDef("retry", "重试上一条消息（重新发送给代理）", "Session"),
    CommandDef("undo", "删除最后一次用户/助手交互", "Session"),
    ...
    CommandDef("rollback", "列出或恢复文件系统检查点", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "创建或恢复 QiQiClaw 配置/状态的快照", "Session",
    ...
    CommandDef("agents", "显示活动代理和正在运行的任务", "Session",
               aliases=("tasks",)),
    CommandDef("queue", "将提示排队到下一轮（不中断）", "Session",
               aliases=("q",), args_hint="<prompt>"),
    CommandDef("steer", "在下一次工具调用后注入消息而不中断", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "设置 QiQiClaw 跨轮次工作的长期目标，直到实现", "Session",
    ...
    CommandDef("resume", "恢复先前命名的会话", "Session",
               args_hint="[name]"),

    # Configuration
    CommandDef("config", "显示当前配置", "Configuration",
               cli_only=True),
    CommandDef("model", "切换此会话的模型", "Configuration",
               aliases=("provider",), args_hint="[model] [--provider name] [--global]"),
    CommandDef("gquota", "显示 Google Gemini Code Assist 配额使用情况", "Info",
               cli_only=True),

    CommandDef("personality", "设置预定义的个性", "Configuration",
               args_hint="[name]"),
    ...
    CommandDef("busy", "控制 QiQiClaw 工作时 Enter 键的行为", "Configuration",
    ...
    CommandDef("toolsets", "列出可用的工具集", "Tools & Skills",
               cli_only=True),
    CommandDef("task", "显示任务面板（新建/更新/列出/详情）",
               "Tools & Skills", cli_only=True,
               args_hint="[list|show|add|done|undo|del <id> | <id>]",
               subcommands=("list", "show", "add", "done", "undo", "del")),
    CommandDef("runs", "查看长任务的后台运行记录",
               "Tools & Skills", cli_only=True,
               aliases=("run",),
               args_hint="[list|tail <id>|kill <id>|cleanup]",
               subcommands=("list", "tail", "kill", "cleanup")),
    ...
    CommandDef("reload", "将 .env 变量重新加载到运行会话中", "Tools & Skills",
               cli_only=True),
    CommandDef("reload-mcp", "从配置重新加载 MCP 服务器", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "重新扫描 ~/.qiqiclaw/skills/ 以查找新安装或删除的技能",
    ...
    CommandDef("restart", "在排空活动运行后优雅地重启网关", "Session",
               gateway_only=True),
    CommandDef("usage", "显示当前会话的令牌使用情况和速率限制", "Info"),
    ...
    CommandDef("paste", "从剪贴板附加剪贴板图像", "Info",
               cli_only=True),
    CommandDef("image", "为下一个提示附加本地图像文件", "Info",
               cli_only=True, args_hint="<path>"),
    CommandDef("update", "将 QiQiClaw 更新到最新版本", "Info",
               gateway_only=True),
    CommandDef("debug", "上传调试报告（系统信息 + 日志）并获取可共享链接", "Info"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`qiqiclaw_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/qiqiclaw`` subcommand mapping, and
    ...
    """
    try:
        from qiqiclaw_cli.plugins import get_plugin_commands
    except Exception:
        return []
    try:
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``qiqiclaw skills
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/qiqiclaw <verb>`` subcommand.
    ...
    avoidance. ``/qiqiclaw`` is always reserved as the first entry so the
    legacy ``/qiqiclaw <subcommand>`` form keeps working for anything that
    ...
    # Reserve /qiqiclaw as the catch-all top-level command.
    entries.append(("qiqiclaw", "Talk to QiQiClaw or run a subcommand", "[subcommand] [args]"))
    seen.add("qiqiclaw")
...
def slack_app_manifest(request_url: str = "https://qiqiclaw.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /qiqiclaw handler.

    Maps both canonical names and aliases so /qiqiclaw bg do stuff works
    the same as /qiqiclaw background do stuff.

    Plugin-registered slash commands are included so ``/qiqiclaw <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from qiqiclaw_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
    ...
    def _model_completions(self, sub_text: str, sub_lower: str):
        ...
        try:
            from qiqiclaw_cli.model_switch import (
