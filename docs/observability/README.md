# QIQI-Claw Observer Hooks

QIQI-Claw observer hooks are the read-only telemetry contract for plugins that
...
They do not replace QIQI-Claw's planner, model providers, memory, tool registry,
...
Hook callbacks are fail-open. QIQI-Claw catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so QIQI-Claw only builds
