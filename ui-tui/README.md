...
React + Ink terminal UI for QiQiClaw. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
Interpreter resolution order is: `QIQICLAW_PYTHON` / `HERMES_PYTHON` → `PYTHON` → `$VIRTUAL_ENV/bin/python` → `./.venv/bin/python` → `./venv/bin/python` → `python3` (or `python` on Windows).
...
tui_gateway/
  ...
  slash_worker.py        persistent QiQiClawCLI subprocess for slash commands
```
