...
# 5. Symlinks the 'qiqiclaw' CLI command into a user-facing bin dir
...
echo ""
echo -e "${CYAN}⚕ QIQI claw Setup${NC}"
echo ""
...
# ============================================================================
# PATH setup — symlink qiqiclaw into a user-facing bin dir
# ============================================================================

echo -e "${CYAN}→${NC} Setting up qiqiclaw command..."

QIQICLAW_BIN="$SCRIPT_DIR/qiqiclaw"
COMMAND_LINK_DIR="$(get_command_link_dir)"
COMMAND_LINK_DISPLAY_DIR="$(get_command_link_display_dir)"
mkdir -p "$COMMAND_LINK_DIR"
ln -sf "$QIQICLAW_BIN" "$COMMAND_LINK_DIR/qiqiclaw"
echo -e "${GREEN}✓${NC} Symlinked qiqiclaw → $COMMAND_LINK_DISPLAY_DIR/qiqiclaw"
...
# ============================================================================
# Seed bundled skills into $QIQICLAW_HOME/skills/
# ============================================================================

QIQICLAW_SKILLS_DIR="${QIQICLAW_HOME:-$HOME/.hermes}/skills"
mkdir -p "$QIQICLAW_SKILLS_DIR"

echo ""
echo "Syncing bundled skills to $QIQICLAW_SKILLS_DIR ..."
if "$SCRIPT_DIR/venv/bin/python" "$SCRIPT_DIR/tools/skills_sync.py" 2>/dev/null; then
    echo -e "${GREEN}✓${NC} Skills synced"
else
    # Fallback: copy if sync script fails (missing deps, etc.)
    if [ -d "$SCRIPT_DIR/skills" ]; then
        cp -rn "$SCRIPT_DIR/skills/"* "$QIQICLAW_SKILLS_DIR/" 2>/dev/null || true
...
echo "Other commands:"
echo "  qiqiclaw status        # Check configuration"
if is_termux; then
    echo "  qiqiclaw gateway       # Run gateway in foreground"
else
    echo "  qiqiclaw gateway install # Install gateway service (messaging + cron)"
fi
echo "  qiqiclaw cron list     # View scheduled jobs"
echo "  qiqiclaw doctor        # Diagnose issues"
echo ""
...
if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
    ...
    "$SCRIPT_DIR/venv/bin/python" -m qiqiclaw_cli.main setup
fi

# ================================================================
# ECC + Subsystem Deployment
# ================================================================
if [ -f "$SCRIPT_DIR/ecc/deploy.sh" ]; then
    echo ""
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${CYAN}  ECC (Everything Claude Code) + Subsystem Setup${NC}"
    echo -e "${CYAN}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    read -p "Install ECC agents/skills for Claude Code & Codex? [Y/n] " -n 1 -r
    echo
    if [[ $REPLY =~ ^[Yy]$ ]] || [[ -z $REPLY ]]; then
        bash "$SCRIPT_DIR/ecc/deploy.sh"
    else
        echo -e "${YELLOW}跳过 ECC 部署。可稍后运行: bash ecc/deploy.sh${NC}"
    fi
fi
