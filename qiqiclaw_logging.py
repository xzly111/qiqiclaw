"""Centralized logging setup for QiQi claw.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.qiqiclaw/logs/`` (profile-aware via ``get_qiqiclaw_home()``).
...
from qiqiclaw_constants import get_config_path, get_qiqiclaw_home
...
# Used by _ComponentFilter and exposed for ``qiqiclaw logs --component``.
COMPONENT_PREFIXES = {
    "gateway": ("gateway",),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("qiqiclaw_cli", "cli"),
    "cron": ("cron",),
}
...
def setup_logging(
    *,
    ...
    """Configure the QiQi claw logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    qiqiclaw_home
        Override for the QiQi claw home directory.  Falls back to
        ``get_qiqiclaw_home()`` (profile-aware).
    log_level
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    inherit the qiqiclaw group. However, both _open() (initial creation) and
    ...
    def __init__(self, *args, **kwargs):
        from qiqiclaw_cli.config import is_managed
