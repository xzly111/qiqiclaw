#!/usr/bin/env python3
"""
QiQiClaw CLI - Interactive Terminal Interface

Interactive command-line interface for QiQiClaw.
Features QiQiClaw branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills qiqiclaw-agent-dev,github-auth
...
from qiqiclaw_cli.banner import _format_context_length, format_banner_version_label
...
_qiqiclaw_home = get_qiqiclaw_home()
# Compatibility aliases keep the older internal variable names working while
# all user-visible paths resolve to the QiQiClaw home.
_hermes_home = _qiqiclaw_home
get_hermes_home = get_qiqiclaw_home
display_hermes_home = display_qiqiclaw_home
_project_env = Path(__file__).parent / '.env'
load_qiqiclaw_dotenv(qiqiclaw_home=_qiqiclaw_home, project_env=_project_env)


def _cli_home_path() -> Path:
    """Return the active CLI home with legacy test/profile compatibility."""
    return Path(os.environ.get("QIQICLAW_HOME") or os.environ.get("HERMES_HOME") or _hermes_home)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.qiqiclaw/.
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.qiqiclaw/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If QIQICLAW_IGNORE_USER_CONFIG=1 is set (via ``qiqiclaw chat --ignore-user-config``),
    the user config at ``~/.qiqiclaw/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({QIQICLAW_HOME}/config.yaml).  Use the legacy
    # alias as the single mutable home hook so older tests/plugins that patch
    # ``cli._hermes_home`` still exercise the same path.
    ...
    ignore_user_config = (
        os.environ.get("QIQICLAW_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain QiQiClaw, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me QiQiClaw - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from qiqiclaw_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and QiQiClawCLI.__init__ checks "default" first.
    ...
    from qiqiclaw_cli.config import _expand_env_vars
...
# Initialize centralized logging early — agent.log + errors.log in ~/.qiqiclaw/logs/.
...
try:
    from qiqiclaw_cli.config import print_config_warnings
...
try:
    from qiqiclaw_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``qiqiclaw --help`` paths inside the chat command flow).  See
...
from qiqiclaw_cli.banner import build_welcome_banner
from qiqiclaw_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from qiqiclaw_cli.callbacks import prompt_for_secret
...
def _install_skin_light_mode_hook() -> None:
    ...
    skin_config_classes = []
    for module_name in ("qiqiclaw_cli.skin_engine", "hermes_cli.skin_engine"):
        try:
            module = __import__(module_name, fromlist=["SkinConfig"])
            skin_config_classes.append(module.SkinConfig)
        except Exception:
            continue

    for SkinConfig in skin_config_classes:
        if getattr(SkinConfig, "_hermes_light_mode_hook_installed", False):
            continue
        _orig_get_color = SkinConfig.get_color

        def _wrapped_get_color(self, key, fallback="", _orig_get_color=_orig_get_color):
...
# ASCII Art - legacy fallback logo kept for import compatibility.
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "QIQI-CLAW - Personal AI Agent"
        tiny_line = "QIQI-CLAW"
    else:
        agent_name = _skin.get_branding("agent_name", "QiQiClaw") if _skin else "QiQiClaw"
        line1 = f"{agent_name} - Personal AI Agent"
        tiny_line = agent_name

    if (
        os.environ.get("QIQICLAW_FAST_STARTUP_BANNER") == "1"
        or os.environ.get("HERMES_FAST_STARTUP_BANNER") == "1"
    ):
        from qiqiclaw_cli import __release_date__ as _release_date
        from qiqiclaw_cli import __version__ as _version

        version_line = f"QiQiClaw v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- QiQiClaw[/]\n"

    inner = w - 2  # inside the box border
    rule = "─" * min(max(w, 30), 88)
    return (
        f"\n[bold {title_color}]{tiny_line}[/] [dim {dim_color}]{version_line}[/]\n"
        f"[{title_color}]欢迎使用，属于您的专属智能体。[/]\n"
        f"[{border_color}]{rule}[/]\n"
    )
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.qiqiclaw/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.qiqiclaw/config.yaml on first use)
...
class HermesCLI:
    ...
    def __init__(
        ...
        # checkpoint repos under ~/.qiqiclaw/checkpoints/.  Opt-in via
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            if width < 52:
                text = f"Q {snapshot['model_short']} · {duration_label}"
                if yolo_active:
                    text += " · ⚠ YOLO"
                return self._trim_status_bar_text(text, width)
            if width < 76:
                parts = [f"Q {snapshot['model_short']}", percent_label]
            ...
            compressions = snapshot.get("compressions", 0)
            parts = [f"Q {snapshot['model_short']}", context_label, percent_label]
            if compressions:
                parts.append(f"🗜️ {compressions}")
            bg_count = snapshot.get("active_background_tasks", 0)
            if bg_count:
            ...
            return f"Q {self.model if getattr(self, 'model', None) else 'QiQiClaw'}"
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from qiqiclaw_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", " Q QiQiClaw ")
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                if _quiet_mode:
                    ...
                    print(
                        "Use a session ID from a previous CLI run (qiqiclaw sessions list).",
                        file=sys.stderr,
                    )
                else:
                    _cprint(f"\033[1;31mSession not found: {self.session_id}{_RST}")
                    _cprint(f"{_DIM}Use a session ID from a previous CLI run (qiqiclaw sessions list).{_RST}")
                return False
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   QIQI-Claw needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        if is_nous_hermes_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  The selected Hermes 3/4 chat model is not agentic and is not "
                "designed for QiQiClaw tool use.[/]"
            )
            self._console_print(
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ QIQI-Claw: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ QIQI-Claw: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.qiqiclaw/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from hermes_cli.clipboard import save_clipboard_image

        img_dir = Path(os.environ.get("HERMES_HOME") or get_hermes_home()) / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: qiqiclaw --checkpoints")
    ...
    def show_help(self):
        ...
        from qiqiclaw_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from qiqiclaw_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with QIQI-Claw!{_RST}")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `qiqiclaw sessions list` to find sessions.")
            return
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `qiqiclaw sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.qiqiclaw/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``qiqiclaw --resume <id>``
        ...
        saved_dir = _cli_home_path() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"qiqiclaw_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: qiqiclaw --resume {self.session_id}")
        except Exception as e:
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        model_input, explicit_provider, persist_global, force_refresh, _explicit_base_url = parse_model_flags(raw_args)
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — QIQI-Claw default (chat_completions)
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from qiqiclaw_cli.commands import resolve_command
    ...
    def process_command(self, command: str) -> bool:
        ...
        from qiqiclaw_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            if self._app:
                ...
                try:
                    from qiqiclaw_cli.tips import get_random_tip
                    _tip = get_random_tip()
                    try:
                        from qiqiclaw_cli.skin_engine import get_active_skin
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from qiqiclaw_cli.commands import COMMANDS
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"QIQI-Claw keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from qiqiclaw_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while QIQI-Claw is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while QIQI-Claw is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while QIQI-Claw is busy."
    ...
    def _handle_update_command(self) -> bool:
        """Handle /update — update QiQiClaw to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``qiqiclaw update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from qiqiclaw_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update QiQiClaw')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update QiQiClaw"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="Q  Update QiQiClaw",
            detail="This will exit the current session and run `qiqiclaw update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.qiqiclaw/skills/ and queue a note for the
        next user turn.
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  qiqiclaw --resume {self.session_id}{profile_flag}")
            if session_title:
            ...
            try:
                from qiqiclaw_cli.skin_engine import get_active_goodbye
                goodbye = get_active_goodbye("Goodbye! Q")
            except Exception:
                goodbye = "Goodbye! Q"
            print(goodbye)
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from qiqiclaw_cli.skin_engine import get_active_prompt_symbol
    ...
    def _get_tui_prompt_fragments(self):
        ...
        if self._agent_running:
            return _state_fragment("class:prompt-working", "Q")
        if self._voice_mode:
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from qiqiclaw_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from qiqiclaw_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "QiQiClaw")
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("QIQI-Claw needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'QIQI-Claw needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("QIQI-Claw needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
...
def main(
    ...
    """
    QiQiClaw CLI - Interactive AI Assistant
    ...
    if gateway:
        ...
        print("Starting QiQiClaw Gateway (messaging platforms)...")
