"""Windows UTF-8 bootstrap for QiQi Claw entry points.

Python on Windows has two long-standing text-encoding footguns:

1. ``sys.stdout`` / ``sys.stderr`` are bound to the console code page
   (``cp1252`` on US-locale installs), so ``print("cafe")`` crashes with
...
should be imported at the very top of every QiQi Claw entry point
(``qiqiclaw``, ``qiqiclaw-agent``, ``qiqiclaw-acp``, ``python -m gateway.run``,
...
What this module does on Windows:
  ...
  - Reconfigures ``sys.stdout`` / ``sys.stderr`` to UTF-8 in the current
    ...
    ``print("cafe")`` in the parent without a re-exec.
...
# Apply on import -- entry points just need ``import qiqiclaw_bootstrap``
# (or ``from qiqiclaw_bootstrap import apply_windows_utf8_bootstrap``) at
