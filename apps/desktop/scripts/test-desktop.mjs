...
const PRODUCT_NAME = PACKAGE_JSON.build?.productName || PACKAGE_JSON.productName || 'QiQiClaw Desktop'
const EXECUTABLE_NAME = PACKAGE_JSON.build?.executableName || PACKAGE_JSON.executableName || PRODUCT_NAME
...
// no longer bundles the QIQI-Claw Python payload (that's fetched at first
...
// resolveQiQiClawHome(). Defaults to ~/.qiqiclaw. The fresh-install sandbox launchFresh() sets its own
// HERMES_HOME and never touches this.
const DEFAULT_HERMES_HOME = (() => {
  return process.env.QIQICLAW_HOME || process.env.HERMES_HOME || path.join(os.homedir(), '.qiqiclaw')
})()
const VENV_ROOT = path.join(DEFAULT_HERMES_HOME, 'qiqiclaw', 'venv')
const FRESH_SANDBOX_ROOT = path.join(os.tmpdir(), 'qiqiclaw-desktop-fresh-install')
...
function resolveDmgPath() {
  if (!exists(RELEASE_ROOT)) {
    return path.join(RELEASE_ROOT, `QiQiClaw-${PACKAGE_JSON.version}-${ARCH}.dmg`)
  }

  const prefix = `QiQiClaw-${PACKAGE_JSON.version}`
  const candidates = fs
  ...
  return candidates.length > 0
    ...
    : path.join(RELEASE_ROOT, `QiQiClaw-${PACKAGE_JSON.version}-${ARCH}.dmg`)
}

function resolveNsisPath() {
  // electron-builder NSIS artifactName template is 'QiQiClaw-${version}-${os}-${arch}.${ext}'
...
function launchFresh() {
  ...
  return { runtimeRoot: path.join(hermesHome, 'qiqiclaw', 'venv') }
}

// Validate the packaged bundle matches the thin-installer architecture:
//   - The QIQI-Claw Python payload is NOT shipped (it's fetched at first
...
function validateBundle() {
  ...
  const staleFactoryMarker = path.join(APP.resourcesPath, 'qiqiclaw', 'hermes_cli', 'main.py')
...
function help() {
  console.log(`Usage:
  npm run test:desktop:existing  # build packaged app, launch with normal PATH/existing QiQiClaw
