 ...
 * product name (`QiQiClaw`). If a PREVIOUS `npm run pack` was interrupted
 ...
 *   '.../release/linux-unpacked/electron' -> '.../release/linux-unpacked/'
 *
 * This is a hard failure with no obvious cause for the user — `QiQiClaw Desktop`
 ...
 * (the mac-unpacked QiQiClaw.app bundle) and Windows (win-unpacked), so we
