...
export function useGatewayBoot({
  ...
  useEffect(() => {
    ...
    // composer stayed disabled forever on "Starting QiQiClaw...". Once the
    ...
    const attemptReconnect = async () => {
      ...
      try {
        ...
        // "Starting QiQiClaw…". The probe is a no-op for a healthy or local backend.
        ...
        // as an opaque "Could not connect to QiQiClaw gateway". resolveGatewayWsUrl
    ...
    function scheduleReconnect() {
      ...
      if (bootCompleted && reconnectAttempt >= 6 && !reconnectFailureSurfaced) {
        reconnectFailureSurfaced = true
        failDesktopBoot('Could not reconnect to QiQiClaw gateway.')
      }
