...
export function useGatewayRequest() {
  ...
  const gatewayRef = useRef<QiQiClawGateway | null>(null)
  ...
  const reconnectingRef = useRef<Promise<QiQiClawGateway | null> | null>(null)
  ...
  useEffect(
    () =>
      $gateway.subscribe(gateway => {
        gatewayRef.current = gateway as QiQiClawGateway | null
      }),
    []
  )
  ...
  const requestGateway = useCallback(
    async <T>(method: string, params: Record<string, unknown> = {}) => {
      ...
      if (!gateway) {
        throw new Error('QiQiClaw gateway unavailable')
      }
