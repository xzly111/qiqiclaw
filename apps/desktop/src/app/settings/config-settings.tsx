...
import {
  getElevenLabsVoices,
  getQiQiClawConfigDefaults,
  getQiQiClawConfigRecord,
  getQiQiClawConfigSchema,
  saveQiQiClawConfig
} from '@/hermes'
import { useI18n } from '@/i18n'
import { cn } from '@/lib/utils'
import { notify, notifyError } from '@/store/notifications'
import type { ConfigFieldSchema, QiQiClawConfigRecord } from '@/types/hermes'
...
export function ConfigSettings({
  ...
  const [_defaults, setDefaults] = useState<QiQiClawConfigRecord | null>(null)
  ...
  useEffect(() => {
    let cancelled = false
    Promise.all([getQiQiClawConfigRecord(), getQiQiClawConfigDefaults(), getQiQiClawConfigSchema()])
      .then(([c, d, s]) => {
        if (cancelled) {
          return
        }
  ...
  useEffect(() => {
    ...
    const t = window.setTimeout(() => {
      void (async () => {
        try {
          await saveQiQiClawConfig(config)
  ...
  const updateConfig = (next: QiQiClawConfigRecord) => {
