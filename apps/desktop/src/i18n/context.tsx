...
import { getQiQiClawConfigRecord, type QiQiClawConfigRecord, saveQiQiClawConfig } from '@/hermes'
...
export interface I18nConfigClient {
  getConfig: () => Promise<QiQiClawConfigRecord>
  saveConfig: (config: QiQiClawConfigRecord) => Promise<{ ok: boolean }>
}
...
const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    ...
    return getQiQiClawConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.hermesDesktop?.api) {
    ...
    return saveQiQiClawConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: QiQiClawConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: QiQiClawConfigRecord, locale: Locale): QiQiClawConfigRecord {
