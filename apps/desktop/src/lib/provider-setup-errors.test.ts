...
describe('isProviderSetupErrorMessage', () => {
  it('matches generic missing-provider copy', () => {
    ...
    expect(isProviderSetupErrorMessage('No QiQiClaw provider is configured.')).toBe(true)
    expect(isProviderSetupErrorMessage('No QiQiClaw provider is configured.')).toBe(true)
    expect(isProviderSetupErrorMessage('set an API key (OPENROUTER_API_KEY) in ~/.qiqiclaw/.env')).toBe(true)
  })
