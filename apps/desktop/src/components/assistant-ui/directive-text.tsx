...
type QiQiClawRefType = (typeof HERMES_REF_TYPES)[number]
...
const ICON_PATHS: Record<QiQiClawRefType, string[]> = {
  file: [
...
const iconPathsFor = (type: string) => ICON_PATHS[type as QiQiClawRefType] ?? ICON_FALLBACK
  ...
  const matches = [
    ...
    ...Array.from(text.matchAll(HERMES_DIRECTIVE_RE)).map(match => {
      ...
      return {
        ...
        type: match[1] || 'file',
        label: shortLabel(match[1] as QiQiClawRefType, id),
        id
      }
    })
  ]
...
function shortLabel(type: QiQiClawRefType, id: string): string {
  if (type === 'terminal') {
    return id || 'terminal'
  }
...
/**
 * Renders text containing QiQiClaw directives (`@file:...`, `@image:...`) as
