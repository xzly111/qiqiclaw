...
// The connecting overlay renders the QiQiClaw opening animation. Match the
// media element specifically so the recovery overlay copy does not read as a
// false positive.
const isConnectingShown = () =>
  Boolean(screen.queryByLabelText(/QIQI-Claw启动中/i) || screen.queryByAltText('QIQI-CLAW'))
...
describe('connecting overlay vs recovery surface', () => {
  it('hard initial-boot failure surfaces the recovery overlay (the working path)', () => {
    ...
    $desktopBoot.set({ ...$desktopBoot.get(), error: 'QiQiClaw backend did not become ready', running: false, visible: true })
  ...
  it('remote socket drops after successful boot → connecting overlay reappears until reconnect recovery fires', () => {
  ...
  it('FIX: once the prolonged reconnect raises a recoverable boot error, the recovery overlay takes over', () => {
    ...
    $desktopBoot.set({
      ...$desktopBoot.get(),
      error: 'Lost connection to the QiQiClaw gateway and could not reconnect.',
      running: false,
      visible: true
    })
