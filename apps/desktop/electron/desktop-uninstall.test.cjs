...
test('resolveRemovableAppPath finds the .app bundle on macOS', () => {
  assert.equal(
    resolveRemovableAppPath('/Applications/QiQiClaw.app/Contents/MacOS/QiQiClaw', 'darwin'),
    '/Applications/QiQiClaw.app'
  )
  assert.equal(
    resolveRemovableAppPath('/Users/x/Applications/QiQiClaw.app/Contents/MacOS/QiQiClaw', 'darwin'),
    '/Users/x/Applications/QiQiClaw.app'
  )
})
...
test('resolveRemovableAppPath finds the install dir on Windows', () => {
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\Programs\\QiQiClaw\\QiQiClaw.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\Programs\\QiQiClaw'
  )
  assert.equal(
    resolveRemovableAppPath('C:\\Users\\x\\AppData\\Local\\qiqiclaw-desktop\\QiQiClaw.exe', 'win32'),
    'C:\\Users\\x\\AppData\\Local\\qiqiclaw-desktop'
  )
})

test('resolveRemovableAppPath returns null for an unrecognized Windows dir', () => {
  assert.equal(resolveRemovableAppPath('C:\\Temp\\foo\\QiQiClaw.exe', 'win32'), null)
})

test('resolveRemovableAppPath uses APPIMAGE on Linux when set', () => {
  assert.equal(
    resolveRemovableAppPath('/tmp/.mount_QiQiClawXXXX/hermes', 'linux', { APPIMAGE: '/home/x/Apps/QiQiClaw.AppImage' }),
    '/home/x/Apps/QiQiClaw.AppImage'
  )
})
...
test('shouldRemoveAppBundle requires packaged AND a resolved path', () => {
  assert.equal(shouldRemoveAppBundle(true, '/Applications/QiQiClaw.app'), true)
  assert.equal(shouldRemoveAppBundle(false, '/Applications/QiQiClaw.app'), false)
...
test('buildWindowsCleanupScript waits (bounded) for PID, runs uninstall, rmdir bundle', () => {
  const script = buildWindowsCleanupScript({
    ...
    agentRoot: 'C:\\hermes',
    uninstallArgs: ['-m', 'hermes_cli.uninstall', '--mode', 'full'],
    appPath: 'C:\\Users\\x\\AppData\\Local\\Programs\\QiQiClaw',
  ...
  assert.match(script, /rmdir \/s \/q "C:\\Users\\x\\AppData\\Local\\Programs\\QiQiClaw" >nul 2>&1/)
