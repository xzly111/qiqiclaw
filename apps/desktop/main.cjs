...
// HERMES_HOME — the user-facing root for everything QiQiClaw-related. Mirrors
...
let bootProgressState = {
  ...
  message: 'Waiting to start QiQiClaw backend',
  phase: 'idle',
  progress: 0,
  running: false,
  timestamp: Date.now()
...
// ALL repo mutation — running `qiqiclaw update` + rebuilding the desktop — so
...
async function applyUpdates(opts = {}) {
  ...
  try {
    ...
    if (!updater) {
      ...
      // checkout they're on — bare `qiqiclaw update` defaults to main and would
      ...
      const updateRoot = resolveUpdateRoot()
      let command = 'qiqiclaw update'
      try {
        const head = await runGit(['rev-parse', '--abbrev-ref', 'HEAD'], { cwd: updateRoot })
        const current = (head.stdout || '').trim()
        if (head.code === 0 && current && current !== 'HEAD') {
          const branch = await resolveHealedBranch(updateRoot, current)
          if (branch !== 'main') command = `qiqiclaw update --branch ${branch}`
        }
      } catch {
        // Best-effort: fall back to bare `qiqiclaw update` if branch detection fails.
      }
    ...
    emitUpdateProgress({ stage: 'restart', message: 'Handing off to the QiQiClaw updater…', percent: 100 })
...
async function applyUpdatesPosixInApp() {
  ...
  emitUpdateProgress({ stage: 'update', message: 'Updating QiQiClaw (git + dependencies)…', percent: 10 })
  ...
  if (updated.code !== 0) {
    emitUpdateProgress({ stage: 'error', message: 'qiqiclaw update failed.', error: updated.error || 'update-failed' })
    return { ok: false, error: 'qiqiclaw update failed' }
  }
  ...
  if (rebuilt.code !== 0) {
    emitUpdateProgress({
      stage: 'error',
      message: 'Backend updated, but the desktop rebuild failed. Restart QiQiClaw to retry.',
  ...
  const rebuiltApp = [
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac-arm64', 'QiQiClaw Desktop.app'),
    path.join(updateRoot, 'apps', 'desktop', 'release', 'mac', 'QiQiClaw Desktop.app')
  ...
  if (!rebuiltApp || !targetApp) {
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend updated. Restart QiQiClaw to load the new version.',
      percent: 100
    })
...
`
  ...
  try {
    ...
    emitUpdateProgress({
      stage: 'done',
      message: 'Backend + app updated. Restart QiQiClaw to load the new version.',
      percent: 100
    })
...
function resolveQiQiClawBackend(dashboardArgs) {
  ...
  if (!IS_PACKAGED && isQiQiClawSourceRoot(SOURCE_REPO_ROOT)) {
  ...
  //    (applyUpdates -> git pull) or `qiqiclaw update` from the CLI.
  ...
  if ((process.env.QIQICLAW_DESKTOP_IGNORE_EXISTING || process.env.HERMES_DESKTOP_IGNORE_EXISTING) !== '1') {
    ...
    if (hermesCommand) {
      if (looksLikeDesktopAppBinary(hermesCommand)) {
        rememberLog(`Ignoring desktop app executable on PATH while resolving QiQiClaw CLI: ${hermesCommand}`)
        hermesCommand = null
      }
    }
    ...
    if (hermesCommand) {
      ...
      rememberLog(
        `Ignoring existing QiQiClaw CLI at ${hermesCommand}: --version probe failed; falling through to bootstrap.`
      )
    }
  }
  ...
  return {
    ...
    label: 'QiQiClaw not installed yet; bootstrap required',
    command: null,
    args: dashboardArgs,
    bootstrap: true,
    env: {},
...
async function ensureRuntime(backend) {
  ...
  if (backend.kind === 'bootstrap-needed') {
    rememberLog('[bootstrap] no QiQiClaw install found; starting first-launch bootstrap')
    ...
    if (bootstrapResult.cancelled) {
      const cancelledError = new Error('QiQiClaw install was cancelled.')
    ...
    if (!bootstrapResult.ok) {
      const bootstrapError = new Error(
        `QiQiClaw bootstrap failed${bootstrapResult.failedStage ? ` at stage '${bootstrapResult.failedStage}'` : ''}: ` +
  ...
  if (IS_WINDOWS && !findGitBash()) {
    throw new Error(
      'Git for Windows is required for QiQiClaw on Windows (provides Git Bash, ' +
        "which the agent's terminal tool uses). Install it from " +
        'https://git-scm.com/download/win or run `winget install -e --id Git.Git`, ' +
        'then relaunch QiQiClaw.'
    )
  }
  ...
  if (!fileExists(venvPython)) {
    ...
    throw new Error(
      `QiQiClaw venv missing at ${VENV_ROOT}. Re-run the desktop installer or ` + '`scripts/install.ps1` to rebuild it.'
    )
  }

  backend.command = venvPython
  backend.label = `QiQiClaw at ${ACTIVE_HERMES_ROOT} (venv: ${VENV_ROOT})`
  updateBootProgress({
    phase: 'runtime.ready',
    message: 'QiQiClaw runtime is ready',
    progress: 82,
    running: true,
    error: null
  })
...
function fetchJson(url, token, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    if (parsed.protocol !== 'http:' && parsed.protocol !== 'https:') {
      reject(new Error(`Unsupported QiQiClaw backend URL protocol: ${parsed.protocol}`))
      return
    }
            ...
            reject(
              new Error(
                `Expected JSON from ${url} but got HTML (status ${res.statusCode}). ` +
                  'The endpoint is likely missing on the QiQiClaw backend.'
              )
            )
            return
          }
    ...
    req.setTimeout(timeoutMs, () => {
      req.destroy(new Error(`Timed out connecting to QiQiClaw backend after ${timeoutMs}ms`))
    })
...
async function waitForQiQiClaw(baseUrl, token) {
  ...
  throw new Error(`QiQiClaw backend did not become ready: ${lastError?.message || 'timeout'}`)
}
...
// signal so the chat composer doesn't stay stuck on "Starting QiQiClaw...".
...
function buildApplicationMenu() {
  const template = []
  const checkForUpdatesItem = UPDATES_DISABLED
    ? null
    : {
...
//
// Hosted QiQiClaw gateways gate the dashboard behind an OAuth provider (e.g.
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      win = new BrowserWindow({
        width: 520,
        height: 720,
        title: 'Sign in to QiQiClaw gateway',
...
function fetchJsonViaOauthSession(url, options = {}) {
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => {
      ...
      reject(new Error(`Timed out connecting to QiQiClaw backend after ${timeoutMs}ms`))
    }, timeoutMs)
...
async function buildRemoteConnection(rawUrl, authMode, token, source) {
  ...
  if (authMode === 'oauth') {
    ...
    if (!(await hasLiveOauthSession(baseUrl))) {
      const err = new Error(
        'Remote QiQiClaw gateway uses OAuth, but you are not signed in. ' +
  ...
  if (!token) {
    throw new Error(
      'Remote QiQiClaw gateway is selected, but no session token is saved. ' +
...
async function testDesktopConnectionConfig(input = {}) {
  ...
  // connect to QiQiClaw gateway". Mirror the renderer's connect here so the test
...
async function spawnPoolBackend(profile, entry) {
  ...
  child.once('error', error => {
    rememberLog(`QiQiClaw backend for profile "${profile}" failed to start: ${error.message}`)
    backendPool.delete(profile)
    rejectStart?.(error)
  })
  child.once('exit', (code, signal) => {
    rememberLog(`QiQiClaw backend for profile "${profile}" exited (${signal || code})`)
    backendPool.delete(profile)
    if (!ready) {
      rejectStart?.(new Error(`QiQiClaw backend for profile "${profile}" exited before it became ready (${signal || code}).`))
    }
  })
...
async function startQiQiClaw() {
  ...
  connectionPromise = (async () => {
    await advanceBootProgress('backend.resolve', 'Resolving QiQiClaw backend', 8)
    ...
    hermesProcess = spawn(backend.command, backend.args, {
      ...
      env: {
        ...
        // resolves to the SAME location our resolveQiQiClawHome() picked. Without
    ...
    hermesProcess.once('error', error => {
      rememberLog(`QiQiClaw backend failed to start: ${error.message}`)
      updateBootProgress(
        {
          error: error.message,
          message: `QiQiClaw backend failed to start: ${error.message}`,
          phase: 'backend.error',
          running: false
        },
        { allowDecrease: true }
    ...
    hermesProcess.once('exit', (code, signal) => {
      rememberLog(`QiQiClaw backend exited (${signal || code})`)
      hermesProcess = null
      connectionPromise = null
      sendBackendExit({ code, signal })
      if (!backendReady) {
        const message = `QiQiClaw backend exited before it became ready (${signal || code}).`
        updateBootProgress(
          {
            error: message,
            message,
    ...
    updateBootProgress({
      phase: 'backend.ready',
      message: 'QiQiClaw backend is ready. Finalizing desktop startup',
      progress: 94,
      running: true,
      error: null
    })
...
// "Starting QiQiClaw…". Before the renderer's backoff loop reconnects, it asks us
...
ipcMain.handle('hermes:terminal:start', async (event, payload = {}) => {
  if (!nodePty) {
    throw new Error('PTY support is unavailable. Reinstall desktop dependencies and restart QiQiClaw.')
  }
...
ipcMain.handle('hermes:updates:check', async () =>
  UPDATES_DISABLED
    ? {
        ...
        message: 'Updates are disabled for this local QiQiClaw build.',
        fetchedAt: Date.now()
      }
...
ipcMain.handle('hermes:updates:branch:set', async (_event, name) => {
  if (UPDATES_DISABLED) {
    return { ...readDesktopUpdateConfig(), disabled: true }
  }
...
async function runDesktopUninstall(mode) {
  ...
  if (!fileExists(venvPy)) {
    return {
      ok: false,
      error: 'agent-missing',
      message: `Can't run the uninstaller: no QiQiClaw backend venv at ${VENV_ROOT}.`
    }
  }
