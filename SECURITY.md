# QIQI-Claw Security Policy

This document describes QIQI-Claw's trust model, names the one
security boundary the project treats as load-bearing, and defines the
scope for vulnerability reports.

## 1. Reporting a Vulnerability

Report privately via [GitHub Security Advisories](https://github.com/xzly111/qiqiclaw/security/advisories/new)
or **security@nousresearch.com**. Do not open public issues for
security vulnerabilities. **QIQI-Claw does not operate a bug
bounty program.**
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted QIQI-Claw access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in QIQI-Claw's documentation
...
QIQI-Claw supports two OS-level isolation postures. They address
...
QIQI-Claw supports this in two ways:

- **QIQI-Claw's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, QIQI-Claw's in-process heuristics
...
QIQI-Claw filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in QIQI-Claw
itself. Bugs in QIQI-Claw's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   QIQI-Claw does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, QIQI-Claw's own documentation, or reasonable
  operator expectations would predict — including cases where
  QIQI-Claw has documented a stance about how its output should
...
- QIQI-Claw includes supply-chain guards for MCP server
