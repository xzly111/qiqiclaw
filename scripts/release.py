...
def generate_changelog(commits, tag_name, semver, repo_url="https://github.com/xzly111/qiqiclaw",
    ...
    lines.append(f"# QIQI-Claw v{semver} ({tag_name})")
    lines.append("")
    ...
    if first_release:
        ...
        lines.append("> for QIQI-Claw. See below for everything included in this initial release.")
        lines.append("")
...
def main():
    parser = argparse.ArgumentParser(description="QIQI-Claw Release Tool")
    ...
    print(f"{'='*60}")
    print(f"  QIQI-Claw Release Preview")
    print(f"{'='*60}")
    ...
    if args.publish:
        ...
        tag_result = git_result(
            "tag", "-a", tag_name, "-m",
            f"QIQI-Claw v{new_version} ({calver_date})\n\nWeekly release"
        )
        ...
        gh_cmd = [
            ...
            "--title", f"QIQI-Claw v{new_version} ({calver_date})",
        ...
        if result and result.returncode == 0:
            ...
            print(
                f"    gh release create {tag_name} --title 'QIQI-Claw v{new_version} ({calver_date})' "
