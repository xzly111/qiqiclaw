...
AlexKucera reported on Discord (2026-04-16) that after ``qiqiclaw update`` pulled
...
Requires ``OPENROUTER_API_KEY`` to be set (or sourced via ~/.qiqiclaw/.env).
"""
...
# Load ~/.qiqiclaw/.env so live runs pick up OPENROUTER_API_KEY without
