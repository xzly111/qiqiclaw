...
The root ``package.json`` is installed by ``qiqiclaw update`` on every user,
...
* ``@askjo/camofox-browser`` is NOT eager. It is an explicit opt-in
  ...
  Firefox-fork binary, which silently blocked ``qiqiclaw update`` for
...
def test_agent_browser_stays_eager() -> None:
    ...
    assert "agent-browser" in deps, (
        ...
        "after `qiqiclaw setup` / `qiqiclaw update` without an explicit "
        "post_setup step."
    )
