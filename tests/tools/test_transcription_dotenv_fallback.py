...
``~/.qiqiclaw/.env`` entries. These tests confirm each STT provider now
...
class TestProviderSelectionGate:
    ...
    ``os.environ`` a user with keys in ``~/.qiqiclaw/.env`` would be told
...
class TestEndToEndRegressionGuard:
    ...
    ``~/.qiqiclaw/.env`` carrying the key while ``os.environ`` does not.
