...
def _make_forum_channel(**attrs):
    if attrs:
        return SimpleNamespace(type=15, **attrs)
    ...
    channel = object.__new__(forum_cls)
    return channel
...
async def test_send_to_forum_create_thread_failure():
    ...
    forum_channel = _make_forum_channel(
        id=999,
        name="ideas",
        create_thread=AsyncMock(side_effect=Exception("rate limited")),
    )
    adapter._client = SimpleNamespace(
        get_channel=lambda _chat_id: forum_channel,
        fetch_channel=AsyncMock(),
...
async def test_forum_post_file_creation_failure():
    ...
    forum_channel = _make_forum_channel(
        id=999,
        create_thread=AsyncMock(side_effect=Exception("missing perms")),
    )
