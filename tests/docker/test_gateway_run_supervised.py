...
def test_gateway_run_no_supervise_flag_preserves_legacy_behavior(
    ...
    Three positive assertions confirm we took the pre-s6 path:
      ...
      * The CMD process is a python ``qiqiclaw gateway run`` invocation
        (not ``sleep infinity``).
...
def test_supervised_gateway_does_not_recurse(
    ...
    supervised ``qiqiclaw gateway run`` from re-entering the redirect.
    ...
    gateway slot would never actually run a python ``qiqiclaw gateway
    ...
    # Count python processes running `qiqiclaw gateway run`. If the
    ...
    assert n <= 1, (
        f"expected at most one supervised python `qiqiclaw gateway run` "
...
def test_supervised_gateway_stdout_reaches_docker_logs(
    ...
    a poor user experience: the iconic "QiQiClaw Gateway Starting…"
    ...
    assert "⚕" in combined or "QiQiClaw Gateway Starting" in combined, (
    ...
    assert "⚕" in file_contents or "QiQiClaw Gateway Starting" in file_contents, (
