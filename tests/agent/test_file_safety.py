...
class TestEnvFileReadBlocking:
    ...
    def test_allowed_hermes_env(self):
        ...
        # Note: hermes internal .env is in ~/.qiqiclaw/.env which is NOT a project-local
        # path, but the basename check applies to ANY .env. This is intentional —
        # even ~/.qiqiclaw/.env should not be readable via read_file.
        error = get_read_block_error(os.path.expanduser("~/.qiqiclaw/.env"))
        assert error is not None
