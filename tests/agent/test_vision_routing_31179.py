...
def _fresh_modules():
    ...
    for mod in list(sys.modules.keys()):
        if mod.startswith(("agent.auxiliary_client", "agent.image_routing",
                           "agent.anthropic_adapter", "agent.credential_pool",
...
""")
        ...
        fake_client = object()
        with patch.object(aux, "_main_model_supports_vision", return_value=True), patch.object(
            aux,
            "resolve_provider_client",
            return_value=(fake_client, "claude-sonnet-4-6"),
        ) as resolve_mock:
        ...
        assert resolve_mock.call_args.args[:2] == ("anthropic", "claude-sonnet-4-6")
