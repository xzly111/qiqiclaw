...
2. **Isolated HERMES_HOME.** HERMES_HOME points to a per-test tempdir so
   code reading ``~/.qiqiclaw/*`` via ``get_hermes_home()`` can't see the
...
_HERMES_BEHAVIORAL_VARS = frozenset({
    ...
    # the real ~/.qiqiclaw/kanban.db instead of the per-test HERMES_HOME.
    "HERMES_KANBAN_DB",
    "HERMES_KANBAN_BOARD",
    "HERMES_KANBAN_HOME",
...
def _hermetic_environment(tmp_path, monkeypatch):
    ...
    reads ``~/.qiqiclaw/*`` can't touch the real one, and pins TZ/LANG so
    ...
    #    ``~/.qiqiclaw/*`` via ``get_hermes_home()`` now gets the tempdir.
    #
    #    NOTE: We do NOT also redirect HOME. Doing so broke CI because
    #    some tests (and their transitive deps) spawn subprocesses that
    #    inherit HOME and expect it to be stable. If a test genuinely
    #    needs HOME isolated, it should set it explicitly in its own
    #    fixture. Any code in the codebase reading ``~/.qiqiclaw/*`` via
    ...
    #    ~/.qiqiclaw/plugins/ (which, per step 3, is now empty — but the
