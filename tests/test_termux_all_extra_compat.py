...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"qiqiclaw[termux]"' in text
    assert '"qiqiclaw[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"qiqiclaw[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
