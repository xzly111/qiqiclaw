...
def test_format_managed_message_homebrew(monkeypatch):
    ...
    message = format_managed_message("update QiQiClaw")
...
def test_recommended_update_command_defaults_to_hermes_update(monkeypatch):
    ...
    # ambient ~/.qiqiclaw/.managed if a prior test left HERMES_HOME pointing
    # somewhere with that marker, which would make get_managed_update_command()
    # return "Update your Nix flake input ..." instead of falling through to
    # detect_install_method().
    monkeypatch.setitem(
        recommended_update_command.__globals__,
    ...
    with patch("hermes_cli.config.get_managed_update_command", return_value=None), \
        ...
        assert recommended_update_command() == "qiqiclaw update"
