"""Tests for Codex auth — tokens stored in Hermes auth store (~/.qiqiclaw/auth.json)."""
...
def test_save_codex_tokens_syncs_manual_device_code_entries(tmp_path, monkeypatch):
    ...
    would have run ``qiqiclaw auth add openai-codex`` as a workaround, leaving
...
def test_refresh_429_classified_as_quota_not_auth_failure(monkeypatch):
    ...
    than a misleading "run qiqiclaw auth".
    """
    from hermes_cli.auth import (
    ...
    assert "qiqiclaw auth" not in rendered
