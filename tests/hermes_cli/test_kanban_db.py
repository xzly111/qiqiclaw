...
class TestSharedBoardPaths:
    ...
    def test_default_install_anchors_at_home_dot_hermes(
        ...
        # Standard install: HERMES_HOME == ~/.qiqiclaw, no profile active.
    ...
    def test_profile_worker_resolves_to_shared_root(
        ...
        # Reproduces the bug: dispatcher uses ~/.qiqiclaw/kanban.db,
        # worker spawned with -p <profile> previously resolved to
        # ~/.qiqiclaw/profiles/<profile>/kanban.db. After the fix both
        # converge on ~/.qiqiclaw/kanban.db.
        default_home = tmp_path / ".hermes"
        default_home.mkdir()
    ...
    def test_docker_custom_hermes_home_uses_env_path_directly(
        ...
        # Docker / custom deployment: HERMES_HOME points outside ~/.qiqiclaw.
...
def test_resolve_hermes_argv_module_actually_runs():
    ...
    assert "QiQiClaw" in r.stdout, f"unexpected output: {r.stdout[:200]!r}"
