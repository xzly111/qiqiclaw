"""Tests for the post-pull syntax guard in ``qiqiclaw update``.
...
markers in ``hermes_cli/config.py``; users who ran ``qiqiclaw update`` in
...
# release; if a future ``qiqiclaw update`` would brick users, this test fails
# in CI first.
# ---------------------------------------------------------------------------
...
def test_production_tree_passes_syntax_guard():
    ...
    assert ok is True, (
        ...
        f"qiqiclaw update would brick users. Error: {error}"
    )
