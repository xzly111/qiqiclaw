...
This saves 500-650ms on ``qiqiclaw --help``, ``hermes version``,
``qiqiclaw logs``, etc., by not importing ``google.cloud.pubsub_v1``,
...
def _live_subcommand_names() -> set[str]:
    """Run ``qiqiclaw --help`` in-process and parse the subcommand block.
