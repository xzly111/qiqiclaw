"""Tests for xAI Grok OAuth — tokens stored in Hermes auth store (~/.qiqiclaw/auth.json)."""
...
def test_xai_callback_handler_returns_400_when_callback_url_lacks_code_and_error():
    ...
    try:
        ...
        assert "qiqiclaw auth add xai-oauth" in body
...
def test_credential_pool_seeds_xai_oauth_from_singleton(tmp_path, monkeypatch):
    ...
    must surface that as a pool entry so `qiqiclaw auth list` reflects truth and
...
def test_credential_pool_seed_respects_suppression(tmp_path, monkeypatch):
    """`qiqiclaw auth remove xai-oauth <N>` for the seeded entry suppresses
    ...
    # Suppress the source — mimic `qiqiclaw auth remove`.
...
def test_auth_remove_xai_oauth_clears_singleton_and_sticks(tmp_path, monkeypatch):
    """End-to-end regression: ``qiqiclaw auth remove xai-oauth 1`` for a
    ...
    # Act: the user runs `qiqiclaw auth remove xai-oauth 1`.
...
def test_pool_manual_xai_entry_not_synced_from_singleton(tmp_path, monkeypatch):
    ...
    ``qiqiclaw auth add xai-oauth``) own their own refresh-token lifecycle
...
def test_pool_manual_entry_does_not_sync_back_to_singleton(tmp_path, monkeypatch):
    """`qiqiclaw auth add xai-oauth` entries (source='manual:xai_pkce') are
...
def test_pool_sync_back_preserves_active_provider(tmp_path, monkeypatch):
    ...
    provider (visible to ``qiqiclaw auth status``, ``qiqiclaw setup``, and the
