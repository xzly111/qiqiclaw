...
def _setup_setting_test(monkeypatch, tmp_path, mode):
    ...
    fake_load_config = lambda *a, **kw: {"updates": {"non_interactive_local_changes": mode}}
    monkeypatch.setattr(hermes_config, "load_config", fake_load_config)
    monkeypatch.setattr(hermes_main, "load_config", fake_load_config, raising=False)
    side_effect, recorded = _make_update_side_effect()
    monkeypatch.setattr(hermes_main.subprocess, "run", side_effect)
...
def test_bootstrap_marker_not_autostashed_by_update(tmp_path):
    ...
    ``qiqiclaw update``'s ``git stash push --include-untracked`` does not sweep it
    ...
    # Exact flags used by qiqiclaw update (hermes_cli/main.py).
