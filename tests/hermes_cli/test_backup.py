"""Tests for qiqiclaw backup and import commands."""
...
def _make_hermes_tree(root: Path) -> None:
    """Create a realistic ~/.qiqiclaw directory structure for testing."""
...
class TestImport:
    ...
    def test_rejects_non_hermes_zip(self, tmp_path, monkeypatch):
        """Import rejects a zip that doesn't look like a qiqiclaw backup."""
        hermes_home = tmp_path / ".hermes"
        hermes_home.mkdir()
        monkeypatch.setenv("HERMES_HOME", str(hermes_home))
        monkeypatch.setattr(Path, "home", lambda: tmp_path)
...
# ---------------------------------------------------------------------------
# Pre-update backup (qiqiclaw update safety net)
# ---------------------------------------------------------------------------

class TestPreUpdateBackup:
    """Tests for create_pre_update_backup — the auto-backup ``qiqiclaw update``
    ...
    def test_backup_contents_match_full_backup(self, hermes_home):
        ...
        ``qiqiclaw backup`` would, and should exclude the same directories."""
        from hermes_cli.backup import create_pre_update_backup
        out = create_pre_update_backup(hermes_home=hermes_home)
        assert out is not None
...
class TestPreMigrationBackup:
    ...
    ``hermes claw migrate`` runs before mutating ~/.qiqiclaw/."""
    ...
    def test_backup_uses_shared_exclusion_rules(self, hermes_home):
        ...
        ``qiqiclaw backup`` / ``create_pre_update_backup`` — no drift."""
        from hermes_cli.backup import create_pre_migration_backup
        out = create_pre_migration_backup(hermes_home=hermes_home)
        assert out is not None
        with zipfile.ZipFile(out) as zf:
    ...
    def test_missing_hermes_home_returns_none(self, tmp_path):
        """Fresh install with no ~/.qiqiclaw yet — nothing to back up."""
        from hermes_cli.backup import create_pre_migration_backup
...
class TestRestoreCronJobsIfEmptied:
    """`qiqiclaw update` config migration can leave cron/jobs.json valid-but-empty,
