...
quarantine retry / reboot-deferred fallback during `qiqiclaw update` on Windows.
...
def test_format_message_mentions_pids_and_remediation(tmp_path):
    ...
    assert "QiQiClaw Desktop" in msg
    assert "--force" in msg
...
def test_quarantine_actionable_warning_when_everything_fails(
    ...
    assert "QiQiClaw Desktop" in captured or "gateway" in captured.lower()
