...
def _pretend_project_root_is_git_checkout(monkeypatch):
    """These tests mock git subprocesses; keep them independent of checkout layout."""
    real_exists = Path.exists

    def fake_exists(path):
        if path == PROJECT_ROOT / ".git":
            return True
...
class TestCmdUpdateBranchFlag:
    """``qiqiclaw update --branch <name>`` targets the requested branch.
...
class TestCmdUpdateCheckBranchFlag:
    """``qiqiclaw update --check --branch <name>`` honors the branch override.
...
class TestCmdUpdateZipBranchRefusal:
    """``qiqiclaw update --branch=<non-main>`` must refuse on the ZIP fallback path.
