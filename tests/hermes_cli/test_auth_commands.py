...
def test_auth_add_google_gemini_cli_sets_active_provider(tmp_path, monkeypatch):
    """qiqiclaw auth add google-gemini-cli must set active_provider in auth.json.
    ...
    # pool entry from pool.add_entry() still present for qiqiclaw auth list
...
def test_auth_add_qwen_oauth_sets_active_provider(tmp_path, monkeypatch):
    """qiqiclaw auth add qwen-oauth must set active_provider in auth.json.
...
def test_auth_add_nous_oauth_persists_pool_entry(tmp_path, monkeypatch):
    ...
    # `qiqiclaw auth add nous` must also populate providers.nous so the
...
def test_auth_add_nous_oauth_honors_custom_label(tmp_path, monkeypatch):
    """`qiqiclaw auth add nous --type oauth --label <name>` must preserve the
...
def test_auth_add_xai_oauth_sets_active_provider(tmp_path, monkeypatch):
    """qiqiclaw auth add xai-oauth must write providers singleton and set active_provider.
...
def test_auth_add_codex_clears_suppression_marker(tmp_path, monkeypatch):
    """Re-linking codex via `qiqiclaw auth add openai-codex` must clear any suppression marker."""
    monkeypatch.setenv("HERMES_HOME", str(tmp_path / "hermes"))
    hermes_home = tmp_path / "hermes"
    hermes_home.mkdir(parents=True, exist_ok=True)

    # Pre-existing suppression (simulating a prior `qiqiclaw auth remove`)
...
def test_auth_remove_env_seeded_suppresses_shell_exported_var(tmp_path, monkeypatch, capsys):
    """`qiqiclaw auth remove xai 1` must stick even when the env var is exported
    by the shell (not written into ~/.qiqiclaw/.env).  Before PR for #13371 the
...
def test_auth_remove_env_seeded_dotenv_only_no_shell_hint(tmp_path, monkeypatch, capsys):
    """When the env var lives only in ~/.qiqiclaw/.env (not the shell), the
...
def test_auth_add_clears_env_suppression_for_provider(tmp_path, monkeypatch):
    """Re-adding a credential via `qiqiclaw auth add <provider>` clears any
...
def test_seed_from_env_respects_env_suppression(tmp_path, monkeypatch):
    ...
    via `qiqiclaw auth remove`.  This is the gate that prevents shell-exported
...
def test_seed_from_singletons_respects_hermes_pkce_suppression(tmp_path, monkeypatch):
    """anthropic hermes_pkce must not re-seed from ~/.qiqiclaw/.anthropic_oauth.json when suppressed."""
    hermes_home = tmp_path / "hermes"
    hermes_home.mkdir(parents=True, exist_ok=True)
    monkeypatch.setenv("HERMES_HOME", str(hermes_home))
...
def test_auth_add_clears_all_suppressions_including_non_env(tmp_path, monkeypatch):
    """Re-adding a credential via `qiqiclaw auth add <provider>` clears ALL
...
def test_auth_remove_codex_manual_device_code_suppresses_canonical(tmp_path, monkeypatch):
    """Removing a manual:device_code entry (from `qiqiclaw auth add openai-codex`)
