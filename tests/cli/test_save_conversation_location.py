...
file to be discoverable via ``qiqiclaw sessions browse``, but CWD-resident
snapshots are not indexed in the state DB and are generally invisible.
The fix writes snapshots under ``~/.qiqiclaw/sessions/saved/`` and prints
...
def hermes_home(tmp_path, monkeypatch):
    home = tmp_path / ".qiqiclaw"
    home.mkdir()
    monkeypatch.setattr(Path, "home", lambda: tmp_path)
    monkeypatch.setenv("QIQICLAW_HOME", str(home))
    monkeypatch.delenv("HERMES_HOME", raising=False)
    import qiqiclaw_constants
    if hasattr(qiqiclaw_constants, "_qiqiclaw_home_cache"):
        qiqiclaw_constants._qiqiclaw_home_cache = None
    return home
...
def test_save_conversation_writes_under_qiqiclaw_home(hermes_home, tmp_path, monkeypatch, capsys):
    """Snapshot must land under ~/.qiqiclaw/sessions/saved/, not CWD."""
    ...
    for mod in [m for m in sys.modules if m.startswith("cli") or m in {"hermes_constants", "qiqiclaw_constants"}]:
    ...
    assert "qiqiclaw --resume 20260101_120000_abc123" in out, out
