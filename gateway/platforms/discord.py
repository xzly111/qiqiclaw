...
def _bind_discord_runtime(*, prefer_real: bool = False):
    """Refresh discord-related globals from the best available runtime module.

    Some tests install a lightweight ``discord`` mock into ``sys.modules``
    before later integration paths need the real discord.py voice runtime.
    Critical paths call this helper so the adapter can recover a real module
    when one is installed, instead of staying pinned to an early mock.
    """
    global DISCORD_AVAILABLE, discord, DiscordMessage, Intents, commands

    runtime = sys.modules.get("discord") or discord

    if prefer_real and (runtime is None or not hasattr(runtime, "__file__")):
        try:
            spec = importlib.util.find_spec("discord")
        except Exception:
            spec = None
        if spec and spec.loader and getattr(spec, "origin", None):
            try:
    ...
    discord = runtime
    DiscordMessage = getattr(runtime, "Message", Any)
    Intents = getattr(runtime, "Intents", Any)
    try:
        commands = importlib.import_module("discord.ext.commands")
    except Exception:
        commands = getattr(getattr(runtime, "ext", None), "commands", None)
    DISCORD_AVAILABLE = True
    return runtime
...
def check_discord_requirements() -> bool:
    ...
    return _bind_discord_runtime(prefer_real=False) is not None
...
def _build_allowed_mentions():
    ...
    allowed_mentions = discord.AllowedMentions(**values)
    for name, value in values.items():
        if getattr(allowed_mentions, name, None) is not value:
            try:
                setattr(allowed_mentions, name, value)
            except Exception:
                pass
...
class VoiceReceiver:
    ...
    def start(self):
        ...
        _bind_discord_runtime(prefer_real=True)
    ...
    def _install_speaking_hook(self, conn):
        ...
        try:
            runtime_discord = _bind_discord_runtime(prefer_real=True) or discord
            missing = getattr(getattr(runtime_discord, "utils", None), "MISSING", object())
    ...
    def _on_packet(self, data: bytes):
        ...
        try:
            ...
            if ssrc not in self._decoders:
                self._decoders[ssrc] = runtime_discord.opus.Decoder()
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = float(_legacy_env("DISCORD_TEXT_BATCH_DELAY_SECONDS", "0.6"))
        self._text_batch_split_delay_seconds = float(_legacy_env("DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", "2.0"))
    ...
    def _is_discord_channel_type(self, channel: Any, type_name: str) -> bool:
        """Return True when channel matches the current discord.py channel type.

        Some tests deliberately reload this module with discord.py unavailable,
        while already-created adapter instances still reference the original
        module globals. Keep channel checks resilient to those module swaps.
        """
        cls = getattr(discord, type_name, None) if discord is not None else None
        if isinstance(cls, type):
            try:
                ...
                pass
        channel_name = type(channel).__name__.lower()
        needle = type_name.lower()
        return channel_name.endswith(needle) or needle in channel_name

    def _is_dm_channel(self, channel: Any) -> bool:
        return self._is_discord_channel_type(channel, "DMChannel")

    def _is_thread_channel(self, channel: Any) -> bool:
        return self._is_discord_channel_type(channel, "Thread")

    def _is_text_channel(self, channel: Any) -> bool:
        return self._is_discord_channel_type(channel, "TextChannel")
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            async def on_message(message: DiscordMessage):
                ...
                if not self._is_dm_channel(message.channel) and message.mentions:
                    _self_mentioned = (
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields QiQi claw manages."""
    ...
    async def get_chat_info(self, chat_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._is_dm_channel(channel):
                chat_type = "dm"
                name = channel.recipient.name if channel.recipient else str(chat_id)
            elif self._is_thread_channel(channel):
                chat_type = "thread"
                name = channel.name
            elif self._is_text_channel(channel):
                chat_type = "channel"
                name = f"#{channel.name}"
                if channel.guild:
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your QiQi claw session")
        ...
        @tree.command(name="status", description="Show QiQi claw session status")
        ...
        @tree.command(name="stop", description="Stop the running QiQi claw agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.qiqiclaw/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update QiQi claw to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the QiQi claw gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a QiQi claw session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to QiQi claw in the thread",
        ...
        # qiqiclaw_cli/commands.py automatically appear as Discord slash
        ...
        try:
            from qiqiclaw_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from qiqiclaw_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        """
        try:
            from qiqiclaw_cli.commands import discord_skill_commands_by_category
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a QiQi claw skill",
    ...
    def _build_slash_event(self, interaction: discord.Interaction, text: str) -> MessageEvent:
        ...
        is_thread = self._is_thread_channel(interaction.channel)
        thread_id = None
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new QiQi claw session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by QiQi claw: **{name}**"
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        thread_name = content[:80] if content else "QiQi claw"
        if len(content) > 80:
        ...
        try:
            ...
            try:
                seed_msg = await message.channel.send(f"\U0001f9f5 Thread created by QiQi claw: **{thread_name}**")
                thread = await seed_msg.create_thread(
                    name=thread_name,
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``qiqiclaw update --gateway``
    ...
    async def send_model_picker(
        ...
        try:
            ...
            try:
                from qiqiclaw_cli.providers import get_label
    ...
    def _is_forum_parent(self, channel: Any) -> bool:
        ...
        if channel is None:
            return False
        # Use sys.modules to get the current discord module (supports mocking in tests)
        import sys
        discord_mod = sys.modules.get("discord", discord)
        forum_cls = getattr(discord_mod, "ForumChannel", None)
    ...
    async def _handle_message(self, message: DiscordMessage) -> None:
        ...
        is_thread = self._is_thread_channel(message.channel)
        if is_thread:
        ...
        if not self._is_dm_channel(message.channel):
        ...
        if not is_thread and not self._is_dm_channel(message.channel):
...
if DISCORD_AVAILABLE:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``qiqiclaw update`` prompts.
    ...
    class ModelPickerView(discord.ui.View):
        ...
        async def _on_model_selected(self, interaction: discord.Interaction):
            ...
            self.clear_items()
            await _maybe_await(interaction.response.edit_message(
                embed=discord.Embed(
            ...
            await _maybe_await(interaction.edit_original_response(
                embed=discord.Embed(
