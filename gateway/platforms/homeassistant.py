...
class HomeAssistantAdapter(BasePlatformAdapter):
    ...
    async def disconnect(self) -> None:
        ...
        # Close the socket after cancelling the listener so we do not block on
        # a task that is still iterating the websocket.
        await self._cleanup_ws()
    ...
    async def _handle_ha_event(self, event: Dict[str, Any]) -> None:
        ...
        # Apply domain/entity watch filters. Explicit filters narrow the
        # stream; no filters means drop everything unless watch_all is set.
