...
Resolution order for text tasks (auto mode):
  ...
  3. Nous Portal (~/.qiqiclaw/auth.json active provider)
...
# Models we know are text-only even when the external capability catalog has
# no row yet. Keep this model-scoped so future vision-capable providers/models
# are not blocked by a broad provider skip.
_KNOWN_TEXT_ONLY_VISION_MODELS: frozenset[tuple[str, str]] = frozenset({
...
_OR_HEADERS_BASE = {
    "HTTP-Referer": "https://github.com/xzly111/qiqiclaw",
    "X-Title": "QIQI-Claw",
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (QIQI-Claw)",
...
def _read_nous_auth() -> Optional[dict]:
    """Read and validate the active QIQI-Claw auth store for a Nous provider.
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `qiqiclaw setup` or `qiqiclaw model` and enter a valid http(s) base URL."
        ) from exc
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    #    scenario where a user switches providers via `qiqiclaw model` but the
    #    old OPENAI_BASE_URL lingers in ~/.qiqiclaw/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: qiqiclaw model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.qiqiclaw/.env",
    ...
    if provider == "nous":
        ...
        if client is None:
            logger.warning("resolve_provider_client: nous requested "
                           "but Nous Portal not configured (run: qiqiclaw auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no Codex OAuth token found (run: qiqiclaw model)")
                return None, None
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: qiqiclaw model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: qiqiclaw doctor for "
                "diagnostics)"
            )
            return None, None
...
def _main_model_supports_vision(provider: str, model: Optional[str]) -> bool:
    ...
    if supports is None:
        ...
        if (provider_key, model_key) in _KNOWN_TEXT_ONLY_VISION_MODELS:
            return False
        ...
        if client is None:
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                raise RuntimeError(
                    ...
                    f"variable, or switch to a different provider with `qiqiclaw model`."
                )
