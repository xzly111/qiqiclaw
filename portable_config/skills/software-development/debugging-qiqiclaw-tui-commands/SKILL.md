---
name: debugging-qiqiclaw-tui-commands
description: "Debug QiQi TUI slash commands: Python, gateway, Ink UI."
version: 1.0.0
author: QiQi claw
license: MIT
metadata:
  qiqiclaw:
    tags: [debugging, qiqiclaw, tui, slash-commands, typescript, python]
    related_skills: [python-debugpy, node-inspect-debugger, systematic-debugging]
---

# Debugging QiQi TUI Slash Commands

## Overview

QiQi claw slash commands span three layers — Python command registry, tui_gateway JSON-RPC bridge, and the Ink/TypeScript frontend. When a command misbehaves (missing from autocomplete, works in CLI but not TUI, config persists but UI doesn't update), the bug is almost always one layer being out of sync with another.

Use this skill when you encounter issues with slash commands in the QiQi TUI, particularly when commands aren't showing in autocomplete, aren't working properly in the TUI, or need to be added/updated.
...
4. If the command runs server-side, add a handler in `QiQiClawCLI.process_command()` in `cli.py`:
   ```python
...
1. Rebuild the TUI:
   ```bash
   cd /home/bb/qiqiclaw && npm --prefix ui-tui run build
   ```
...
4. Execute the command and confirm:
   ...
   - Any persisted config updates correctly (`read_file ~/.qiqiclaw/config.yaml`)
