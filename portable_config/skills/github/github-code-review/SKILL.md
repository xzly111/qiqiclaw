...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if [ -f ~/.qiqiclaw/.env ] && grep -q "^GITHUB_TOKEN=" ~/.qiqiclaw/.env; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" ~/.qiqiclaw/.env | head -1 | cut -d= -f2 | tr -d '\n\r')
...
```bash
source "${QIQICLAW_HOME:-$HOME/.qiqiclaw}/skills/github/github-auth/scripts/gh-env.sh"
