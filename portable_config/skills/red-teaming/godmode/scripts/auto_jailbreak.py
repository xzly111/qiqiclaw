...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("QIQICLAW_HOME", os.path.expanduser("~/.qiqiclaw")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("QIQICLAW_HOME", Path.home() / ".qiqiclaw")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# QiQi claw config paths
# ═══════════════════════════════════════════════════════════════════

QIQICLAW_HOME = Path(os.getenv("QIQICLAW_HOME", Path.home() / ".qiqiclaw"))
CONFIG_PATH = QIQICLAW_HOME / "config.yaml"
PREFILL_PATH = QIQICLAW_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # Nous/QiQi claw models — already uncensored, just needs clean prompt
    "qiqiclaw": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "qiqiclaw" in model_lower or "nous" in model_lower:
        return "qiqiclaw"
...
def _get_current_model() -> tuple:
    """Read current model and provider from QiQi claw config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.qiqiclaw/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart QiQi claw for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart QiQi claw for changes to take effect.")
