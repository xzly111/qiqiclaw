...
QIQICLAW_HOME_DIR="${QIQICLAW_HOME:-$HOME/.qiqiclaw}"
HERMES_CFG="${QIQICLAW_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure QiQi claw config has twozero_td MCP entry ──
if [[ ! -f "$HERMES_CFG" ]]; then
    echo -e " ${FAIL} QiQi claw config not found at ${HERMES_CFG}"
    manual_steps+=("Create ${HERMES_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HERMES_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in QiQi claw config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to QiQi claw config..."
    python3 -c "
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
    ...
    manual_steps+=("Restart QiQi claw session to pick up config change")
fi
