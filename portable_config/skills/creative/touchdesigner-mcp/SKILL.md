...
metadata:
  qiqiclaw:
    ...
    related_skills: [native-mcp, ascii-video, manim-video, qiqiclaw-video]
...
```bash
bash "${QIQICLAW_HOME:-$HOME/.qiqiclaw}/skills/creative/touchdesigner-mcp/scripts/setup.sh"
```

The script will:
1. Check if TD is running
2. Download twozero.tox if not already cached
3. Add `twozero_td` MCP server to QiQi config (if missing)
...
3. **Restart QiQi session** to pick up the new MCP server
