...
## 5. QiQi claw Configuration

### Config location

`$QIQICLAW_HOME/config.yaml` (defaults to `~/.qiqiclaw/config.yaml` when `QIQICLAW_HOME` is unset)
...
Restart the QiQi claw session for changes to take effect. The MCP connection is
