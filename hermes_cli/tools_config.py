...
`hermes tools` and `qiqiclaw setup tools` both enter this module.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``qiqiclaw update`` and
    ...
    if _plat.system() != "Darwin":
        if upgrade:
            # Silent on non-macOS — `qiqiclaw update` calls this for every
...
def _run_post_setup(post_setup_key: str):
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.qiqiclaw/config.yaml")
        ...
        try:
            ...
            _print_info("    Run manually: qiqiclaw auth spotify")
            return
        ...
        try:
            ...
            _print_info("    Run later: qiqiclaw auth spotify")
        except Exception as exc:
        ...
        try:
            ...
            _print_info("    Run later: qiqiclaw auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `qiqiclaw auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: qiqiclaw auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: qiqiclaw auth add xai-oauth"
                )
        else:
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `hermes tools` and `qiqiclaw setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print(color("Q Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        print()
        return
    print(color("Q QiQiClaw Tool Configuration", Colors.CYAN, Colors.BOLD))
    print(color("  Enable or disable tools per platform.", Colors.DIM))
    print(color("  Tools that need API keys will be configured when enabled.", Colors.DIM))
    print(color("  Guide: QiQiClaw tools can be configured here per platform.", Colors.DIM))
    print()
