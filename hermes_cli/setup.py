...
# display_hermes_home imported lazily at call sites (stale-module safety during qiqiclaw update)
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("Q QiQiClaw Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
...
# between `hermes tools` and `qiqiclaw setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    # on demand via `qiqiclaw auth add`, `qiqiclaw setup` vision, and
    # `qiqiclaw setup tts`. This keeps both quick and full setup thin.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via qiqiclaw setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_terminal_backend(config: dict):
    ...
    if selected_backend == "local":
        ...
        # configurable later via `qiqiclaw setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `qiqiclaw setup terminal`.
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `qiqiclaw setup tools` and `hermes tools` use the same flow:
...
def _run_portal_one_shot(config: dict) -> None:
    ...
    Wired into ``qiqiclaw setup --portal`` and ``hermes portal``. This is the
    ...
    ``qiqiclaw setup`` and hunt for the quick-setup option.
    ...
    print(color("│     Q QiQiClaw Setup — Nous Portal (one-shot)           │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
...
def run_setup_wizard(args):
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     Q QiQiClaw Setup — {label:<32s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    # Tune later with `qiqiclaw setup agent`.
    if not is_existing:
...
def _run_first_time_quick_setup(config: dict, hermes_home, is_existing: bool):
    ...
    settings, tools); the user can customize later via ``qiqiclaw setup <section>``
