...
TIPS = [
    ...
    "qiqiclaw --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hermes chat --source telegram tags the session for filtering in qiqiclaw sessions list.",
    ...
    "qiqiclaw sessions browse opens an interactive session picker with search.",
    "qiqiclaw sessions stats shows session counts by platform and database size.",
    "qiqiclaw sessions prune --older-than 30 cleans up old sessions.",
    ...
    "qiqiclaw auth add lets you add multiple API keys for credential pool rotation.",
    "hermes completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "qiqiclaw logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "qiqiclaw backup creates a zip backup of your entire Hermes home directory.",
    ...
    "qiqiclaw update syncs new bundled skills to ALL profiles automatically.",
    "qiqiclaw gateway install sets up Hermes as a system service (systemd/launchd).",
    ...
    "qiqiclaw sessions export backup.jsonl exports all sessions for backup or analysis.",
    ...
    "qiqiclaw gateway install sets it up as a system service that starts on boot.",
    ...
    "Checkpoints use shadow repos in ~/.qiqiclaw/checkpoints/ — your project's .git is never touched.",
    ...
    "qiqiclaw auth add nous or qiqiclaw auth add openai-codex sets up OAuth-based providers.",
    ...
    "Cron scripts live in ~/.qiqiclaw/scripts/ and run before the agent — perfect for data collection pipelines.",
    ...
    "The skills audit log at ~/.qiqiclaw/skills/.hub/audit.log tracks every install and removal operation.",
    ...
    "Every interrupt during an agent run is logged to ~/.qiqiclaw/interrupt_debug.log with timestamps.",
    ...
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.qiqiclaw/.env.",
    "Skills from trusted QiQiClaw sources get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.qiqiclaw/skills/.hub/quarantine/ holds skills pending security review.",
    ...
    '/reload reloads ~/.qiqiclaw/.env into the running session — pick up new API keys without restarting.',
    ...
    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.qiqiclaw/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.qiqiclaw/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',
    ...
    'qiqiclaw auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    'qiqiclaw dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'qiqiclaw dashboard embeds the full QiQiClaw TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.qiqiclaw/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.qiqiclaw/dashboard-plugins/ — no npm build required.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.qiqiclaw/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.qiqiclaw/skills/ so drop-in skills appear without restarting the session.',
    ...
    "hermes chat --source tool tags programmatic chats so they don't clutter qiqiclaw sessions list.",
    'hermes dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'qiqiclaw sessions rename <ID> "new title" renames any past session; qiqiclaw sessions delete <ID> removes one.',
    ...
    'qiqiclaw setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    ...
    '--source tool chats are excluded from qiqiclaw sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.qiqiclaw/dashboard-plugins/.',
]
