...
# When the user upgrades code via ``git pull`` (or ``qiqiclaw update``
...
# ``qiqiclaw update`` to recover.  Missing the bootstrap means UTF-8 stdio
...
def _suppress_mouse_residue_early() -> None:
    ...
    if not _wants_tui_early():
        return
    try:
        # Skip when stdout is redirected (`qiqiclaw --tui … >log`, CI capture):
...
def _try_termux_ultrafast_version() -> bool:
    """Handle ``qiqiclaw --version`` before config/logging imports on Termux."""
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hermes tools, qiqiclaw setup, hermes model) use
...
def _apply_profile_override() -> None:
    ...
    # parent directory name (e.g. ~/.qiqiclaw/profiles/coder or
...
def _launch_tui(
    ...
    # in the user's shell would otherwise make a plain `qiqiclaw --tui` try to
    ...
    # Exit code 42 = TUI requested an update. Relaunch as `qiqiclaw update` so
...
def _sync_bundled_skills_quietly() -> None:
    """Seed ``~/.qiqiclaw/skills/`` with the bundled skill library on first launch.
...
def cmd_chat(args):
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'qiqiclaw sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'qiqiclaw setup' at any time to configure.")
        sys.exit(1)
...
def cmd_whatsapp(args):
    ...
    # has no creds.json.  Every subsequent `qiqiclaw gateway` then paid a 30s
    ...
    if (session_dir / "creds.json").exists():
        ...
        # and `qiqiclaw gateway` skips it cleanly instead of paying a 30s
        ...
        print()
        print("  Or install as a service: qiqiclaw gateway install")
    else:
...
def cmd_postinstall(args):
    ...
    print("Q QiQiClaw post-install bootstrap")
    print()
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.qiqiclaw/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `qiqiclaw setup` from scratch. OpenRouter
...
def _model_flow_xai_oauth(_config, current_model="", *, args=None):
    ...
    # live only in the pool (e.g. after ``qiqiclaw auth add xai-oauth``).  Fall
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use QiQiClaw URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    if use_entra:
        ...
        if not has_azure_identity_installed():
            ...
            print(
                "  QiQiClaw will install it now (the preflight below "
...
# fast paths like `qiqiclaw --version` and slash-command dispatch that never
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``qiqiclaw setup`` / ``hermes model``.
    ...
    recover from a malformed paste without editing ``~/.qiqiclaw/.env`` by hand.
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `qiqiclaw setup` to configure {pconfig.name} again."
        )
        return "", True
...
def cmd_uninstall(args):
    """Uninstall QiQiClaw (or just the Chat GUI with --gui)."""
...
# even run ``qiqiclaw update`` again to roll forward. The post-pull syntax
...
#   - ``qiqiclaw update`` can unconditionally call ``hermes desktop --build-only``
...
def _desktop_build_needed(desktop_dir: Path, project_root: Path, *, source_mode: bool) -> bool:
    ...
    ``qiqiclaw update`` that pulled new source but hasn't built yet).
    """
...
def _desktop_linux_sandbox_fixup(packaged_executable: Path) -> bool:
    ...
    if not sandbox.exists():
        print(f"✗ QiQiClaw Desktop is missing Electron's Linux sandbox helper: {sandbox}")
        return False
    ...
    if not sudo:
        print("✗ QiQiClaw Desktop requires sudo to configure Electron's Linux sandbox helper.")
        return False
...
def cmd_gui(args: argparse.Namespace):
    ...
    if source_mode:
        print("→ Launching QiQiClaw Desktop from source build...")
    ...
    print(f"→ Launching packaged QiQiClaw Desktop: {packaged_executable}")
...
def _find_stale_dashboard_pids(
    ...
    *exclude_pids* is an optional set of PIDs that must never be returned.
    This is used by the QiQiClaw Desktop Electron app to protect its own
    backend child process: when the desktop spawns ``qiqiclaw dashboard`` as
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `qiqiclaw update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``qiqiclaw update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``qiqiclaw update`` invocations skip the block until a newer
...
def _kill_stale_dashboard_processes(
    ...
    """
    # When the QiQiClaw Desktop Electron app spawns this dashboard as a
...
def _update_via_zip(args):
    ...
    if branch != "main":
        ...
        print(
            ...
            "or NTFS filter holding files open) and rerun `qiqiclaw update "
            f"--branch {branch}`, or update against main with `qiqiclaw update`."
        )
        sys.exit(1)
    zip_url = (
...
def _discard_stashed_changes(
    ...
    if stash_selector is None:
        print(
            ...
            "but QiQiClaw couldn't find the stash entry to drop."
        )
        _print_stash_cleanup_guidance(stash_ref)
        return False
    ...
    if drop.returncode != 0:
        print(
            "⚠ Configured to discard local changes, but QiQiClaw couldn't drop "
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print("  This means you may miss updates from QiQiClaw.")
        print()
        try:
            response = (
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``qiqiclaw update`` is still progressing even if pip/uv itself is silent.
    """
...
def _detect_concurrent_hermes_instances(
    ...
    The QiQiClaw Desktop Electron app spawns ``hermes.EXE`` as a backend child,
    so during ``qiqiclaw update`` the user-invoked process and the desktop's
    ...
    chain are excluded so the running ``qiqiclaw update`` invocation never
    ...
    # file lock) is the parent. Without excluding it, every ``qiqiclaw update``
...
def _format_concurrent_instances_message(
    ...
    lines.append("")
    lines.append("  Close QiQiClaw Desktop, exit any open `hermes` REPLs, and")
    lines.append("  stop the gateway (`qiqiclaw gateway stop`) before retrying.")
    lines.append("")
    if matches:
        ...
        lines.append("")
    lines.append("  Override with `qiqiclaw update --force` if you've already")
...
def _quarantine_running_hermes_exe(
    ...
    when ``qiqiclaw update`` runs, ``hermes.exe`` IS the live process, and uv
    ...
    handles (recovers in <1s), or the QiQiClaw Desktop backend child process
    ...
    3. Print a clear warning naming the most likely culprit (running
       QiQiClaw Desktop / gateway / REPL) and pointing to ``--force``.
    ...
    for shim in _hermes_exe_shims(scripts_dir):
        ...
        print(
            "    Close QiQiClaw Desktop, exit other `hermes` REPLs, stop the "
            "gateway, or pause AV scanning, then re-run `qiqiclaw update`."
        )
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``qiqiclaw update`` runs ``uv pip install -e .[all]`` which
    ...
    if failed:
        ...
        print("  `qiqiclaw update` once the upstream issue is resolved.")
...
def _install_python_dependencies_with_optional_fallback(
    ...
    *,
    ...
    # hours later inside ``qiqiclaw update``'s desktop-rebuild or skill-sync
...
def _verify_core_dependencies_installed(
    ...
    *,
    ...
    # outer Python that drove ``qiqiclaw update``, not the venv we just wrote
    ...
    try:
        ...
        print("  ⚠ Repair install failed; check `qiqiclaw update` output above.")
        return
    ...
    try:
        ...
        print(
            ...
            "Run `qiqiclaw update --force` after closing other hermes processes."
        )
        return
...
def _update_node_dependencies() -> None:
    ...
    # need desktop during `qiqiclaw update`, so we install root-only first
...
def _cmd_update_check(branch: str = "main", *, branch_explicit: bool = False):
    ...
    if method == "docker":
        ...
        # same long-form ``docker pull`` guidance ``qiqiclaw update`` (apply
...
def _ensure_fhs_path_guard() -> None:
    ...
    path_comment = (
        "# QiQiClaw — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    ...
    HERMES_HOME directories.  The ``--backup`` flag on ``qiqiclaw update``
    ...
    # Render path using display_hermes_home so the user sees ~/.qiqiclaw/...
    try:
...
def _discard_lockfile_churn(git_cmd, repo_root):
    ...
    ``qiqiclaw update`` sees a clean tree instead of autostashing every run.
...
def cmd_update(args):
    ...
    if getattr(args, "check", False):
        ...
        # what a subsequent `qiqiclaw update --branch=<x>` would actually pull.
        branch = _resolve_update_branch(args)
        _cmd_update_check(
            branch=branch,
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # every user who ran ``qiqiclaw update`` for the 7 minutes between
            ...
            if not syntax_ok:
                ...
                if pre_pull_sha:
                    ...
                    if rollback_result.returncode == 0:
                        ...
                        print("  Try ``qiqiclaw update`` again later once a fix lands.")
                    else:
        ...
        # Electron build by ``qiqiclaw update``.
        desktop_dir = PROJECT_ROOT / "apps" / "desktop"
        ...
        # Tying the refresh to ``qiqiclaw update`` gives users a predictable
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            # path in `qiqiclaw gateway restart`
...
def cmd_profile(args):
    ...
    if action == "list":
        ...
        if data.get("hermes_requires"):
            print(f"Requires:     QiQiClaw {data['hermes_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    if mf.hermes_requires:
        print(f"  Requires: QiQiClaw {mf.hermes_requires}")
...
def _try_termux_fast_tui_launch() -> bool:
    ...
    `qiqiclaw --tui` is the hot path on phones. The full parser setup imports
...
def main():
    ...
    # ``qiqiclaw update`` runs on Windows. Silent no-op on non-Windows or when
    ...
    secrets_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "instead of storing them in ~/.qiqiclaw/.env.  Currently supports "
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate QiQiClaw with Spotify via PKCE"
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.qiqiclaw/scripts/. Default mode: "
    ...
    cron_create.add_argument(
        "--profile",
        help="QiQiClaw profile name to run the job under. Use 'default' for the root profile. Named profiles must already exist. Omit to preserve the scheduler's existing profile.",
    )
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.qiqiclaw/scripts/. Pass empty string to clear. "
    ...
    cron_edit.add_argument(
        "--profile",
        help="QiQiClaw profile name to run the job under. Use 'default' for the root profile. Pass empty string to clear.",
    )
    ...
    security_parser = subparsers.add_parser(
        ...
        description=(
            "On-demand vulnerability scan against OSV.dev. Covers the QiQiClaw "
            "venv (installed PyPI dists), Python deps declared by plugins under "
            "~/.qiqiclaw/plugins/, and pinned npx/uvx MCP servers in config.yaml. "
    ...
    audit_parser.add_argument(
        "--skip-venv",
        action="store_true",
        help="Skip scanning the QiQiClaw Python venv",
    )
    audit_parser.add_argument(
        "--skip-plugins",
        action="store_true",
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for QiQiClaw. Use 'hermes debug share' to "
...
""",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.qiqiclaw/checkpoints/",
    ...
    skills_opt_out = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "`qiqiclaw update`, and any direct sync stop seeding bundled skills "
    ...
    skills_opt_in = skills_subparsers.add_parser(
        ...
        description=(
            ...
            "again on the next `qiqiclaw update`. Pass --sync to re-seed now."
        ),
    )
    ...
    # subcommand — ``qiqiclaw --help``, ``hermes version``, ``qiqiclaw logs``,
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "'hermes mcp serve' to expose QiQiClaw conversations over MCP."
        ),
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print QiQiClaw ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive QiQiClaw provider/model setup for ACP terminal auth",
    )
    acp_parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.qiqiclaw/node/ "
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `qiqiclaw update` skill sync)",
    )
    profile_create.add_argument(
        "--description",
        default=None,
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a QiQiClaw profile distribution. SOURCE can be a git URL "
    ...
    # backend as `qiqiclaw dashboard --no-open --tui --host ... --port ...`. The
    ...
    # `qiqiclaw dashboard register` — register a self-hosted dashboard OAuth
    # client with Nous Portal and write the client_id into ~/.qiqiclaw/.env.
    # Nested subparser so bare `qiqiclaw dashboard` keeps launching the server
    ...
    dashboard_register_parser = dashboard_subparsers.add_parser(
        ...
        description=(
            ...
            "HERMES_DASHBOARD_OAUTH_CLIENT_ID into ~/.qiqiclaw/.env, and prints "
            "how to engage the login gate. Requires being logged in (qiqiclaw setup)."
        ),
    )
    ...
    gui_parser = subparsers.add_parser(
        ...
        description=(
            "Launch the QiQiClaw Electron desktop app. By default this installs "
    ...
    gui_parser.add_argument(
        "--hermes-root",
        help="Override the QiQiClaw source root used by Desktop (sets HERMES_DESKTOP_HERMES_ROOT)",
    )
    gui_parser.add_argument(
        "--cwd",
...
Examples:
    ...
    qiqiclaw logs gui -f             Follow gui.log in real time
    qiqiclaw logs desktop -f         Follow desktop.log (Electron app boot/backend)
