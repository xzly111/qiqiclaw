...
``qiqiclaw auth add nous --type oauth`` (which still works), is identical to
``qiqiclaw setup --portal``, and runs the same Nous flow as the first-time quick
setup.
...
already in ``qiqiclaw auth`` or ``hermes tools``. It's the onboarding + discovery
...
def _cmd_login(args) -> int:
    ...
    This is the human-readable front door for `qiqiclaw auth add nous --type
    oauth`. It reuses the exact wiring behind `qiqiclaw setup --portal` (which in
...
def portal_command(args) -> int:
    ...
    if sub in {None, "", "login"}:
        ...
        # human-readable alias for `qiqiclaw auth add nous --type oauth` /
        # `qiqiclaw setup --portal`.
...
def add_parser(subparsers) -> None:
    ...
    portal_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "the Tool Gateway (the human-readable alias for `qiqiclaw auth add "
            "nous --type oauth`, identical to `qiqiclaw setup --portal`). "
