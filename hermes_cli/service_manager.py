...
``qiqiclaw gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``qiqiclaw gateway start/stop/restart``).
    """
...
class S6ServiceManager:
    ...
    def _render_run_script(
        ...
        Special case: ``profile == "default"`` emits ``qiqiclaw gateway
        ...
        if profile == "default":
            gateway_cmd = "qiqiclaw gateway run"
        else:
    ...
    def _render_log_run(profile: str) -> str:
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``qiqiclaw logs`` reads and what
