"""On-demand supply-chain audit for QiQiClaw installs.
...
2. Python deps declared by user-installed plugins under ``~/.qiqiclaw/plugins``
...
def _discover_plugins(hermes_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.qiqiclaw/plugins``.
