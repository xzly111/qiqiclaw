...
def _warn_if_gateway_running(auto_yes: bool) -> None:
    ...
    print()
    print_error(
        "QiQiClaw gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'qiqiclaw gateway stop'.")
    print()
...
def _cmd_migrate(args):
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.qiqiclaw/. "
