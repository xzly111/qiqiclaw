...
manual BotFather token copy-paste. QiQiClaw talks only to the onboarding
...
def auto_setup_telegram_bot_result(
    ...
    print()
    print(f"  Contacting QiQiClaw Telegram onboarding service: {resolved_api_url}")
    sys.stdout.flush()
    pairing = create_pairing(resolved_api_url)
    if not pairing:
        print("  ✗ Could not reach the QiQiClaw Telegram onboarding service.")
