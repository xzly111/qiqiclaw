...
def reconcile_profile_gateways(
    *,
    ...
    is what ``qiqiclaw gateway start`` (no ``-p``) targets. Without it,
    bare ``qiqiclaw gateway start`` inside the container would land on
    ...
    # ``qiqiclaw gateway start`` (no ``-p``) has somewhere to land;
    ...
    if profiles_root.is_dir():
        for entry in sorted(profiles_root.iterdir()):
            ...
            # is not — that comes later via `qiqiclaw setup`). Use it as the
