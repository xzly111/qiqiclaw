...
def _get_ancestor_pids() -> set[int]:
    ...
    ``qiqiclaw gateway status`` from falsely counting the ``hermes`` CLI that
...
def _scan_gateway_pids(exclude_pids: set[int], all_profiles: bool = False) -> list[int]:
    ...
    # scan (e.g. ``qiqiclaw gateway status``) is never mistaken for a running
    # gateway.  See #13242.
...
def launch_detached_profile_gateway_restart(profile: str, old_pid: int) -> bool:
    ...
    # when the user closes the terminal, leaving ``qiqiclaw update`` users
    # with no running gateway until they re-invoke ``qiqiclaw gateway``
...
def _wait_for_systemd_service_restart(
    *,
    ...
    print(
        ...
        f"  Check status: {'sudo ' if system else ''}qiqiclaw gateway status\n"
...
def _print_systemd_start_limit_wait(system: bool = False) -> None:
    ...
    print(
        f"  Wait for the start-limit window to expire, then run: {'sudo ' if system else ''}qiqiclaw gateway restart{scope_flag}"
    )
...
def _print_gateway_process_mismatch(snapshot: GatewayRuntimeSnapshot) -> None:
    ...
    print("  This is usually a manual foreground/tmux/nohup run, so `qiqiclaw gateway`")
...
def _print_other_profiles_gateway_status() -> None:
    ...
    Shown at the bottom of ``qiqiclaw gateway status`` output so users with
...
def _windows_gateway_should_absorb_console_controls() -> bool:
    ...
    Foreground ``qiqiclaw gateway run`` must remain interruptible from
...
def get_service_name() -> str:
    ...
    Default ``~/.qiqiclaw`` returns ``hermes-gateway`` (backward compatible).
    Profile ``~/.qiqiclaw/profiles/coder`` returns ``hermes-gateway-coder``.
...
def remove_legacy_hermes_units(
    ...
    print()
    print("Legacy QiQiClaw gateway unit(s) found:")
    for name, path, is_system in legacy:
        scope = "system" if is_system else "user"
    ...
    if interactive and not prompt_yes_no("Remove these legacy units?", True):
        print("Skipped. Run again with: qiqiclaw gateway migrate-legacy")
        return 0, [p for _, p, _ in legacy]
...
def install_linux_gateway_from_setup(force: bool = False, enable_on_startup: bool = True) -> tuple[str | None, bool]:
    ...
    if scope == "system":
        ...
        if os.geteuid() != 0:  # windows-footgun: ok — Linux systemd install wizard, never invoked on Windows
            ...
            if run_as_user:
                print_info(
                    f"  After setup, run: sudo qiqiclaw gateway install --system --run-as-user {run_as_user}"
                )
            else:
                print_info(
                    "  After setup, run: sudo qiqiclaw gateway install --system --run-as-user <your-user>"
                )
...
def get_launchd_plist_path() -> Path:
    ...
    Default ``~/.qiqiclaw`` → ``ai.hermes.gateway.plist`` (backward compatible).
    Profile ``~/.qiqiclaw/profiles/coder`` → ``ai.hermes.gateway-coder.plist``.
    """
    suffix = _profile_suffix()
...
def _stable_service_working_dir() -> str:
    ...
    dir, or a clone that ``qiqiclaw update`` later relocates/removes — the path
...
def _print_system_scope_remediation(action: str) -> None:
    ...
    print_info("         sudo qiqiclaw gateway uninstall --system")
    print_info("         qiqiclaw gateway install")
    print_info("         qiqiclaw gateway start")
...
def systemd_install(
    ...
    print("Next steps:")
    print(
        f"  {'sudo ' if system else ''}qiqiclaw gateway start{scope_flag}              # Start the service"
    )
    print(
        f"  {'sudo ' if system else ''}qiqiclaw gateway status{scope_flag}             # Check status"
    )
    print(
...
def systemd_stop(system: bool = False):
    ...
    try:
        ...
        print(
            ...
            "check `qiqiclaw gateway status` or logs for final shutdown state."
        )
        return
    ...
    if pid is not None:
        ...
        try:
            ...
            print(
                ...
                "check `qiqiclaw gateway status` or logs for final state."
            )
            return
...
def systemd_status(deep: bool = False, system: bool = False, full: bool = False):
    ...
    if not systemd_unit_is_current(system=system):
        ...
        print(
            f"  Run: {'sudo ' if system else ''}qiqiclaw gateway restart{scope_flag}  # auto-refreshes the unit"
        )
        print()
    ...
    if active_state == "activating" and sub_state == "auto-restart":
        ...
        print(
            f"  Run after the start-limit window expires: {'sudo ' if system else ''}qiqiclaw gateway restart{scope_flag}"
        )
        print(
        ...
        print(
            f"  Run: systemctl {'--user ' if not system else ''}reset-failed {get_service_name()} && {'sudo ' if system else ''}qiqiclaw gateway start{scope_flag}"
        )
...
# (the documented `nohup qiqiclaw gateway run` workaround). See #23387.
...
def _spawn_detached_gateway() -> bool:
    ...
    macOS 26+ (issue #23387). Mirrors the `nohup qiqiclaw gateway run --replace`
...
def _launchd_fallback_to_detached(reason: str, *, exit_on_failure: bool = True) -> bool:
    ...
    if _spawn_detached_gateway():
        ...
        print("  Stop it with: qiqiclaw gateway stop")
        return True
    print_error("Failed to start the gateway as a background process.")
    print(
        f"  Try manually: nohup qiqiclaw gateway run --replace "
...
def _guard_official_docker_root_gateway() -> None:
    ...
    print_error(
        "Refusing to run the QiQiClaw gateway as root inside the official Docker image."
    )
    print(
...
def run_gateway(verbose: int = 0, quiet: bool = False, replace: bool = False):
    ...
    # from sibling CLI processes, but foreground `qiqiclaw gateway run` still
    ...
    # /restart) rather than through `qiqiclaw gateway restart` which already
    ...
    # `qiqiclaw gateway start/restart` — leaving the gateway vulnerable to
        ...
        if var.get("is_allowlist"):
            ...
            if value:
                ...
                if access_idx == 0:
                    ...
                    print_info(
                        "  Skipped — configure later with 'qiqiclaw gateway setup'"
                    )
            continue
...
def _setup_weixin():
    ...
    print_info(
        "  3. Hermes will store the returned account_id/token in ~/.qiqiclaw/.env."
    )
    print_info(
        ...
        if service_running:
            ...
            if supports_systemd_services() or is_macos() or is_windows():
                ...
                if start_now or start_on_login:
                    ...
                    if supports_systemd_services():
                        print_info(
                            "  Or as a boot-time service: sudo qiqiclaw gateway install --system"
                        )
                ...
                print_info(
                    "  For persistence:   tmux new -s hermes 'qiqiclaw gateway run'"
                )
                print_info(
                ...
                print_info("  Run in foreground: qiqiclaw gateway run")
                print_info(
                    f"  Or start it manually in the background (best effort): nohup qiqiclaw gateway run >{_dhh()}/logs/gateway.log 2>&1 &"
                )
            else:
...
def _dispatch_all_via_service_manager_if_s6(action: str) -> bool:
    ...
    Without this, ``qiqiclaw gateway stop --all`` and ``... restart --all``
...
def gateway_command(args):
    ...
    try:
        ...
        # The direct ``qiqiclaw gateway install|uninstall|start|stop|restart``
    ...
    if subcmd == "install":
        ...
        if supports_systemd_services():
            if is_wsl():
                ...
                print_info(
                    "  Consider running in foreground instead: qiqiclaw gateway run"
                )
                print_info(
                    "  Or use tmux/screen for persistence: tmux new -s hermes 'qiqiclaw gateway run'"
                )
                print()
            ...
            print()
            print(
                "  qiqiclaw gateway run                              # direct foreground"
            )
            print(
                "  tmux new -s hermes 'qiqiclaw gateway run'         # persistent via tmux"
            )
            print(
                "  nohup qiqiclaw gateway run > ~/.qiqiclaw/logs/gateway.log 2>&1 &  # background"
            )
            sys.exit(1)
        elif is_container():
            ...
            print()
            print("To run the gateway: qiqiclaw gateway run")
            sys.exit(0)
        else:
        ...
        if supports_systemd_services():
            ...
            print()
            print("Or run the gateway directly: qiqiclaw gateway run")
            sys.exit(0)
        else:
            print("Not supported on this platform.")
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "Use `qiqiclaw gateway stop` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if os.getenv("_HERMES_GATEWAY") == "1":
            print_error(
                ...
                "This command was blocked to prevent restart loops.\n"
                "Use `qiqiclaw gateway restart` from a shell outside the running gateway."
            )
            sys.exit(1)
        ...
        if supports_systemd_services() and (
            ...
            if pids:
                ...
                if is_termux():
                    print(
                        "  nohup qiqiclaw gateway run > ~/.qiqiclaw/logs/gateway.log 2>&1 &  # Best-effort background start"
                    )
                elif is_wsl():
                    print(
                        "  tmux new -s hermes 'qiqiclaw gateway run'         # persistent via tmux"
                    ...
                    print(
                        "  qiqiclaw gateway install  # Install as Windows Scheduled Task (auto-start on login)"
                    )
                else:
                    print("  qiqiclaw gateway install  # Install as user service")
                    print(
                        "  sudo qiqiclaw gateway install --system  # Install as boot-time system service"
                    )
