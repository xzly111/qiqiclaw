    ...
    for config_path in configs:
        try:
            ...
            for line in content.split('\n'):
                # Skip the legacy "# QiQiClaw" comment and following line
                if '# QiQiClaw' in line or '# hermes-agent' in line:
                    skip_next = True
                    continue
...
#   4. On the ``qiqiclaw dashboard`` + gateway paths, drops files into
...
def run_gui_uninstall(args):
    ...
    print(color("│            Q QiQiClaw Desktop Uninstaller              │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()

    if not summary["gui_installed"]:
        print("No QiQiClaw Desktop installation was found.")
        print(f"  Checked: {hermes_home}, and the standard app locations for this OS.")
        return

    print(color("This removes the desktop app only. The QiQiClaw backend stays installed.", Colors.CYAN))
    print()
    ...
    if agent_is_installed(hermes_home):
        ...
        print(f"  • The QiQiClaw backend at {hermes_home / 'hermes-agent'}")
    ...
    print(color("└─────────────────────────────────────────────────────────┘", Colors.GREEN, Colors.BOLD))
    print()
    print("The QiQiClaw backend is still installed. Run 'hermes' to use the compatible CLI,")
    print("or 'hermes uninstall' to remove the backend too.")
    print()
...
def run_uninstall(args):
    ...
    # full wipe (code + ~/.qiqiclaw data); otherwise keep-data. Named profiles
    ...
    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.MAGENTA, Colors.BOLD))
    print(color("│               Q QiQiClaw Uninstaller                   │", Colors.MAGENTA, Colors.BOLD))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.MAGENTA, Colors.BOLD))
    print()
    ...
    print()
    if full_uninstall:
        print(color("⚠️  WARNING: This will permanently delete ALL QiQiClaw data!", Colors.RED, Colors.BOLD))
        ...
        print("This will remove the QiQiClaw code but keep your configuration and data.")
    ...
    *,
    ...
    if _is_windows():
        ...
        if removed_path_entries:
            ...
            log_info("No QiQiClaw-owned PATH entries in User environment")
        ...
        if removed_env:
            ...
            log_info("No QiQiClaw-set User env vars to remove")
    ...
    log_info("Removing QiQiClaw-managed node/npm/npx symlinks...")
    ...
    if removed_node_links:
        ...
        log_info("No QiQiClaw-managed node/npm/npx symlinks found")
    ...
    try:
        if project_root.exists():
            # If the install is inside ~/.qiqiclaw/, just remove the hermes-agent subdir
    ...
    print()
    print("Thank you for using QiQiClaw!")
    print()
