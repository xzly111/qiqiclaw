...
def _list_targets(platform_filter: Optional[str], *, json_mode: bool) -> int:
    ...
    if not any(platforms.values()):
        ...
        print("Set one up with `qiqiclaw gateway setup`, or run the gateway once so")
        print("channel discovery can populate ~/.qiqiclaw/channel_directory.json.")
        return _SUCCESS_EXIT
...
def _load_hermes_env() -> None:
    """Populate ``os.environ`` from ``~/.qiqiclaw/.env`` AND bridge top-level
    ...
    1. ``load_dotenv(~/.qiqiclaw/.env)`` — brings bot tokens into the env.
    2. Bridge top-level simple values from ``~/.qiqiclaw/config.yaml`` into
...
def cmd_send(args: argparse.Namespace) -> None:
    ...
    # Bridge ~/.qiqiclaw/.env and ~/.qiqiclaw/config.yaml into os.environ so the
...
def register_send_subparser(subparsers) -> argparse.ArgumentParser:
    ...
    parser = subparsers.add_parser(
        ...
        description=(
            ...
            "credentials (~/.qiqiclaw/.env + ~/.qiqiclaw/config.yaml) — no LLM, "
