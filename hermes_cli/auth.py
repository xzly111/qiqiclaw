...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``qiqiclaw auth``.
    """
    return (
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.qiqiclaw/auth.json`` even when HERMES_HOME is set to a profile
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``qiqiclaw auth login <provider>`` inside
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``qiqiclaw auth add <provider>`` inside the profile, profile entries
...
def _assert_nous_inference_jwt_usable(
    ...
    *,
    ...
    raise AuthError(
        ...
        f"({reason}). Re-authenticate with: qiqiclaw auth add nous",
        provider="nous",
        code=reason,
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `qiqiclaw auth list` / `hermes status`."""
    try:
...
def _make_xai_callback_handler(expected_path: str) -> tuple[type[BaseHTTPRequestHandler], dict[str, Any]]:
    ...
    class _XAICallbackHandler(BaseHTTPRequestHandler):
        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            # ``~/.qiqiclaw/logs/agent.log`` which captures INFO+ unconditionally).
            try:
                logger.info(
            ...
            if incoming["code"] is None and incoming["error"] is None:
                ...
                body = (
                    ...
                    "<code>qiqiclaw auth add xai-oauth</code> to retry.</p>"
                    "</body></html>"
                )
                self.wfile.write(body.encode("utf-8"))
                return
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    print()
    print("Saved HERMES_SPOTIFY_CLIENT_ID to ~/.qiqiclaw/.env")
    print()
    return raw
...
def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read Codex OAuth tokens from Hermes auth store (~/.qiqiclaw/auth.json).
...
def _sync_codex_pool_entries(
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``qiqiclaw setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``qiqiclaw auth add openai-codex``
      ...
      ``qiqiclaw auth add`` workaround for #33000 would silently leave that
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save Codex OAuth tokens to Hermes auth store (~/.qiqiclaw/auth.json)."""
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    if response.status_code == 429:
        ...
        # "run qiqiclaw auth" prompt (see issue #32790).
...
# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.qiqiclaw/auth.json
# =============================================================================
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.qiqiclaw/auth.json``. A single MITM during initial
...
# so a new `qiqiclaw --profile <name> auth add nous --type oauth` can one-tap
...
# ``get_default_hermes_root()`` returns — ``~/.qiqiclaw`` on Linux/macOS,
...
def _nous_shared_auth_dir() -> Path:
    ...
    Linux/macOS classic installs land at ``~/.qiqiclaw/shared/``, native
    ...
    *,
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            ...
            "~/.qiqiclaw/auth.json) called POST /api/oauth/token with Hermes's "
...
def persist_nous_credentials(
    ...
    *,
    ...
    # these credentials via `qiqiclaw auth add nous --type oauth`. Best-
...
# mtime so that `qiqiclaw auth login/logout/add/remove` invalidate naturally on the next call.
...
def login_command(args) -> None:
    """Deprecated: use 'hermes model' or 'qiqiclaw setup' instead."""
    print("The 'hermes login' command has been removed.")
    print("Use 'qiqiclaw auth' to manage credentials,")
    print("'hermes model' to select a provider, or 'qiqiclaw setup' for full setup.")
    raise SystemExit(0)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hermes auth store (~/.qiqiclaw/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting QiQiClaw login via MiniMax ({region}) OAuth...")
...
def _nous_device_code_login(
    *,
    ...
    print(f"Starting QiQiClaw login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
