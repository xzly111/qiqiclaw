...
# default profile (``~/.qiqiclaw``).  Named profiles never contain these
...
def _clone_all_copytree_ignore(source_dir: Path):
    ...
    Two categories:
      1. Root-level entries in ``_CLONE_ALL_DEFAULT_EXCLUDE_ROOT`` — known
         ...
         (``~/.qiqiclaw``) ever contains.  Gated on ``source_dir`` actually
...
def _get_profiles_root() -> Path:
    ...
    ``~/.qiqiclaw``, profiles live under ``HERMES_HOME/profiles/`` so
...
def _get_default_hermes_home() -> Path:
    ...
    In Docker/custom deployments where HERMES_HOME is outside ``~/.qiqiclaw``
...
def validate_profile_name(name: str) -> None:
    ...
    collisions (a ``hermes`` profile inside ``~/.qiqiclaw/``) or get refused
...
def create_profile(
    ...
    no_skills:
        ...
        a marker file so ``qiqiclaw update`` skips re-seeding this profile's
    ...
    # Write the opt-out marker so seed_profile_skills() and `qiqiclaw update`'s
    ...
    if no_skills:
        try:
            (profile_dir / NO_BUNDLED_SKILLS_MARKER).write_text(
                ...
                "Delete this file to re-enable sync on the next `qiqiclaw update`.\n",
                encoding="utf-8",
            )
        except OSError:
...
def get_active_profile_name() -> str:
    ...
    Returns ``"default"`` if HERMES_HOME is not set or points to ``~/.qiqiclaw``.
    Returns the profile name if HERMES_HOME points into ``~/.qiqiclaw/profiles/<name>``.
