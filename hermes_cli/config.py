...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.qiqiclaw/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``qiqiclaw logs``
...
#   on every plugin install / ``qiqiclaw update``.
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    ...
    Resolution order:
    1. Stamped ``~/.qiqiclaw/.install_method`` file (written by installers)
    ...
    so ``qiqiclaw update`` bailed with "doesn't apply inside the Docker
...
def stamp_install_method(method: str) -> None:
    """Write the install method to ~/.qiqiclaw/.install_method."""
    stamp = get_hermes_home() / ".install_method"
    try:
...
def is_uv_tool_install() -> bool:
    ...
    ``qiqiclaw update`` to upgrade the wrong copy. It would also block on a
...
# Long-form text for ``qiqiclaw update`` / ``--check`` when running inside the
...
#     — that script installs a *new* host-side QiQiClaw, it doesn't update
...
✗ ``qiqiclaw update`` doesn't apply inside the Docker container.

QiQiClaw runs as a published image, not a
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``qiqiclaw update`` inside Docker.
...
DEFAULT_CONFIG = {
    ...
    "checkpoints": {
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.qiqiclaw/checkpoints/`` size (MB).  When
    ...
    "display": {
        ...
        # When true (default), `qiqiclaw --tui` drops a one-time hint
    ...
    "curator": {
        ...
        # normally restored on every `qiqiclaw update`, so pruning them only
        ...
        # skipped), snapshot ~/.qiqiclaw/skills/ into
        # ~/.qiqiclaw/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "QIQI-CLAW" header.
    ...
    "gateway": {
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.qiqiclaw/.env,
        ...
        # cache (~/.qiqiclaw/cache/{documents,images,audio,video,screenshots})
    ...
    "sessions": {
        ...
        # rewrites ``~/.qiqiclaw/sessions/session_{sid}.json`` on every turn
    ...
    "updates": {
        # Run a full ``qiqiclaw backup``-style zip of HERMES_HOME before every
        # ``qiqiclaw update``.  Backups land in ``<HERMES_HOME>/backups/`` and
        ...
        "backup_keep": 5,
        # What `qiqiclaw update` does with uncommitted local changes to the
    ...
    # rather than storing them in ~/.qiqiclaw/.env.
    "secrets": {
        "bitwarden": {
            ...
            # in ~/.qiqiclaw/.env (or your shell) and never in config.yaml.
            ...
            # ~/.qiqiclaw/bin/ on first use.  When False you must install
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HERMES_HOME", "~/.qiqiclaw")
...
def load_env() -> Dict[str, str]:
    ...
    interactive menu render (`hermes tools`, `qiqiclaw setup`, status
...
def show_config():
    ...
    print(color("│              Q QiQiClaw Configuration                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
