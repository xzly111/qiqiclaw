...
try:
    ...
    # running `qiqiclaw dashboard` needs fastapi+uvicorn; lazy install keeps
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``qiqiclaw gateway run`` — but the
    desktop app spawns a ``qiqiclaw dashboard`` backend, not a gateway, so a cron
...
app = FastAPI(title="QiQiClaw", version=__version__, lifespan=_lifespan)
...
async def check_hermes_update(force: bool = False):
    ...
    ``POST /api/hermes/update`` actually runs ``qiqiclaw update``.
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.qiqiclaw/config.yaml`` under ``tts.``.
    """
...
async def run_langgraph_workflow(body: LangGraphRunRequest):
    """Run the QiQiClaw LangGraph workflow through the desktop REST API."""
    try:
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "QiQiClaw").strip() or "QiQiClaw"
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.qiqiclaw/.anthropic_oauth.json`` — Hermes-managed PKCE flow
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    ...
    },
    {
        ...
        "flow": "loopback",
        "cli_command": "qiqiclaw auth add xai-oauth",
...
# the tokens exactly like `qiqiclaw auth add xai-oauth`.
...
def _add_xai_oauth_pool_entry(
    ...
    """Mirror `qiqiclaw auth add xai-oauth`'s credential-pool insert.
...
def _minimax_poller(session_id: str) -> None:
    ...
    ``qiqiclaw auth add minimax-oauth``.
    """
    from hermes_cli.auth import (
        _minimax_poll_token,
...
async def get_session_stats():
    """Session-store statistics for the Sessions page (mirrors `qiqiclaw sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions older than N days (mirrors `qiqiclaw sessions prune`)."""
...
# spawn the real `qiqiclaw gateway <verb>` so behaviour matches the CLI exactly.
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate):
    ...
    Writes each ``key: value`` to ``~/.qiqiclaw/.env`` via ``save_env_value`` —
...
def mount_spa(application: FastAPI):
    ...
    # (the MC Pages app), not the QiQiClaw backend. Intercept CSS asset
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"qiqiclaw auth {name}"
                        break
            except Exception:
                pass
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.qiqiclaw/plugins/ if you trust it)",
...
def start_server(
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # Each provider plugin that ships with QiQiClaw exposes a
