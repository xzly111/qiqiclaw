...
- Secrets prompted at install time go to ``~/.qiqiclaw/.env`` (the
...
def _do_git_install(entry: CatalogEntry) -> Path:
    """Clone the entry's repo into ``~/.qiqiclaw/mcp-installs/<name>`` and run
...
def _prompt_env_vars(specs: List[EnvVarSpec]) -> Dict[str, str]:
    ...
    non-secrets alike to ~/.qiqiclaw/.env via save_env_value()."""
...
def install_entry(entry: CatalogEntry, *, enable: bool = True) -> None:
    ...
    if entry.auth.type == "api_key":
        ...
        if entry.auth.provider:
            ...
            # the existing `qiqiclaw auth <provider>` flow. Surface guidance
            ...
            print(color(
                ...
                f"`qiqiclaw auth {entry.auth.provider}` if you have not "
