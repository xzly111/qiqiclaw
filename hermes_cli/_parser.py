...
Examples:
    ...
    qiqiclaw --tui                  Launch the modern TUI (or set display.interface: tui)
    qiqiclaw --cli                  Force the classic REPL (overrides display.interface: tui)
    hermes -c                     Resume the most recent session
    hermes -c "my project"        Resume a session by name (latest in lineage)
    qiqiclaw --resume <session_id>  Resume a specific session by ID
    qiqiclaw setup                  Run setup wizard
    hermes logout                 Clear stored authentication
    qiqiclaw auth add <provider>    Add a pooled credential
    qiqiclaw auth list              List pooled credentials
    qiqiclaw auth remove <p> <t>    Remove pooled credential by index, id, or label
    qiqiclaw auth reset <provider>  Clear exhaustion status for a provider
    ...
    qiqiclaw gateway                Run messaging gateway
    hermes -s hermes-agent-dev,github-auth
    hermes -w                     Start in isolated git worktree
    qiqiclaw gateway install        Install gateway background service
    qiqiclaw sessions list          List past sessions
    qiqiclaw sessions browse        Interactive session picker
    qiqiclaw sessions rename ID T   Rename/title a session
    qiqiclaw logs                   View agent.log (last 50 lines)
    qiqiclaw logs -f                Follow agent.log in real time
    qiqiclaw logs errors            View errors.log
    qiqiclaw logs --since 1h        Lines from the last hour
    hermes debug share             Upload debug report for support
    qiqiclaw update                 Update to latest version
    qiqiclaw dashboard              Start web UI dashboard (port 9119)
    qiqiclaw dashboard --stop       Stop running dashboard processes
    qiqiclaw dashboard --status     List running dashboard processes
...
def build_top_level_parser():
    ...
    parser = argparse.ArgumentParser(
        prog="hermes",
        description="QiQiClaw - AI assistant with tool-calling capabilities",
    ...
    _inherited_flag(
        ...
        help=(
            ...
            "under model.provider — use `qiqiclaw setup` or edit the file to change it."
        ),
    )
    parser.add_argument(
        "-t",
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=False,
        help="Ignore ~/.qiqiclaw/config.yaml and fall back to built-in defaults (credentials in .env are still loaded)",
    )
    _inherited_flag(
        parser,
        "--ignore-rules",
    ...
    chat_parser = subparsers.add_parser(
        ...
        description="Start an interactive chat session with QiQiClaw",
    )
    ...
    _inherited_flag(
        ...
        action="store_true",
        default=argparse.SUPPRESS,
        help="Ignore ~/.qiqiclaw/config.yaml and fall back to built-in defaults (credentials in .env are still loaded). Useful for isolated CI runs, reproduction, and third-party integrations.",
    )
    _inherited_flag(
        chat_parser,
        "--ignore-rules",
