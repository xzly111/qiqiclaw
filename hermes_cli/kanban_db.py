...
In standard installs ``<root>`` is ``~/.qiqiclaw``. In Docker / custom
deployments where ``HERMES_HOME`` points outside ``~/.qiqiclaw`` (e.g.
...
def _module_hermes_argv() -> list[str]:
    """Return the interpreter-bound QiQiClaw CLI invocation."""
...
def _kanban_worker_skill_available(hermes_home: Optional[str]) -> bool:
    ...
    # home (``~/.qiqiclaw``), which ships the bundled skill.
