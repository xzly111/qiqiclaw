...
_DHH = display_hermes_home()  # user-facing display path (e.g. ~/.qiqiclaw or ~/.qiqiclaw/profiles/coder)
...
def _check_version_consistency(issues: list[str]) -> None:
    ...
    other, leaving ``qiqiclaw --version`` reporting a stale version while
    ...
    if pyproject_version == init_version:
        ...
        _fail_and_issue(
            ...
            "Re-sync version files (e.g. run 'qiqiclaw update', or set "
...
def run_doctor(args):
    ...
    if ack_target:
        ...
        if ack_advisory(ack_target):
            ...
            print(color(
                ...
                f"Check ~/.qiqiclaw/config.yaml is writable.",
                Colors.RED,
            ))
            sys.exit(1)
        return
    ...
    print(color("│                 Q QiQiClaw Doctor                       │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if env_path.exists():
        ...
        if _has_provider_env_config(content):
            ...
            issues.append("Run 'qiqiclaw setup' to configure API keys")
    else:
        ...
        if fallback_env.exists():
            ...
            if should_fix:
                ...
                check_info("Run 'qiqiclaw setup' to configure API keys")
                fixed_count += 1
            else:
                check_info("Run 'qiqiclaw setup' to create one")
                issues.append("Run 'qiqiclaw setup' to create .env")
                ...
                try:
                    ...
                    if not configured:
                        _fail_and_issue(
                            ...
                            "(check ~/.qiqiclaw/.env or run 'qiqiclaw setup')",
                            (
                                f"No credentials found for provider '{runtime_provider}'. "
                                f"Run 'qiqiclaw setup' or set the provider's API key in {_DHH}/.env, "
            ...
            if current_ver < latest_ver:
                ...
                if should_fix:
                    try:
                        ...
                        issues.append("Run 'qiqiclaw setup' to migrate config")
                else:
                    issues.append("Run 'hermes doctor --fix' or 'qiqiclaw setup' to migrate config")
            else:
        ...
        try:
            ...
            if drift:
                check_warn(
                    ...
                    "(stale ghost from an earlier `qiqiclaw setup` run)",
                )
                if should_fix:
    ...
    if soul_path.exists():
        ...
        check_warn(f"{_DHH}/SOUL.md not found", "(create it to give QiQiClaw a custom personality)")
        if should_fix:
    ...
    try:
        ...
        if api_disabled:
            issues.append("Run 'qiqiclaw setup' to configure missing API keys for full tool access")
    except Exception as e:
