# OpenAI-Compatible API Server for QIQI-Claw
...
SSE streaming. By exposing this endpoint, QIQI-Claw becomes instantly
...
│  AnythingLLM     │  │    {"messages": [...]}             │  QIQI-Claw      │
│  NextChat        │  │                                    │  gateway        │
...
A user would:
1. Set `API_SERVER_ENABLED=true` in `~/.qiqiclaw/.env`
2. Run `qiqiclaw gateway` (API server starts alongside Telegram/Discord/etc.)
3. Point Open WebUI (or any frontend) at `http://localhost:8642/v1`
4. Chat with QIQI-Claw through any OpenAI-compatible UI
...
| GET | `/v1/models` | List available "models" (returns qiqiclaw as a model ID) |
...
Authorization: Bearer qiqiclaw-api-key-here
...
But QIQI-Claw sessions have persistent state (memory, skills, tool context).
...
Frontends send `"model": "qiqiclaw"` (or whatever). The actual LLM model
used is configured server-side in config.yaml. The API server maps any
requested model name to the configured QIQI-Claw model.
...
1. `gateway/platforms/api_server.py` — new adapter
   ...
   - Frontend system prompt layered on top of QIQI-Claw's core prompt
...
Once implemented, QIQI-Claw works as a drop-in backend for:
