...
description: Migrate a user's OpenClaw customization footprint into QIQI claw. Imports QiQi claw-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: QIQI claw (Nous Research)
license: MIT
metadata:
  qiqiclaw:
    tags: [Migration, OpenClaw, QiQi claw, Memory, Persona, Import]
    related_skills: [qiqiclaw]
---

# OpenClaw -> QiQi claw Migration

Use this skill when a user wants to move their OpenClaw setup into QIQI claw with minimal manual cleanup.

## OpenClaw plugin mechanism research

When the task is not migration execution but understanding what can be ported from OpenClaw, use the condensed upstream snapshot in `references/openclaw-plugin-mechanism.md`. It covers the native plugin vs compatible bundle split, manifest/control-plane vs runtime/data-plane boundary, registry/activation/contracts model, install/dependency behavior, and safe staged migration guidance for QiQi claw.
...
```bash
qiqiclaw claw migrate              # Full interactive migration
...
qiqiclaw claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `qiqiclaw setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_qiqiclaw.py` to:

- import `SOUL.md` into the QiQi claw home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into QiQi claw memory entries
- merge OpenClaw command approval patterns into QiQi claw `command_allowlist`
- migrate QiQi claw-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS` and `MESSAGING_CWD`
- copy OpenClaw skills into `~/.qiqiclaw/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen QiQi claw workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.qiqiclaw/tts/`
- archive non-secret docs that do not have a direct QiQi claw destination
...
- `scripts/openclaw_to_qiqiclaw.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py`

Do not guess a shorter path like `~/.qiqiclaw/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.qiqiclaw/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of QiQi claw-compatible secrets, currently:
...
QiQi claw CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in QiQi claw. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a QiQi claw workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing QiQi claw skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.qiqiclaw/skills/migration/openclaw-migration/scripts/openclaw_to_qiqiclaw.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of QiQi claw unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty QiQi claw targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean QiQi claw destination. Unsupported auth blobs must still be reported as skipped.
...
- QiQi claw persona state imported
- QiQi claw memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.qiqiclaw/skills/openclaw-imports/`
