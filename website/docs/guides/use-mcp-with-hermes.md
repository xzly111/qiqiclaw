---
sidebar_position: 6
title: "Use MCP with QiQiClaw"
description: "A practical guide to connecting MCP servers to QIQI-Claw, filtering their tools, and using them safely in real workflows"
---

# Use MCP with QiQiClaw

This guide shows how to actually use MCP with QIQI-Claw in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native QiQiClaw tool
- you want QiQiClaw to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect QiQiClaw to internal APIs, databases, or company systems without modifying QiQiClaw core

Do not use MCP when:
- a built-in QiQiClaw tool already solves the job well
...
- QiQiClaw remains the agent
- MCP servers contribute tools
- QiQiClaw discovers those tools at startup or reload time
...
If you installed QiQiClaw with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- QiQiClaw banner/status should show MCP integration when configured
- ask QiQiClaw what tools it has available
...
## WSL2: bridge QiQiClaw in WSL to Windows Chrome

This is the practical setup when:

- QiQiClaw runs inside WSL2
...
In this setup, QiQiClaw does **not** connect to Chrome directly. Instead:

- QiQiClaw runs in WSL
- QiQiClaw starts a local stdio MCP server
...
```text
QiQiClaw (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- QiQiClaw stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on QiQiClaw core browser transport
...
Then start a fresh QiQiClaw session or run:
...
Once loaded, QiQiClaw can use the MCP-prefixed browser tools directly. For example:
...
If QiQiClaw runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start QiQiClaw from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start QiQiClaw from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in QiQiClaw:
...
2. QiQiClaw-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So QiQiClaw will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want QiQiClaw to reason over a bounded workspace.
...
Now QiQiClaw can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing QiQiClaw core.
...
Because QiQiClaw now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
