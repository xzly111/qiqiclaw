...
2. **QiQiClaw spins up** a fresh agent session with your prompt
...
QiQiClaw will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by QIQI-Claw
```
...
Before creating cron jobs, ensure QiQiClaw has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell QiQiClaw what you want:
...
QiQiClaw will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell QiQiClaw to delegate each topic to a sub-agent:
...
QiQiClaw will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
