...
messaging platform QiQiClaw is already configured for. Think of it as a
...
Any platform QiQiClaw ships adapters for works as a target:
...
When stdin is a TTY (no pipe), QiQiClaw does **not** wait for input — you'll
...
Because `hermes send` reuses your QiQiClaw config, the same script works on
any host where QiQiClaw is installed — no need to export bot tokens into
...
| Approach | Multi-platform | Reuses QiQiClaw creds | Needs gateway | Best for |
|----------|----------------|---------------------|---------------|----------|
