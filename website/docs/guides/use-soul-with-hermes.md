---
sidebar_position: 7
title: "Use SOUL.md with QiQiClaw"
description: "How to use SOUL.md to shape QIQI-Claw's default voice, what belongs there, and how it differs from AGENTS.md and /personality"
---

# Use SOUL.md with QiQiClaw

`SOUL.md` is the **primary identity** for your QiQiClaw instance. It's the first thing in the system prompt — it defines who the agent is, how it speaks, and what it avoids.

If you want QiQiClaw to feel like the same assistant every time you talk to it — or if you want to replace the QiQiClaw persona entirely with your own — this is the file to use.
...
- personality
- communication style
- how direct or warm QiQiClaw should be
- what QiQiClaw should avoid stylistically
- how QiQiClaw should relate to uncertainty, disagreement, and ambiguity

In short:
- `SOUL.md` is about who QiQiClaw is and how QiQiClaw speaks
...
QiQiClaw now uses only the global SOUL file for the current instance:

```text
~/.hermes/SOUL.md
```

If you run QiQiClaw with a custom home directory, it becomes:
...
QiQiClaw automatically seeds a starter `SOUL.md` for you if one does not already exist.

That means most users now begin with a real file they can read and edit immediately.

Important:
- if you already have a `SOUL.md`, QiQiClaw does not overwrite it
- if the file exists but is empty, QiQiClaw adds nothing from it to the prompt

## How QiQiClaw uses it

When QiQiClaw starts a session, it reads `SOUL.md` from `HERMES_HOME`, scans it for prompt-injection patterns, truncates it if needed, and uses it as the **agent identity** — slot #1 in the system prompt. This means SOUL.md completely replaces the built-in default identity text.
...
That alone can noticeably change how QiQiClaw feels.
...
QiQiClaw already tries to be helpful and clear. `SOUL.md` should add real personality and style, not restate obvious defaults.
...
# Style
How QiQiClaw should sound.

# Avoid
What QiQiClaw should not do.

# Defaults
How QiQiClaw should behave when ambiguity appears.
```
...
Then restart QiQiClaw or start a new session.
...
4. Talk to QiQiClaw for a while
...
### I edited SOUL.md but QiQiClaw still sounds the same
...
### QiQiClaw is ignoring parts of my SOUL.md
