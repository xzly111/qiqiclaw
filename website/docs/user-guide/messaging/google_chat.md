...
sidebar_position: 12
title: "Google Chat"
description: "Set up QIQI-Claw as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect QIQI-Claw to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your QiQiClaw
...
download the file. Save it somewhere only QiQiClaw can read (e.g.,
...
Also grant `Pub/Sub Viewer` on the same subscription — QiQiClaw calls
...
- **App name**: whatever you want users to see ("QiQiClaw" is reasonable).
...
`ADDED_TO_SPACE` event that QiQiClaw uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure QiQiClaw
...
Install the dependencies the Google Chat adapter needs (no QiQiClaw extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "QiQiClaw is thinking…" marker, then
...
Thread support: when a user replies inside a thread, QiQiClaw detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate QiQiClaw session.
...
3. Download the JSON. Move it onto the host that runs QiQiClaw.
4. Register the client with QiQiClaw (run under the profile you want it scoped to):
...
That writes the client secret into the active profile's QiQiClaw home (e.g.
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, QiQiClaw isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
- **Attachment download protection**: QiQiClaw will only attach the SA bearer
