...
description: Control your smart home with QIQI-Claw via Home Assistant integration.
...
QIQI-Claw integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "QIQI-Claw"
5. Copy the token
...
QIQI-Claw registers four tools for smart home control:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "QIQI-Claw".
...
from `config.yaml`. Double-check the file lives under the active QiQiClaw profile
...
running QiQiClaw — `curl -H "Authorization: Bearer <token>" <url>/api/` should
