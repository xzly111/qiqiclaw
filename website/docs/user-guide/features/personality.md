...
description: "Customize QIQI-Claw's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

QIQI-Claw's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who QiQiClaw is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

QiQiClaw now seeds a default `SOUL.md` automatically in:

```text
~/.hermes/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run QiQiClaw with a custom home directory, it will use:
...
- QiQiClaw creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- QiQiClaw loads `SOUL.md` only from `HERMES_HOME`
- QiQiClaw does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, QiQiClaw falls back to a built-in default identity
...
If QiQiClaw loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the QiQiClaw instance itself.

That also makes it easier to teach users:
- "Edit `~/.hermes/SOUL.md` to change QiQiClaw's default personality."
...
- how QiQiClaw should handle uncertainty, disagreement, or ambiguity
...
## What QiQiClaw injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, QiQiClaw falls back to a built-in default identity ("You are QIQI-Claw, an intelligent AI assistant created by Nous Research..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
QiQiClaw ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain QiQiClaw, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives QiQiClaw its persistent default personality unless the overlay meaningfully changes it.
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how QiQiClaw speaks
- `display.skin` and `/skin` affect how QiQiClaw looks in the terminal
