...
**Tool Search** is QiQiClaw's opt-in progressive-disclosure layer for that
...
:::info Built-in QiQiClaw tools never defer
The tools that make up QiQiClaw's core capability set (`terminal`,
...
When the model invokes `tool_call`, QiQiClaw **unwraps the bridge** and
...
- **No JS sandbox.** QiQiClaw uses the simpler "structured tools" mode
