...
description: "Configure and use Honcho memory with QiQiClaw -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
# Honcho Memory for QiQiClaw

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every QiQiClaw profile its own peer identity while sharing a unified view of the user.
...
3. **AI peer card** -- the identity card for this QiQiClaw profile's AI peer
...
Honcho models conversations as interactions between **peers**. QiQiClaw creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this QiQiClaw instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each QiQiClaw run |
...
Each QiQiClaw profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
Guidelines for QiQiClaw when Honcho memory is active.
...
| `hermes honcho sync` | Create host blocks for all QiQiClaw profiles that don't have one yet |
| `hermes honcho migrate` | Step-by-step migration guide from OpenClaw native memory to QiQiClaw + Honcho |
