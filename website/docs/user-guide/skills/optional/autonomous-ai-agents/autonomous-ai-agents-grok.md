...
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), QIQI-Claw |
| License | MIT |
...
# Grok Build CLI — QiQiClaw Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the QiQiClaw terminal. Grok
...
1. Prepare stable input files first with QiQiClaw tools (`read_file`,
...
2. **Don't conflate QiQiClaw's xAI auth with the `grok` CLI's auth.** '
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume QiQiClaw's xAI auth carries
   over.
