...
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by QIQI-Claw |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during QiQiClaw terminal calls (desktop app flow only)
...
### Service Account (recommended for QiQiClaw)
...
## QiQiClaw Execution Pattern (desktop app flow)

QiQiClaw terminal commands are non-interactive by default and can lose auth context between calls.
