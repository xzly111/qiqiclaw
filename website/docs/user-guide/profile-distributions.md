...
A **profile distribution** packages a complete QIQI-Claw — personality, skills, cron jobs, MCP connections, config — as a git repository. Anyone with access to the repo can install the whole agent with one command, update it in place, and keep their own memories, sessions, and API keys untouched.

If a [profile](./profiles.md) is a local agent, a distribution is that agent made shareable.

## What this means

Before distributions, sharing a QIQI-Claw meant sending someone:
...
The tradeoff: recipients need git installed. On any machine running QiQiClaw in 2026, that's already true.
...
You built QiQiClaw-on-top — maybe a compliance-monitoring harness, a customer-support stack, a domain-specific research platform. You want to distribute it as a product.
...
- [Using SOUL with QiQiClaw](../guides/use-soul-with-hermes.md) — authoring personalities
