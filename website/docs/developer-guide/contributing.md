...
sidebar_position: 4
title: "Contributing"
description: "How to contribute to QIQI-Claw — dev setup, code style, PR process"
---

# Contributing

Thank you for contributing to QIQI-Claw! This guide covers setting up your dev environment, understanding the codebase, and getting your PR merged.
...
- Building a custom/local tool without modifying QiQiClaw core? Start with [Build a QiQiClaw Plugin](../guides/build-a-hermes-plugin.md)
- Building a new built-in core tool for QiQiClaw itself? Start with [Adding Tools](./adding-tools.md)
...
- **Profile-safe paths**: Never hardcode `~/.hermes` — use `get_hermes_home()` from `hermes_constants` for code paths and `display_hermes_home()` for user-facing messages. See [AGENTS.md](https://github.com/xzly111/qiqiclaw/blob/main/AGENTS.md#profiles-multi-instance-support) for full rules.

## Cross-Platform Compatibility

QiQiClaw officially supports **Linux, macOS, WSL2, and native Windows (via PowerShell install)**.  Native Windows uses Git Bash (from [Git for Windows](https://git-scm.com/download/win)) for shell commands.  A few features require POSIX kernel primitives and are gated: the dashboard's embedded PTY terminal pane (`/chat` tab) is WSL2-only. If you're doing Windows-heavy dev, run the Windows-footgun lint (`scripts/check-windows-footguns.py`) before pushing.
...
QiQiClaw has terminal access. Security matters.
...
- Include: OS, Python version, QiQiClaw version (`hermes version`), full error traceback
...
By contributing, you agree that your contributions will be licensed under the [MIT License](https://github.com/xzly111/qiqiclaw/blob/main/LICENSE).
