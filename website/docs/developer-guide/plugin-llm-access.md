...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept QiQiClaw already has applies. The plugin never sees a
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — QiQiClaw provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different QiQiClaw provider
        # (must be one QiQiClaw already knows about — same names as
        ...
        # plugin sends — QiQiClaw does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through QiQiClaw's usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing QiQiClaw subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/xzly111/qiqiclaw/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/xzly111/qiqiclaw/blob/main/tests/agent/test_plugin_llm.py)
