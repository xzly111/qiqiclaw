...
sidebar_position: 1
title: "Architecture"
description: "QIQI-Claw internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of QIQI-Claw internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # QiQiClawCLI — interactive terminal UI (large file)
...
```text
User input → QiQiClawCLI.process_input()
...
Exposes QiQiClaw as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
