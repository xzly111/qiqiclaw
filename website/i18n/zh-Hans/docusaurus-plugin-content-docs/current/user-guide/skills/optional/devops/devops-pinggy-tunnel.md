...
远程客户端使用 `Authorization: Bearer $TOKEN` 连接到 `$URL`。QiQiClaw 原生 MCP 客户端配置：`{"transport": "http", "url": "<URL>", "headers": {"Authorization": "Bearer <TOKEN>"}}`。
