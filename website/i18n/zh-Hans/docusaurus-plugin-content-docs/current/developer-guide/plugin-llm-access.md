...
* **宿主持有凭据。** OAuth token、刷新流程、凭据池、每任务辅助覆盖——QiQiClaw 已有的所有凭据概念均适用。Plugin 永远看不到 token；宿主通过 `result.audit` 将调用归因回溯。
...
result = ctx.llm.complete(
    ...
    provider=None,         # 可选，受门控——QiQiClaw provider id（如 "openrouter"）
...
plugins:
  entries:
    my-plugin:
      llm:
        # 允许此 plugin 选择不同的 QiQiClaw provider
        # （必须是 QiQiClaw 已知的 provider——与
...
* **回退链。** 若用户主 provider 返回 5xx 或 429，请求在向 plugin 返回错误前会经过 QiQiClaw 常规的聚合器感知回退流程。
...
现有 `ctx.*` 方法各自扩展一个已有的 QiQiClaw 子系统：
...
* 实现：[`agent/plugin_llm.py`](https://github.com/xzly111/qiqiclaw/blob/main/agent/plugin_llm.py)
* 测试：[`tests/agent/test_plugin_llm.py`](https://github.com/xzly111/qiqiclaw/blob/main/tests/agent/test_plugin_llm.py)
* 参考 plugin（配套仓库）：
