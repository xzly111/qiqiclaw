...
4. 填写以下内容：
   - **Name：**`QiQiClaw Teams Meeting Pipeline`（或任何你能识别的名称）。
...
```powershell
# Create a policy scoped to the QiQiClaw app
New-CsApplicationAccessPolicy `
  -Identity "QiQiClaw-Meeting-Pipeline-Policy" `
  -AppIds "<MSGRAPH_CLIENT_ID>" `
  -Description "Restrict QiQiClaw meeting pipeline to allow-listed users"

# Grant the policy to specific users whose meetings the pipeline may read
Grant-CsApplicationAccessPolicy `
  -PolicyName "QiQiClaw-Meeting-Pipeline-Policy" `
...
QiQiClaw 内置了 Graph 身份验证冒烟测试。在 QiQiClaw 安装目录下执行：
