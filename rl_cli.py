...
from qiqiclaw_constants import OPENROUTER_BASE_URL, get_qiqiclaw_home
...
_loaded_env_paths = load_qiqiclaw_dotenv(qiqiclaw_home=_qiqiclaw_home, project_env=_project_env)
...
if tinker_atropos_dir.exists():
    ...
    _set_legacy_env('QUIET', '1')  # Disable temp subdirectory creation
    print(f"📂 Terminal working directory: {tinker_atropos_dir}")
else:
    # Fall back to qiqiclaw directory if submodule not found
...
def load_hermes_config() -> dict:
    ...
    """
    config_path = _qiqiclaw_home / 'config.yaml'
...
def main(
    ...
    Args:
        ...
        model: Model to use for the agent (reads from ~/.qiqiclaw/config.yaml if not provided)
    ...
    """
    # Load config from ~/.qiqiclaw/config.yaml
    ...
    if check_server:
        ...
        if ok:
            ...
            if missing:
                ...
                print("   Add them to ~/.qiqiclaw/.env")
            else:
