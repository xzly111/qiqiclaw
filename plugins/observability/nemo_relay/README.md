...
Optional QiQiClaw observability plugin that maps QiQiClaw observer hooks to
...
not replace QIQI-Claw's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets QiQiClaw emit NeMo Relay lifecycle events for
the work QiQiClaw already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, QIQI-Claw can:

- Preserve QiQiClaw execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
`HERMES_HOME` is the QiQiClaw profile/config home used by both
`hermes plugins enable ...` and the later `hermes chat ...` run. If unset,
QiQiClaw uses the user's default home, usually `~/.hermes`. For isolated smoke
...
exposes `llm.execute(...)` / `tools.execute(...)`, QiQiClaw routes LLM and tool
...
For the full generic QiQiClaw middleware contract, see
...
This run starts a parent QiQiClaw session, delegates to a child subagent, has the
...
export HERMES_NEMO_RELAY_ATIF_AGENT_NAME='QIQI-Claw E2E'
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "QIQI-Claw E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. QiQiClaw dispatches the read-only tools as one batch, and NeMo Relay
...
- QiQiClaw sessions map to `agent` scopes.
- QiQiClaw API request hooks map to `llm` scope start/end events.
- QiQiClaw tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses QiQiClaw execution middleware to hand
...
When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, QiQiClaw routes execution
through these boundaries:

```text
QiQiClaw provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> QiQiClaw provider adapter next_call(...)

QiQiClaw tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> QiQiClaw tool dispatcher next_call(...)
```
...
middleware for a local QiQiClaw run.
...
agent_name = "QiQiClaw Middleware E2E"
agent_version = "local"
