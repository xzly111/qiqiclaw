# QiQi claw Achievements Performance Spec (Post-Hackathon)

Status: Draft (no code changes yet)
Owner: qiqiclaw-achievements plugin
...
### `GET /api/plugins/qiqiclaw-achievements/achievements`
...
### `POST /api/plugins/qiqiclaw-achievements/rescan` (optional)
Manual refresh trigger.
Prefer async trigger + immediate status response.

### `GET /api/plugins/qiqiclaw-achievements/scan-status` (optional new)
Reports scan state for UX/ops.

### Removed
- `GET /api/plugins/qiqiclaw-achievements/overview`
...
- Persist plugin data under:
  - `~/.qiqiclaw/plugins/qiqiclaw-achievements/scan_snapshot.json`

### Phase 2
- Incremental scan checkpoints with per-session fingerprints.
- Persist checkpoint data under:
  - `~/.qiqiclaw/plugins/qiqiclaw-achievements/scan_checkpoint.json`
...
- `~/.qiqiclaw/plugins/qiqiclaw-achievements/`
